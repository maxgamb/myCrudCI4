...
final class CountryApiArchitectureContractTest extends CIUnitTestCase
{
    public function testApiControllerUsesModelForReadsAndServiceForWritesWithoutSql(): void
    {
        $path = APPPATH . 'Controllers/Api/V1/CountryApiController.php';
        $this->assertFileExists($path);
        $php = (string) file_get_contents($path);

        $this->assertStringContainsString('private readonly CountryModel $model', $php);
        $this->assertStringContainsString('private readonly CountryService $service', $php);
    ...
    {
        $service = (string) file_get_contents(APPPATH . 'Services/CountryService.php');
    ...
    {
        $resource = (string) file_get_contents(APPPATH . 'API/Resources/CountryResource.php');
