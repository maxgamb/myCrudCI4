...
final class InventoryApiArchitectureContractTest extends CIUnitTestCase
{
    public function testApiControllerUsesModelForReadsAndServiceForWritesWithoutSql(): void
    {
        $path = APPPATH . 'Controllers/Api/V1/InventoryApiController.php';
        $this->assertFileExists($path);
        $php = (string) file_get_contents($path);

        $this->assertStringContainsString('private readonly InventoryModel $model', $php);
        $this->assertStringContainsString('private readonly InventoryService $service', $php);
    ...
    {
        $service = (string) file_get_contents(APPPATH . 'Services/InventoryService.php');
    ...
    {
        $resource = (string) file_get_contents(APPPATH . 'API/Resources/InventoryResource.php');
