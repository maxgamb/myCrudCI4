...
final class CityApiArchitectureContractTest extends CIUnitTestCase
{
    public function testApiControllerUsesModelForReadsAndServiceForWritesWithoutSql(): void
    {
        $path = APPPATH . 'Controllers/Api/V1/CityApiController.php';
        $this->assertFileExists($path);
        $php = (string) file_get_contents($path);

        $this->assertStringContainsString('private readonly CityModel $model', $php);
        $this->assertStringContainsString('private readonly CityService $service', $php);
    ...
    {
        $service = (string) file_get_contents(APPPATH . 'Services/CityService.php');
    ...
    {
        $resource = (string) file_get_contents(APPPATH . 'API/Resources/CityResource.php');
