...
use App\Models\NazioniLinqueModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class NazioniLinqueService
{
    ...
    public function __construct(private readonly NazioniLinqueModel $model = new NazioniLinqueModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new NazioniLinqueEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('isoKey', $data) && (is_int($data['isoKey']) || is_string($data['isoKey']))) {
            return $data['isoKey'];
        }
