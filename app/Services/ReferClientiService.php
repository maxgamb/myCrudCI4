...
use App\Models\ReferClientiModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class ReferClientiService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'ref_clinti_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly ReferClientiModel $model = new ReferClientiModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ReferClientiEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('conto_id', $data) && (is_int($data['conto_id']) || is_string($data['conto_id']))) {
            return $data['conto_id'];
        }
