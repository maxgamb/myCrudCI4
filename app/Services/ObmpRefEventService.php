...
use App\Models\ObmpRefEventModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class ObmpRefEventService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'ref_event_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly ObmpRefEventModel $model = new ObmpRefEventModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ObmpRefEventEntity($data), true);
        if ($id === false) {
