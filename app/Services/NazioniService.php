...
final class NazioniService
{
    ...
    public function __construct(private readonly NazioniModel $model = new NazioniModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new NazioniEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('Nazioni_Id_Codice', $data) && (is_int($data['Nazioni_Id_Codice']) || is_string($data['Nazioni_Id_Codice']))) {
            return $data['Nazioni_Id_Codice'];
        }
