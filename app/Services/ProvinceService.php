...
final class ProvinceService
{
    ...
    public function __construct(private readonly ProvinceModel $model = new ProvinceModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ProvinceEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('Prov', $data) && (is_int($data['Prov']) || is_string($data['Prov']))) {
            return $data['Prov'];
        }
