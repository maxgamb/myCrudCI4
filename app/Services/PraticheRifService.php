...
use App\Models\PraticheRifModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class PraticheRifService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'pratica_rif_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly PraticheRifModel $model = new PraticheRifModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new PraticheRifEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('pratica_rif_pratica_id', $data) && (is_int($data['pratica_rif_pratica_id']) || is_string($data['pratica_rif_pratica_id']))) {
            return $data['pratica_rif_pratica_id'];
        }
