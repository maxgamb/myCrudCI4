...
use App\Models\ContiNoteModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class ContiNoteService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'conto_nota_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly ContiNoteModel $model = new ContiNoteModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ContiNoteEntity($data), true);
        if ($id === false) {
