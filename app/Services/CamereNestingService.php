...
use App\Models\CamereNestingModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class CamereNestingService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'nesting_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly CamereNestingModel $model = new CamereNestingModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new CamereNestingEntity($data), true);
        if ($id === false) {
