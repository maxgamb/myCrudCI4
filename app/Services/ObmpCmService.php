...
use App\Models\ObmpCmModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class ObmpCmService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'obmp_cm_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly ObmpCmModel $model = new ObmpCmModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ObmpCmEntity($data), true);
        if ($id === false) {
