...
final class ComuniService
{
    ...
    public function __construct(private readonly ComuniModel $model = new ComuniModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ComuniEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('Comuni_Codice', $data) && (is_int($data['Comuni_Codice']) || is_string($data['Comuni_Codice']))) {
            return $data['Comuni_Codice'];
        }
