...
use App\Models\DocFileModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class DocFileService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'doc_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly DocFileModel $model = new DocFileModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new DocFileEntity($data), true);
        if ($id === false) {
