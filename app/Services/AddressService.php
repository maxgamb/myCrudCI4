...
use App\Services\Extensions\AddressServiceExtension;
use App\Models\AddressModel;
use App\Validation\AddressRules;
use RuntimeException;

/**
 * Service applicativo per la risorsa `address`.
 *
 * Responsabilità:
 ...
 *
 * Queries remain the responsibility of AddressModel.
 */
final class AddressService
{
    use AddressServiceExtension;
  ...
  1 => 'postal_code',
    ...
    public function __construct(private readonly AddressModel $model = new AddressModel())
    {
    }

    /**
     * Creates the parent resource for relation city_id.
     *
     * Delegates the write to CityService; this Service only orchestrates the FK assignment.
     *
     * @param array<string,mixed> $payload Parent resource payload.
     * @return int|string Created parent identifier.
     */
    private function createCityForCityId(array $payload): int|string
    {
        return (new CityService())->createRelated($payload);
    }
    /** @param array<string,mixed> $data */
    private function validateCreatePayload(array $data): void
    {
        $this->validatePayload($data, AddressRules::createRules(), AddressRules::messages(), 'Create validation failed.');
    }
    /** @param array<string,mixed> $data */
    private function validateUpdatePayload(int|string $id, array $data): void
    {
        $this->validatePayload($data, AddressRules::updateRules($id), AddressRules::messages(), 'Update validation failed.');
    }
    /**
    ...
    public function create(
        ...
        try {
            if (isset($related['city_id']) && is_array($related['city_id'])) {
                $data['city_id'] = $this->createCityForCityId($related['city_id']);
            }
    ...
    {
        $data = $this->prepareData($data);
        $rules = array_intersect_key(AddressRules::updateRules($id), $data);
        if ($rules !== []) {
            $this->validatePayload($data, $rules, AddressRules::messages(), 'Patch validation failed.');
        }
        $data = $this->beforeUpdate($id, $data);
        if (!$this->model->updateRecord($id, $data)) {
