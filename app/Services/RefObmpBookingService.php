...
use App\Models\RefObmpBookingModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class RefObmpBookingService
{
    ...
    public function __construct(private readonly RefObmpBookingModel $model = new RefObmpBookingModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new RefObmpBookingEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('ref_obm_data', $data) && (is_int($data['ref_obm_data']) || is_string($data['ref_obm_data']))) {
            return $data['ref_obm_data'];
        }
