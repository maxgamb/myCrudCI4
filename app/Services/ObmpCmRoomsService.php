...
use App\Models\ObmpCmRoomsModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class ObmpCmRoomsService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'obmp_cm_rooms_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly ObmpCmRoomsModel $model = new ObmpCmRoomsModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ObmpCmRoomsEntity($data), true);
        if ($id === false) {
