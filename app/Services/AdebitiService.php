...
use App\Models\AdebitiModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class AdebitiService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'adebiti_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly AdebitiModel $model = new AdebitiModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new AdebitiEntity($data), true);
        if ($id === false) {
