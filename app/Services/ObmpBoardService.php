...
final class ObmpBoardService
{
    ...
    public function __construct(private readonly ObmpBoardModel $model = new ObmpBoardModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ObmpBoardEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('obmp_board_id', $data) && (is_int($data['obmp_board_id']) || is_string($data['obmp_board_id']))) {
            return $data['obmp_board_id'];
        }
