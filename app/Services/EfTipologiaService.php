...
use App\Models\EfTipologiaModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class EfTipologiaService
{
    ...
    public function __construct(private readonly EfTipologiaModel $model = new EfTipologiaModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new EfTipologiaEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('pax', $data) && (is_int($data['pax']) || is_string($data['pax']))) {
            return $data['pax'];
        }
