...
use App\Models\PrezziModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class PrezziService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'prezzo_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly PrezziModel $model = new PrezziModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new PrezziEntity($data), true);
        if ($id === false) {
