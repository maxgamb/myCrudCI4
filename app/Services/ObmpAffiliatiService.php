...
use App\Models\ObmpAffiliatiModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class ObmpAffiliatiService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'obmp_aff_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly ObmpAffiliatiModel $model = new ObmpAffiliatiModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ObmpAffiliatiEntity($data), true);
        if ($id === false) {
