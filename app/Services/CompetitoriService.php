...
use App\Models\CompetitoriModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class CompetitoriService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'competitore_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly CompetitoriModel $model = new CompetitoriModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new CompetitoriEntity($data), true);
        if ($id === false) {
