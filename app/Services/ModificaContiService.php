...
use App\Models\ModificaContiModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class ModificaContiService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'mod_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly ModificaContiModel $model = new ModificaContiModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ModificaContiEntity($data), true);
        if ($id === false) {
