...
use App\Models\CassaModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class CassaService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'cassa_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly CassaModel $model = new CassaModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new CassaEntity($data), true);
        if ($id === false) {
