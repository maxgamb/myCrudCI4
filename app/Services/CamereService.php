...
use App\Models\CamereModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class CamereService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'camere_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly CamereModel $model = new CamereModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new CamereEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('camera_id', $data) && (is_int($data['camera_id']) || is_string($data['camera_id']))) {
