...
use App\Models\NazioniBandieraModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class NazioniBandieraService
{
    ...
    public function __construct(private readonly NazioniBandieraModel $model = new NazioniBandieraModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new NazioniBandieraEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('nazione_iso2', $data) && (is_int($data['nazione_iso2']) || is_string($data['nazione_iso2']))) {
            return $data['nazione_iso2'];
        }
