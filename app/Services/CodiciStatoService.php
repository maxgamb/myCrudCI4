...
use App\Models\CodiciStatoModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class CodiciStatoService
{
    ...
    public function __construct(private readonly CodiciStatoModel $model = new CodiciStatoModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new CodiciStatoEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('cod_stato', $data) && (is_int($data['cod_stato']) || is_string($data['cod_stato']))) {
            return $data['cod_stato'];
        }
