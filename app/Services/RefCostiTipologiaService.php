...
use App\Models\RefCostiTipologiaModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class RefCostiTipologiaService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'ref_costi_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly RefCostiTipologiaModel $model = new RefCostiTipologiaModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new RefCostiTipologiaEntity($data), true);
        if ($id === false) {
