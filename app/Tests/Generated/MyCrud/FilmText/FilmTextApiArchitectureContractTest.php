...
final class FilmTextApiArchitectureContractTest extends CIUnitTestCase
{
    public function testApiControllerUsesModelForReadsAndServiceForWritesWithoutSql(): void
    {
        $path = APPPATH . 'Controllers/Api/V1/FilmTextApiController.php';
        $this->assertFileExists($path);
        $php = (string) file_get_contents($path);

        $this->assertStringContainsString('private readonly FilmTextModel $model', $php);
        $this->assertStringContainsString('private readonly FilmTextService $service', $php);
    ...
    {
        $service = (string) file_get_contents(APPPATH . 'Services/FilmTextService.php');
    ...
    {
        $resource = (string) file_get_contents(APPPATH . 'API/Resources/FilmTextResource.php');
