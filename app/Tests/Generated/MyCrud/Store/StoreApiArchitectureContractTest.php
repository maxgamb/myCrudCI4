...
final class StoreApiArchitectureContractTest extends CIUnitTestCase
{
    public function testApiControllerUsesModelForReadsAndServiceForWritesWithoutSql(): void
    {
        $path = APPPATH . 'Controllers/Api/V1/StoreApiController.php';
        $this->assertFileExists($path);
        $php = (string) file_get_contents($path);

        $this->assertStringContainsString('private readonly StoreModel $model', $php);
        $this->assertStringContainsString('private readonly StoreService $service', $php);
    ...
    {
        $service = (string) file_get_contents(APPPATH . 'Services/StoreService.php');
    ...
    {
        $resource = (string) file_get_contents(APPPATH . 'API/Resources/StoreResource.php');
