...
final class ActorApiArchitectureContractTest extends CIUnitTestCase
{
    public function testApiControllerUsesModelForReadsAndServiceForWritesWithoutSql(): void
    {
        $path = APPPATH . 'Controllers/Api/V1/ActorApiController.php';
        $this->assertFileExists($path);
        $php = (string) file_get_contents($path);

        $this->assertStringContainsString('private readonly ActorModel $model', $php);
        $this->assertStringContainsString('private readonly ActorService $service', $php);
    ...
    {
        $service = (string) file_get_contents(APPPATH . 'Services/ActorService.php');
    ...
    {
        $resource = (string) file_get_contents(APPPATH . 'API/Resources/ActorResource.php');
