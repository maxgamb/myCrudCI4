...
final class PaymentApiArchitectureContractTest extends CIUnitTestCase
{
    public function testApiControllerUsesModelForReadsAndServiceForWritesWithoutSql(): void
    {
        $path = APPPATH . 'Controllers/Api/V1/PaymentApiController.php';
        $this->assertFileExists($path);
        $php = (string) file_get_contents($path);

        $this->assertStringContainsString('private readonly PaymentModel $model', $php);
        $this->assertStringContainsString('private readonly PaymentService $service', $php);
    ...
    {
        $service = (string) file_get_contents(APPPATH . 'Services/PaymentService.php');
    ...
    {
        $resource = (string) file_get_contents(APPPATH . 'API/Resources/PaymentResource.php');
