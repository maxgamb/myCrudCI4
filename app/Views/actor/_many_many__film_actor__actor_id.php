<?php
$manyCollapseId = 'many_view_many__film_actor__actor_id';
...
<section class="card shadow-sm mb-4">
    <div class="card-header d-flex justify-content-between align-items-center gap-2">
        <div class="d-flex align-items-center gap-2">
            ...
            <?php endif; ?>
            <span><i class="bi bi-diagram-2 me-1" aria-hidden="true"></i><strong>Film</strong> <small class="text-muted ms-2">pivot film_actor</small></span>
        </div>
        <span class="badge bg-secondary"><?= (int) ($children['many__film_actor__actor_id']['count'] ?? 0) ?><?= !empty($children['many__film_actor__actor_id']['hasMore']) ? '+' : '' ?></span>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-sm align-middle mb-0">
            ...
            <tbody>
            <?php foreach ((array) ($children['many__film_actor__actor_id']['rows'] ?? []) as $child): ?>
                <tr>
                    <td><?= esc($child->{'title'} ?? $child->{'film_id'} ?? '') ?></td>
                    <td class="d-print-none text-end"><?php $manyTrail = \App\Libraries\Crud\CrudNavigationTrail::encode(\App\Libraries\Crud\CrudNavigationTrail::append((array) ($cascadeTrail ?? []), 'actor', (string) ($row->{'actor_id'} ?? ''), trim((string) ($row->{'first_name'} ?? '') . ' ' . (string) ($row->{'last_name'} ?? '')) ?: 'Actor' . ' #' . (string) ($row->{'actor_id'} ?? ''))); ?>
<a class="btn btn-sm btn-outline-info" href="<?= site_url('film/view/' . rawurlencode((string) ($child->{'film_id'} ?? ''))) . ($manyTrail !== '' ? '?_trail=' . rawurlencode($manyTrail) : '') ?>" title="Open related record"><i class="bi bi-eye"></i></a></td>
            ...
            <?php endforeach; ?>
            <?php if (empty($children['many__film_actor__actor_id']['rows'])): ?>
