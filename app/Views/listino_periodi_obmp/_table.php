...
<div class="card shadow-sm">
    <div class="card-body">
        <?= view('listino_periodi_obmp/_pager', [
        ...
        <div class="table-responsive">
            <table class="table table-sm table-striped table-hover align-middle text-nowrap mb-0 crud-table">
                <thead class="table-light">
                    <tr>
                        <?php
                        $nextDirection = ($sort ?? '') === 'listino_periodi_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'listino_periodi_id',
                        ...
                        <th>
                            <a
                                ...
                                data-sort="listino_periodi_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('ListinoPeriodiObmp.listino_periodi_id')) ?>
                                <?php if (($sort ?? '') === 'listino_periodi_id'): ?>
                                    <i class="bi bi-sort-<?= ($direction ?? 'desc') === 'asc' ? 'up' : 'down' ?>"></i>
                                <?php endif; ?>
                            </a>
                        </th>                        <?php
                        $nextDirection = ($sort ?? '') === 'listino_nome_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'listino_nome_id',
                            'direction' => $nextDirection,
                            'page' => 1,
                        ]);
                        ?>
                        <th>
                            <a
                                href="<?= current_url() . '?' . http_build_query($sortQuery) ?>"
                                class="js-list-link text-decoration-none"
                                data-sort="listino_nome_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('ListinoPeriodiObmp.listino_nome_id')) ?>
                                <?php if (($sort ?? '') === 'listino_nome_id'): ?>
                                    <i class="bi bi-sort-<?= ($direction ?? 'desc') === 'asc' ? 'up' : 'down' ?>"></i>
                                <?php endif; ?>
                            </a>
                        </th>                        <th><?= esc(lang('ListinoPeriodiObmp.listino_periodi_flex')) ?></th>
                        <?php
                        $nextDirection = ($sort ?? '') === 'listino_dal' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'listino_dal',
                            'direction' => $nextDirection,
                            'page' => 1,
                        ...
                        <th>
                            <a
                                href="<?= current_url() . '?' . http_build_query($sortQuery) ?>"
                                class="js-list-link text-decoration-none"
                                data-sort="listino_dal"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('ListinoPeriodiObmp.listino_dal')) ?>
                                <?php if (($sort ?? '') === 'listino_dal'): ?>
                                    <i class="bi bi-sort-<?= ($direction ?? 'desc') === 'asc' ? 'up' : 'down' ?>"></i>
                                <?php endif; ?>
                            </a>
                        </th>                        <th><?= esc(lang('ListinoPeriodiObmp.listino_al')) ?></th>
                        <th><?= esc(lang('ListinoPeriodiObmp.hotel_id')) ?></th>
                        <th><?= esc(lang('ListinoPeriodiObmp.listino_periodi')) ?></th>
                        ...
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                ...
                                <td class="text-end text-nowrap">
                                    <?php $id = $row->listino_periodi_id ?? ''; ?>
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Azioni record">
                                        <a href="<?= site_url('listino_periodi_obmp/view/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-info" title="Visualizza">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="<?= site_url('listino_periodi_obmp/edit/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-warning" title="Modifica">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('listino_periodi_obmp/delete/' . rawurlencode((string) $id)) ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
                                            <?= csrf_field() ?>
