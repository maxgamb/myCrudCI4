        ...
        <div class="table-responsive">
            <table class="table table-sm table-striped table-hover align-middle text-nowrap mb-0 crud-table">
                <thead class="table-light">
                    <tr>
                        <?php
                        $nextDirection = ($sort ?? '') === 'payment_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        ...
                        <th>
                            <a
                                ...
                                data-sort="payment_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('Payment.payment_id')) ?>
                                <?php if (($sort ?? '') === 'payment_id'): ?>
                        ...
                        </th>                        <th><?= esc(lang('Payment.amount')) ?></th>
                        <th><?= esc(lang('Payment.payment_date')) ?></th>
                        <th><?= esc(lang('Payment.last_update')) ?></th>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                <td>
                                    <?php if ((string) ($row->{'payment_id'} ?? '') !== ''): ?>
                                        <?php
                                        $quickQuery = (array) ($query ?? []);
                                        $quickQuery['payment_id'] = (string) $row->{'payment_id'};
