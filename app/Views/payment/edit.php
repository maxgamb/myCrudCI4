...
$formAction = site_url('payment/update/' . rawurlencode((string) $rowId)) . $navigationQuery;
?>
...
<div class="container py-4 pb-0">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        ...
        <a href="<?= site_url('payment') . $navigationQuery ?>" class="btn btn-outline-secondary" title="Back to list">
            <i class="bi bi-list-ul me-1" aria-hidden="true"></i> List
        </a>
        <a href="<?= site_url('payment/view/' . rawurlencode((string) $rowId)) . $navigationQuery ?>" class="btn btn-outline-info" title="Visualizza record">
            <i class="bi bi-eye me-1" aria-hidden="true"></i> Visualizza
        </a>
        <form method="post" action="<?= site_url('payment/delete/' . rawurlencode((string) $rowId)) . $navigationQuery ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
            <?= csrf_field() ?>
