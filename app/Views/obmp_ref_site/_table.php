        ...
        <div class="table-responsive">
            <table class="table table-sm table-striped table-hover align-middle text-nowrap mb-0 crud-table">
                <thead class="table-light">
                    <tr>
                        <?php
                        $nextDirection = ($sort ?? '') === 'ref_site_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        ...
                        <th>
                            <a
                                ...
                                data-sort="ref_site_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('ObmpRefSite.ref_site_id')) ?>
                                <?php if (($sort ?? '') === 'ref_site_id'): ?>
                        ...
                        </th>                        <th><?= esc(lang('ObmpRefSite.ref_site_nome')) ?></th>
                        <th><?= esc(lang('ObmpRefSite.ref_site_date_record')) ?></th>
                        ...
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                ...
                                <td class="text-end text-nowrap">
                                    <?php $id = $row->ref_site_id ?? ''; ?>
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Azioni record">
                                        <a href="<?= site_url('obmp_ref_site/view/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-info" title="Visualizza">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="<?= site_url('obmp_ref_site/edit/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-warning" title="Modifica">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('obmp_ref_site/delete/' . rawurlencode((string) $id)) ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
                                            <?= csrf_field() ?>
