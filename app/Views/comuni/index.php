...
<div class="container-fluid px-0">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            ...
            <a id="exportCsvButton" href="<?= site_url('comuni/export-csv') ?>" class="btn btn-success">
                <i class="bi bi-filetype-csv"></i> Esporta CSV
            </a>
            <a id="exportWordButton" href="<?= site_url('comuni/export-word') ?>" class="btn btn-outline-primary">
    ...
    <details class="mb-3" <?= $hasActiveFilters ? 'open' : '' ?>>
        <summary class="fw-semibold"><?= esc(lang('Comuni.filtersSummary')) ?></summary>
        <div class="card card-body mt-2">
            <?= view('comuni/_filters', [
                'filters' => $filters ?? [],
                'options' => $options ?? [],
                'perPage' => $perPage ?? 25,
                'sort' => $sort ?? 'Comuni_Codice',
    ...
    <div id="crudTableContainer" aria-live="polite" aria-busy="false">
        <?= view('comuni/_table', [
            ...
            'primaryKey' => $primaryKey ?? 'Comuni_Codice',
            'sort' => $sort ?? 'Comuni_Codice',
            'direction' => $direction ?? 'desc',
            'query' => $query ?? [],
        ]) ?>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateExportUrls = () => {
        ...
        const csvUrl = new URL("<?= site_url('comuni/export-csv') ?>", window.location.origin);
        const wordUrl = new URL("<?= site_url('comuni/export-word') ?>", window.location.origin);
