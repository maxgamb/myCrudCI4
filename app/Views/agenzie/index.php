...
<div class="container-fluid py-4">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        <div>
            <h1 class="h3 mb-0">agenzie</h1>
            <small class="text-muted">Tabella Bootstrap, Pager CI4 e caricamento AJAX</small>
        </div>
        <div class="d-flex flex-wrap gap-2">
            <a href="<?= site_url('agenzie/create') ?>" class="btn btn-primary">
                <i class="bi bi-plus-circle"></i> Nuovo
            </a>
            <a id="exportCsvButton" href="<?= site_url('agenzie/export-csv') ?>" class="btn btn-success">
                <i class="bi bi-filetype-csv"></i> Esporta CSV
            </a>
            <a id="exportWordButton" href="<?= site_url('agenzie/export-word') ?>" class="btn btn-outline-primary">
    ...
    <?= view('agenzie/_filters', [
        'filters' => $filters ?? [],
        'options' => $options ?? [],
        'perPage' => $perPage ?? 25,
        'sort' => $sort ?? 'agenzia_id',
        'direction' => $direction ?? 'desc',
    ]) ?>

    <div id="crudTableContainer" aria-live="polite">
        <?= view('agenzie/_table', [
            'rows' => $rows ?? [],
            'total' => $total ?? 0,
            'page' => $page ?? 1,
            ...
            'primaryKey' => $primaryKey ?? 'agenzia_id',
            'sort' => $sort ?? 'agenzia_id',
            'direction' => $direction ?? 'desc',
            'query' => $query ?? [],
        ]) ?>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateExportUrls = () => {
        const params = formParameters().toString();
        const csvUrl = new URL("<?= site_url('agenzie/export-csv') ?>", window.location.origin);
        const wordUrl = new URL("<?= site_url('agenzie/export-word') ?>", window.location.origin);
