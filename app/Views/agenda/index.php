...
<div class="container-fluid px-0">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            ...
            <a id="exportCsvButton" href="<?= site_url('agenda/export-csv') ?>" class="btn btn-success">
                <i class="bi bi-filetype-csv"></i> Esporta CSV
            </a>
            <a id="exportWordButton" href="<?= site_url('agenda/export-word') ?>" class="btn btn-outline-primary">
    ...
    <details class="mb-3" <?= $hasActiveFilters ? 'open' : '' ?>>
        <summary class="fw-semibold"><?= esc(lang('Agenda.filtersSummary')) ?></summary>
        <div class="card card-body mt-2">
            <?= view('agenda/_filters', [
                'filters' => $filters ?? [],
                'options' => $options ?? [],
                'perPage' => $perPage ?? 25,
                'sort' => $sort ?? 'preno_id',
    ...
    <div id="crudTableContainer" aria-live="polite" aria-busy="false">
        <?= view('agenda/_table', [
            ...
            'primaryKey' => $primaryKey ?? 'preno_id',
            'sort' => $sort ?? 'preno_id',
            'direction' => $direction ?? 'desc',
            'query' => $query ?? [],
        ]) ?>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateExportUrls = () => {
        ...
        const csvUrl = new URL("<?= site_url('agenda/export-csv') ?>", window.location.origin);
        const wordUrl = new URL("<?= site_url('agenda/export-word') ?>", window.location.origin);
