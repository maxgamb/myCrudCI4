...
<div class="card shadow-sm">
    <div class="card-body">
        <?= view('wreh_order_details/_pager', [
        ...
        <div class="table-responsive">
            <table class="table table-sm table-striped table-hover align-middle text-nowrap mb-0 crud-table">
                <thead class="table-light">
                    <tr>
                        <?php
                        $nextDirection = ($sort ?? '') === 'order_detail_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'order_detail_id',
                        ...
                        <th>
                            <a
                                ...
                                data-sort="order_detail_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('WrehOrderDetails.order_detail_id')) ?>
                                <?php if (($sort ?? '') === 'order_detail_id'): ?>
                        ...
                        </th>                        <th><?= esc(lang('WrehOrderDetails.quantity')) ?></th>
                        <th><?= esc(lang('WrehOrderDetails.price')) ?></th>
                        <th><?= esc(lang('WrehOrderDetails.utente_id')) ?></th>
                        ...
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                ...
                                <td class="text-end text-nowrap">
                                    <?php $id = $row->order_detail_id ?? ''; ?>
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Azioni record">
                                        <a href="<?= site_url('wreh_order_details/view/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-info" title="Visualizza">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="<?= site_url('wreh_order_details/edit/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-warning" title="Modifica">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('wreh_order_details/delete/' . rawurlencode((string) $id)) ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
                                            <?= csrf_field() ?>
