...
<div class="container-fluid px-0">
    <nav aria-label="breadcrumb" class="mb-2">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">actor_info</li>
        </ol>
    </nav>

    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        <div>
            <h1 class="h3 mb-0">actor_info</h1>
            <small class="text-muted">Elenco</small>
        </div>
        <div class="d-flex flex-wrap gap-2">
            <a id="exportCsvButton" href="<?= site_url('actor_info/export-csv') . $navigationQuery ?>" class="btn btn-outline-success" title="Esporta i risultati filtrati in CSV">
                <i class="bi bi-filetype-csv me-1" aria-hidden="true"></i> CSV
            </a>
            <a id="exportWordButton" href="<?= site_url('actor_info/export-word') . $navigationQuery ?>" class="btn btn-outline-primary" title="Esporta i risultati filtrati in Word">
    ...
    <details class="mb-3" <?= $hasActiveFilters ? 'open' : '' ?>>
        <summary class="fw-semibold"><?= esc(lang('ActorInfo.filtersSummary')) ?></summary>
        <div class="card card-body mt-2">
            <?= view('actor_info/_filters', [
                'filters' => $filters ?? [],
                'options' => $options ?? [],
                'perPage' => $perPage ?? 25,
                'sort' => $sort ?? 'actor_id',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateActionUrls = (source = window.location.href) => {
        ...
        const csvUrl = new URL("<?= site_url('actor_info/export-csv') ?>", window.location.origin);
        const wordUrl = new URL("<?= site_url('actor_info/export-word') ?>", window.location.origin);
