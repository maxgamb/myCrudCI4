            ...
            <table class="table table-sm table-striped table-hover align-middle text-nowrap mb-0 crud-table">
                <thead class="table-light">
                    <tr>
                        ...
                        </th>                        <th><?= esc(lang('Utenti.Nome_Utente')) ?></th>
                        <th><?= esc(lang('Utenti.Pass_Utente')) ?></th>
                        <th><?= esc(lang('Utenti.Email_Utente')) ?></th>
                        <?php
                        ...
                        </th>                        <th><?= esc(lang('Utenti.utenti_livello')) ?></th>
                        <th><?= esc(lang('Utenti.utenti_Utente_id')) ?></th>
                        ...
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                <td>
                                    ...
                                    <?php if ((string) ($row->staff_id ?? '') !== ''): ?><a href="<?= site_url('staff/view/' . rawurlencode((string) $row->staff_id)) ?>" class="text-decoration-none"><?= esc($row->staff__staff_id__label ?? $row->staff_id ?? '') ?></a><?php else: ?><?= esc($row->staff__staff_id__label ?? '') ?><?php endif; ?>
                                    <?php
                                    $quickFilters = array_values((array) ($filters ?? []));
                                    $quickFilters[] = [
                                        'field' => 'staff_id',
                                    ...
                                    <?php endif; ?>                                </td>                                <td><?= esc($row->Nome_Utente ?? '') ?></td>
                                ...
                                <td>
                                    <?php if ((string) ($row->hotel_id ?? '') !== ''): ?>
                                        ...
                                        ><?= esc($row->hotel_id) ?></a>
                                    <?php endif; ?>
                                </td>                                <td><?= esc($row->utenti_livello ?? '') ?></td>
                                <td><?= esc($row->utenti_Utente_id ?? '') ?></td>
                                <td class="text-end text-nowrap">
                                    <?php $id = $row->Utente_id ?? ''; ?>
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Azioni record">
                                        <a href="<?= site_url('utenti/view/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-info" title="Visualizza">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="<?= site_url('utenti/edit/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-warning" title="Modifica">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('utenti/delete/' . rawurlencode((string) $id)) ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
                                            <?= csrf_field() ?>
