...
$formAction = site_url('country/update/' . rawurlencode((string) $rowId)) . $navigationQuery;
?>
...
<div class="container py-4 pb-0">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        ...
        <a href="<?= site_url('country') . $navigationQuery ?>" class="btn btn-outline-secondary" title="Torna alla lista">
            <i class="bi bi-list-ul me-1" aria-hidden="true"></i> Lista
        </a>
        <a href="<?= site_url('country/view/' . rawurlencode((string) $rowId)) . $navigationQuery ?>" class="btn btn-outline-info" title="Visualizza record">
            <i class="bi bi-eye me-1" aria-hidden="true"></i> Visualizza
        </a>
        <form method="post" action="<?= site_url('country/delete/' . rawurlencode((string) $rowId)) . $navigationQuery ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
            <?= csrf_field() ?>
