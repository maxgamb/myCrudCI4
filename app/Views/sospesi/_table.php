        ...
        <div class="table-responsive">
            <table class="table table-sm table-striped table-hover align-middle text-nowrap mb-0 crud-table">
                <thead class="table-light">
                    <tr>
                        <?php
                        $nextDirection = ($sort ?? '') === 'sospeso_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        ...
                        <th>
                            <a
                                ...
                                data-sort="sospeso_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('Sospesi.sospeso_id')) ?>
                                <?php if (($sort ?? '') === 'sospeso_id'): ?>
                        ...
                        $nextDirection = ($sort ?? '') === 'pagamento_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'pagamento_id',
                            'direction' => $nextDirection,
                            'page' => 1,
                        ...
                        <th>
                            <a
                                href="<?= current_url() . '?' . http_build_query($sortQuery) ?>"
                                class="js-list-link text-decoration-none"
                                data-sort="pagamento_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('Sospesi.pagamento_id')) ?>
                                <?php if (($sort ?? '') === 'pagamento_id'): ?>
                                    <i class="bi bi-sort-<?= ($direction ?? 'desc') === 'asc' ? 'up' : 'down' ?>"></i>
                                <?php endif; ?>
                            </a>
                        </th>                        <th><?= esc(lang('Sospesi.cassa_id')) ?></th>
                        <?php
                        $nextDirection = ($sort ?? '') === 'sospeso_data' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'sospeso_data',
                            'direction' => $nextDirection,
                            'page' => 1,
                        ...
                        <th>
                            <a
                                href="<?= current_url() . '?' . http_build_query($sortQuery) ?>"
                                class="js-list-link text-decoration-none"
                                data-sort="sospeso_data"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('Sospesi.sospeso_data')) ?>
                                <?php if (($sort ?? '') === 'sospeso_data'): ?>
                                    <i class="bi bi-sort-<?= ($direction ?? 'desc') === 'asc' ? 'up' : 'down' ?>"></i>
                                <?php endif; ?>
                            </a>
                        </th>                        <?php
                        $nextDirection = ($sort ?? '') === 'sospeso_conto_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'sospeso_conto_id',
                            'direction' => $nextDirection,
                            'page' => 1,
                        ]);
                        ?>
                        <th>
                            <a
                                href="<?= current_url() . '?' . http_build_query($sortQuery) ?>"
                                class="js-list-link text-decoration-none"
                                data-sort="sospeso_conto_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('Sospesi.sospeso_conto_id')) ?>
                                <?php if (($sort ?? '') === 'sospeso_conto_id'): ?>
                                    <i class="bi bi-sort-<?= ($direction ?? 'desc') === 'asc' ? 'up' : 'down' ?>"></i>
                                <?php endif; ?>
                            </a>
                        </th>                        <?php
                        $nextDirection = ($sort ?? '') === 'sospeso_pratica_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'sospeso_pratica_id',
                            'direction' => $nextDirection,
                            'page' => 1,
                        ]);
                        ?>
                        <th>
                            <a
                                href="<?= current_url() . '?' . http_build_query($sortQuery) ?>"
                                class="js-list-link text-decoration-none"
                                data-sort="sospeso_pratica_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('Sospesi.sospeso_pratica_id')) ?>
                                <?php if (($sort ?? '') === 'sospeso_pratica_id'): ?>
                                    <i class="bi bi-sort-<?= ($direction ?? 'desc') === 'asc' ? 'up' : 'down' ?>"></i>
                                <?php endif; ?>
                            </a>
                        </th>                        <th><?= esc(lang('Sospesi.sopeso_importo')) ?></th>
                        <?php
                        $nextDirection = ($sort ?? '') === 'sopeso_societa' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'sopeso_societa',
                            'direction' => $nextDirection,
                            'page' => 1,
                        ...
                        <th>
                            <a
                                href="<?= current_url() . '?' . http_build_query($sortQuery) ?>"
                                class="js-list-link text-decoration-none"
                                data-sort="sopeso_societa"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('Sospesi.sopeso_societa')) ?>
                                <?php if (($sort ?? '') === 'sopeso_societa'): ?>
                                    <i class="bi bi-sort-<?= ($direction ?? 'desc') === 'asc' ? 'up' : 'down' ?>"></i>
                                <?php endif; ?>
                            </a>
                        </th>                        <th><?= esc(lang('Sospesi.sospeso_stato')) ?></th>
                            ...
                            <tr>
                                ...
                                <td class="text-end text-nowrap">
                                    ...
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Azioni record">
                                        <a href="<?= site_url('sospesi/view/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-info" title="Visualizza">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="<?= site_url('sospesi/edit/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-warning" title="Modifica">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('sospesi/delete/' . rawurlencode((string) $id)) ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
                                            <?= csrf_field() ?>
