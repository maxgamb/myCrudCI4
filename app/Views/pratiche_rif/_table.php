        ...
        <div class="table-responsive">
            <table class="table table-sm table-striped table-hover align-middle text-nowrap mb-0 crud-table">
                <thead class="table-light">
                    <tr>
                        <?php
                        $nextDirection = ($sort ?? '') === 'pratica_rif_pratica_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'pratica_rif_pratica_id',
                        ...
                        $nextDirection = ($sort ?? '') === 'pratica_rif_conto_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'pratica_rif_conto_id',
                            'direction' => $nextDirection,
                            'page' => 1,
                        ]);
                        ?>
                        <th>
                            <a
                                href="<?= current_url() . '?' . http_build_query($sortQuery) ?>"
                                class="js-list-link text-decoration-none"
                                data-sort="pratica_rif_conto_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('PraticheRif.pratica_rif_conto_id')) ?>
                                <?php if (($sort ?? '') === 'pratica_rif_conto_id'): ?>
                                    <i class="bi bi-sort-<?= ($direction ?? 'desc') === 'asc' ? 'up' : 'down' ?>"></i>
                                <?php endif; ?>
                            </a>
                        </th>                        <th><?= esc(lang('PraticheRif.pratica_rif_totale_modificato')) ?></th>
                        <th><?= esc(lang('PraticheRif.pratica_rif_totale_importo')) ?></th>
                        <th><?= esc(lang('PraticheRif.pratica_rif_pagamento_importo_pag')) ?></th>
                        <th><?= esc(lang('PraticheRif.pratica_rif_out_conto')) ?></th>
                        <?php
                        $nextDirection = ($sort ?? '') === 'pratiche_rif_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'pratiche_rif_id',
                            'direction' => $nextDirection,
                            'page' => 1,
                        ]);
                        ?>
                        <th>
                            <a
                                href="<?= current_url() . '?' . http_build_query($sortQuery) ?>"
                                class="js-list-link text-decoration-none"
                                data-sort="pratiche_rif_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('PraticheRif.pratiche_rif_id')) ?>
                                <?php if (($sort ?? '') === 'pratiche_rif_id'): ?>
                                    <i class="bi bi-sort-<?= ($direction ?? 'desc') === 'asc' ? 'up' : 'down' ?>"></i>
                                <?php endif; ?>
                            </a>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Azioni record">
                                        ...
                                        <a href="<?= site_url('pratiche_rif/edit/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-warning" title="Modifica">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('pratiche_rif/delete/' . rawurlencode((string) $id)) ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
                                            <?= csrf_field() ?>
