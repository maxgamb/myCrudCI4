...
<div class="container-fluid px-0">
    <nav aria-label="breadcrumb" class="mb-2">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">film_list</li>
        </ol>
    </nav>

    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        <div>
            <h1 class="h3 mb-0">film_list</h1>
            <small class="text-muted">Elenco</small>
        </div>
        <div class="d-flex flex-wrap gap-2">
            <a id="exportCsvButton" href="<?= site_url('film_list/export-csv') . $navigationQuery ?>" class="btn btn-outline-success" title="Esporta i risultati filtrati in CSV">
                <i class="bi bi-filetype-csv me-1" aria-hidden="true"></i> CSV
            </a>
            <a id="exportWordButton" href="<?= site_url('film_list/export-word') . $navigationQuery ?>" class="btn btn-outline-primary" title="Esporta i risultati filtrati in Word">
    ...
    <details class="mb-3" <?= $hasActiveFilters ? 'open' : '' ?>>
        <summary class="fw-semibold"><?= esc(lang('FilmList.filtersSummary')) ?></summary>
        <div class="card card-body mt-2">
            <?= view('film_list/_filters', [
                'filters' => $filters ?? [],
                'options' => $options ?? [],
                'perPage' => $perPage ?? 25,
                'sort' => $sort ?? 'FID',
    ...
    <div id="crudTableContainer" aria-live="polite" aria-busy="false">
        <?= view('film_list/_table', [
            'rows' => $rows ?? [],
            'total' => $total ?? 0,
            'page' => $page ?? 1,
            'perPage' => $perPage ?? 25,
            'pagerLinks' => $pagerLinks ?? '',
            'primaryKey' => $primaryKey ?? 'FID',
            'sort' => $sort ?? 'FID',
            'direction' => $direction ?? 'desc',
            'query' => $query ?? [],
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateActionUrls = (source = window.location.href) => {
        ...
        const csvUrl = new URL("<?= site_url('film_list/export-csv') ?>", window.location.origin);
        const wordUrl = new URL("<?= site_url('film_list/export-word') ?>", window.location.origin);
