            ...
            <table class="table table-sm table-striped table-hover align-middle text-nowrap mb-0 crud-table">
                <thead class="table-light">
                    <tr>
                        ...
                        $nextDirection = ($sort ?? '') === 'shift_date' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        $sortQuery = array_replace((array) ($query ?? []), [
                            'sort' => 'shift_date',
                            'direction' => $nextDirection,
                            'page' => 1,
                        ...
                        <th>
                            <a
                                href="<?= current_url() . '?' . http_build_query($sortQuery) ?>"
                                class="js-list-link text-decoration-none"
                                data-sort="shift_date"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('Shifts.shift_date')) ?>
                                <?php if (($sort ?? '') === 'shift_date'): ?>
                                    <i class="bi bi-sort-<?= ($direction ?? 'desc') === 'asc' ? 'up' : 'down' ?>"></i>
                                <?php endif; ?>
                            </a>
                        </th>                        <th><?= esc(lang('Shifts.position')) ?></th>
                        <th><?= esc(lang('Shifts.shift_time')) ?></th>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                <td><?= esc($row->id ?? '') ?></td>
                                <td><?= esc($row->staff_nome ?? $row->staff_id ?? '') ?></td>
                                ...
                                <td class="text-end text-nowrap">
                                    ...
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Azioni record">
                                        <a href="<?= site_url('shifts/view/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-info" title="Visualizza">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="<?= site_url('shifts/edit/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-warning" title="Modifica">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('shifts/delete/' . rawurlencode((string) $id)) ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
                                            <?= csrf_field() ?>
