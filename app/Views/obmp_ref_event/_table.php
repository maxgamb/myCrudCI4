        ...
        <div class="table-responsive">
            <table class="table table-sm table-striped table-hover align-middle text-nowrap mb-0 crud-table">
                <thead class="table-light">
                    <tr>
                        <?php
                        $nextDirection = ($sort ?? '') === 'ref_event_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        ...
                        <th>
                            <a
                                ...
                                data-sort="ref_event_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('ObmpRefEvent.ref_event_id')) ?>
                                <?php if (($sort ?? '') === 'ref_event_id'): ?>
                        ...
                        </th>                        <th><?= esc(lang('ObmpRefEvent.ref_event_nome')) ?></th>
                        <th><?= esc(lang('ObmpRefEvent.event_dal')) ?></th>
                        <th><?= esc(lang('ObmpRefEvent.event_al')) ?></th>
                        <th><?= esc(lang('ObmpRefEvent.ref_event_note')) ?></th>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                <td>
                                    <?php if ((string) ($row->ref_event_id ?? '') !== ''): ?>
                                        <?php
                                        $quickFilters = array_values((array) ($filters ?? []));
                                        $quickFilters[] = [
                                            'field' => 'ref_event_id',
                                            'operator' => 'eq',
                                            'value' => (string) $row->ref_event_id,
                                            'logic' => 'and',
                                        ];
                                        ...
                                        ><?= esc($row->ref_event_id) ?></a>
                                    <?php endif; ?>
                                </td>                                <td>
                                    <?php if ((string) ($row->ref_site_id ?? '') !== ''): ?><a href="<?= site_url('obmp_ref_site/view/' . rawurlencode((string) $row->ref_site_id)) ?>" class="text-decoration-none"><?= esc($row->obmp_ref_site__ref_site_id__label ?? $row->ref_site_id ?? '') ?></a><?php else: ?><?= esc($row->obmp_ref_site__ref_site_id__label ?? '') ?><?php endif; ?>
                                    <?php
                                    $quickFilters = array_values((array) ($filters ?? []));
                                    $quickFilters[] = [
                                        'field' => 'ref_site_id',
                                        'operator' => 'eq',
                                        'value' => (string) ($row->ref_site_id ?? ''),
                                        'logic' => 'and',
                                    ];
                                    $quickQuery = array_replace((array) ($query ?? []), [
                                        'filters' => $quickFilters,
                                    ...
                                    <?php if ((string) ($row->agenzia_id ?? '') !== ''): ?>
                                        <a
                                            ...
                                            aria-label="Filtra per questo valore"
                                        ><i class="bi bi-funnel"></i></a>
                                    <?php endif; ?>                                </td>                                <td><?= esc($row->ref_event_nome ?? '') ?></td>
                                <td><?= esc($row->event_dal ?? '') ?></td>
                                <td><?= esc($row->event_al ?? '') ?></td>
                                <td><?= esc($row->ref_event_note ?? '') ?></td>
                                <td class="text-end text-nowrap">
                                    <?php $id = $row->ref_event_id ?? ''; ?>
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Azioni record">
                                        <a href="<?= site_url('obmp_ref_event/view/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-info" title="Visualizza">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="<?= site_url('obmp_ref_event/edit/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-warning" title="Modifica">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('obmp_ref_event/delete/' . rawurlencode((string) $id)) ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
                                            <?= csrf_field() ?>
