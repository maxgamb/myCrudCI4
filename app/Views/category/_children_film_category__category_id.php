<?php
    $hasManyCollapseId = 'hasmany_' . preg_replace('/[^A-Za-z0-9_]/', '_', 'film_category__category_id');
    ...
    <div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between align-items-center gap-2">
            ...
            <div class="d-flex align-items-center gap-2 d-print-none">
                <a href="<?= site_url('film_category/create') . '?' . http_build_query(['category_id' => $row->{'category_id'} ?? '', '_parent_field' => 'category_id', '_trail' => \App\Libraries\Crud\CrudNavigationTrail::encode(\App\Libraries\Crud\CrudNavigationTrail::append((array) ($cascadeTrail ?? []), 'category', (string) ($row->{'category_id'} ?? ''), trim((string) ($row->{'name'} ?? '')) ?: 'Category' . ' #' . (string) ($row->{'category_id'} ?? '')))]) ?>" class="btn btn-sm btn-primary" title="New related record"><i class="bi bi-plus-circle me-1" aria-hidden="true"></i> New</a>
                <a href="<?= site_url('film_category') . '?' . http_build_query(['category_id' => $row->{'category_id'} ?? '', '_trail' => \App\Libraries\Crud\CrudNavigationTrail::encode(\App\Libraries\Crud\CrudNavigationTrail::append((array) ($cascadeTrail ?? []), 'category', (string) ($row->{'category_id'} ?? ''), trim((string) ($row->{'name'} ?? '')) ?: 'Category' . ' #' . (string) ($row->{'category_id'} ?? '')))]) ?>" class="btn btn-sm btn-outline-primary" title="View all related records"><i class="bi bi-list-ul me-1" aria-hidden="true"></i> View all</a>
