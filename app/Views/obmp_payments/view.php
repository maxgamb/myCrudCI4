        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-sm table-bordered table-striped align-middle">
                    <tbody>
                        <tr>
                            <th class="w-25"><?= esc(lang('ObmpPayments.obmp_payment_id')) ?></th>
                            <td><?= esc($row->obmp_payment_id ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('ObmpPayments.obmp_payment_cod')) ?></th>
                            <td><?= esc($row->obmp_payment_cod ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('ObmpPayments.obmp_payment_title')) ?></th>
                            <td><?= esc($row->obmp_payment_title ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('ObmpPayments.obmp_payment')) ?></th>
                            <td><?= esc($row->obmp_payment ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('ObmpPayments.obmp_payment_value')) ?></th>
                            <td><?= esc($row->obmp_payment_value ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('ObmpPayments.payment_lg')) ?></th>
...
<div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <span class="badge bg-secondary"><?= (int) ($children['obmp_rates__obmp_payment_cod']['count'] ?? 0) ?><?= !empty($children['obmp_rates__obmp_payment_cod']['hasMore']) ? '+' : '' ?></span>
        </div>
        <div class="card-body">
            <?php $relatedRows = $children['obmp_rates__obmp_payment_cod']['rows'] ?? []; ?>
            ...
            <?php else: ?>
                ...
                <?php if (!empty($children['obmp_rates__obmp_payment_cod']['hasMore'])): ?>
