...
<div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between align-items-center gap-2">
            ...
            <div class="d-flex align-items-center gap-2 d-print-none">
                <a href="<?= site_url('customer/create') . '?' . http_build_query(['store_id' => $row->{'store_id'} ?? '']) ?>" class="btn btn-sm btn-primary" title="Nuovo record collegato"><i class="bi bi-plus-circle me-1" aria-hidden="true"></i> Nuovo</a>
                <span class="badge bg-secondary"><?= (int) ($children['customer__store_id']['count'] ?? 0) ?><?= !empty($children['customer__store_id']['hasMore']) ? '+' : '' ?></span>
            </div>
        </div>
        <div class="card-body">
            <?php $relatedRows = $children['customer__store_id']['rows'] ?? []; ?>
                        ...
                        <tbody>
                            <?php foreach ($relatedRows as $child): ?>
                                ...
                                <td><?= esc($child->{'last_update'} ?? '') ?></td>
                                    <td class="d-print-none"><a href="<?= site_url('customer/view/' . rawurlencode((string) ($child->{'customer_id'} ?? ''))) . '?' . http_build_query(['store_id' => $row->{'store_id'} ?? '']) ?>" class="btn btn-sm btn-outline-info" title="Visualizza record"><i class="bi bi-eye" aria-hidden="true"></i></a></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>
                <?php if (!empty($children['customer__store_id']['hasMore'])): ?>
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 d-print-none">
                        ...
                        <a href="<?= site_url('customer') . '?' . http_build_query(['store_id' => $row->{'store_id'} ?? '']) ?>" class="btn btn-sm btn-outline-primary">
...
<div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between align-items-center gap-2">
            ...
            <div class="d-flex align-items-center gap-2 d-print-none">
                <a href="<?= site_url('inventory/create') . '?' . http_build_query(['store_id' => $row->{'store_id'} ?? '']) ?>" class="btn btn-sm btn-primary" title="Nuovo record collegato"><i class="bi bi-plus-circle me-1" aria-hidden="true"></i> Nuovo</a>
        ...
        <div class="card-body">
            ...
            <?php else: ?>
                    ...
                    <table class="table table-sm table-striped table-hover align-middle">
                        ...
                        <tbody>
                            <?php foreach ($relatedRows as $child): ?>
                                ...
                                <td><?= esc($child->{'store_id'} ?? '') ?></td>
                                <td><?= esc($child->{'last_update'} ?? '') ?></td>
                                    <td class="d-print-none"><a href="<?= site_url('inventory/view/' . rawurlencode((string) ($child->{'inventory_id'} ?? ''))) . '?' . http_build_query(['store_id' => $row->{'store_id'} ?? '']) ?>" class="btn btn-sm btn-outline-info" title="Visualizza record"><i class="bi bi-eye" aria-hidden="true"></i></a></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                ...
                <?php if (!empty($children['inventory__store_id']['hasMore'])): ?>
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 d-print-none">
                        <div class="small text-muted">Visualizzati i primi 20 record.</div>
                        <a href="<?= site_url('inventory') . '?' . http_build_query(['store_id' => $row->{'store_id'} ?? '']) ?>" class="btn btn-sm btn-outline-primary">
                            Vedi tutti <i class="bi bi-arrow-right"></i>
                        </a>
                    </div>
                <?php endif; ?>
...
<div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between align-items-center gap-2">
            ...
            <div class="d-flex align-items-center gap-2 d-print-none">
                <a href="<?= site_url('staff/create') . '?' . http_build_query(['store_id' => $row->{'store_id'} ?? '']) ?>" class="btn btn-sm btn-primary" title="Nuovo record collegato"><i class="bi bi-plus-circle me-1" aria-hidden="true"></i> Nuovo</a>
                <span class="badge bg-secondary"><?= (int) ($children['staff__store_id']['count'] ?? 0) ?><?= !empty($children['staff__store_id']['hasMore']) ? '+' : '' ?></span>
            </div>
        </div>
        <div class="card-body">
            <?php $relatedRows = $children['staff__store_id']['rows'] ?? []; ?>
            <?php if (empty($relatedRows)): ?>
                <div class="alert alert-light border mb-0">Nessun record collegato.</div>
            <?php else: ?>
                <div class="table-responsive">
                    <table class="table table-sm table-striped table-hover align-middle">
                        ...
                        <tbody>
                            <?php foreach ($relatedRows as $child): ?>
                                ...
                                <td><?= esc($child->{'last_update'} ?? '') ?></td>
                                    <td class="d-print-none"><a href="<?= site_url('staff/view/' . rawurlencode((string) ($child->{'staff_id'} ?? ''))) . '?' . http_build_query(['store_id' => $row->{'store_id'} ?? '']) ?>" class="btn btn-sm btn-outline-info" title="Visualizza record"><i class="bi bi-eye" aria-hidden="true"></i></a></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>
                <?php if (!empty($children['staff__store_id']['hasMore'])): ?>
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 d-print-none">
                        <div class="small text-muted">Visualizzati i primi 20 record.</div>
                        <a href="<?= site_url('staff') . '?' . http_build_query(['store_id' => $row->{'store_id'} ?? '']) ?>" class="btn btn-sm btn-outline-primary">
                            Vedi tutti <i class="bi bi-arrow-right"></i>
                        </a>
                    </div>
                <?php endif; ?>
