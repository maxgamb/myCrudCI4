        ...
        <div class="table-responsive">
            <table class="table table-sm table-striped table-hover align-middle text-nowrap mb-0 crud-table">
                <thead class="table-light">
                    <tr>
                        <?php
                        $nextDirection = ($sort ?? '') === 'checklist_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        ...
                        <th>
                            <a
                                ...
                                data-sort="checklist_id"
                                data-direction="<?= esc($nextDirection) ?>"
                            >
                                <?= esc(lang('ChecklistPreno.checklist_id')) ?>
                                <?php if (($sort ?? '') === 'checklist_id'): ?>
                        ...
                        </th>                        <th><?= esc(lang('ChecklistPreno.preno_dal')) ?></th>
                        <th><?= esc(lang('ChecklistPreno.email')) ?></th>
                        <th><?= esc(lang('ChecklistPreno.email_pms')) ?></th>
                        <th><?= esc(lang('ChecklistPreno.lista')) ?></th>
                        <th><?= esc(lang('ChecklistPreno.lista_pms')) ?></th>
                        <th><?= esc(lang('ChecklistPreno.importo')) ?></th>
                        <th><?= esc(lang('ChecklistPreno.data_check')) ?></th>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                <td>
                                    <?php if ((string) ($row->checklist_id ?? '') !== ''): ?>
                                        <?php
                                        $quickFilters = array_values((array) ($filters ?? []));
                                        $quickFilters[] = [
                                            'field' => 'checklist_id',
                                            'operator' => 'eq',
                                            'value' => (string) $row->checklist_id,
                                            'logic' => 'and',
                                        ];
                                ...
                                <td class="text-end text-nowrap">
                                    <?php $id = $row->checklist_id ?? ''; ?>
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Azioni record">
                                        <a href="<?= site_url('checklist_preno/view/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-info" title="Visualizza">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="<?= site_url('checklist_preno/edit/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-warning" title="Modifica">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('checklist_preno/delete/' . rawurlencode((string) $id)) ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
                                            <?= csrf_field() ?>
