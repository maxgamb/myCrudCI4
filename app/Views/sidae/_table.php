        ...
        <div class="table-responsive">
            <table class="table table-sm table-striped table-hover align-middle text-nowrap mb-0 crud-table">
                <thead class="table-light">
                    <tr>
                        <?php
                        $nextDirection = ($sort ?? '') === 'sidae_id' && ($direction ?? 'desc') === 'asc' ? 'desc' : 'asc';
                        ...
                        <th>
                            ...
                            >
                                <?= esc(lang('Sidae.sidae_id')) ?>
                                <?php if (($sort ?? '') === 'sidae_id'): ?>
                        ...
                        </th>                        <th><?= esc(lang('Sidae.foglio_id')) ?></th>
                        <th><?= esc(lang('Sidae.nome_cliente')) ?></th>
                        <th><?= esc(lang('Sidae.pag_room')) ?></th>
                        <th><?= esc(lang('Sidae.aliquota')) ?></th>
                        <th><?= esc(lang('Sidae.quan_room')) ?></th>
                        <th><?= esc(lang('Sidae.pag_extra')) ?></th>
                        <th><?= esc(lang('Sidae.data')) ?></th>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                <td>
                                    <?php if ((string) ($row->sidae_id ?? '') !== ''): ?>
                                        <?php
                                        $quickFilters = array_values((array) ($filters ?? []));
                                        $quickFilters[] = [
                                            'field' => 'sidae_id',
                                            'operator' => 'eq',
                                            'value' => (string) $row->sidae_id,
                                            'logic' => 'and',
                                        ];
                                    ...
                                    <?php endif; ?>                                </td>                                <td><?= esc($row->foglio_id ?? '') ?></td>
                                ...
                                <td class="text-end text-nowrap">
                                    ...
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Azioni record">
                                        <a href="<?= site_url('sidae/view/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-info" title="Visualizza">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="<?= site_url('sidae/edit/' . rawurlencode((string) $id)) ?>" class="btn btn-outline-warning" title="Modifica">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('sidae/delete/' . rawurlencode((string) $id)) ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
                                            <?= csrf_field() ?>
