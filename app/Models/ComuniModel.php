...
/** Model per comuni; tutte le query del CRUD sono centralizzate qui. */
final class ComuniModel extends Model
{
  ...
  9 => 'nazione_iso3',
    ...
    {
        $builder = $this->db->table('comuni');
        $builder->select([
            'comuni.Comuni_Codice AS Comuni_Codice',
            'comuni.Comuni_Nome AS Comuni_Nome',
            'comuni.Comuni_Prov AS Comuni_Prov',
            'comuni.Comuni_CAP AS Comuni_CAP',
            'comuni.Comuni_Prefisso AS Comuni_Prefisso',
            'comuni.Comuni_ColExcel AS Comuni_ColExcel',
            'comuni.Comuni_Nazione AS Comuni_Nazione',
            'comuni.Comuni_Lingua AS Comuni_Lingua',
            'comuni.nazione_iso2 AS nazione_iso2',
            'comuni.nazione_iso3 AS nazione_iso3'
        ]);
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'Comuni_Codice',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'Comuni_Codice';
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('comuni.Comuni_Codice >', $after);
        }

        return $builder
            ->orderBy('comuni.Comuni_Codice', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'comuni.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        $sort = (string) ($query['sort'] ?? 'Comuni_Codice');
        $sort = in_array($sort, $sortable, true) ? $sort : 'Comuni_Codice';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('comuni.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
