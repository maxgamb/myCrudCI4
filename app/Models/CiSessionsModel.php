...
use App\Entities\CiSessionsEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per ci_sessions; tutte le query del CRUD sono centralizzate qui. */
final class CiSessionsModel extends Model
{
    protected $table = 'ci_sessions';
    protected $primaryKey = 'id';
    protected $returnType = CiSessionsEntity::class;
  ...
  2 => 'timestamp',
    ...
    public function baseBuilder(): BaseBuilder
    {
        ...
        $builder->select([
            'ci_sessions.id AS id',
            'ci_sessions.ip_address AS ip_address',
            'ci_sessions.timestamp AS timestamp'
        ]);
    ...
    public function getListPage(
        ...
        $rows = $dataBuilder
            ->orderBy('ci_sessions.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('ci_sessions.id >', $after);
        }

        return $builder
            ->orderBy('ci_sessions.id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'ci_sessions.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('ci_sessions.' . $field, $value);
            }
        }
        ...
        $rows = $builder->orderBy('ci_sessions.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
