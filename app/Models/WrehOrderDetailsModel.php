...
use App\Entities\WrehOrderDetailsEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per wreh_order_details; tutte le query del CRUD sono centralizzate qui. */
final class WrehOrderDetailsModel extends Model
{
    protected $table = 'wreh_order_details';
    protected $primaryKey = 'order_detail_id';
    protected $returnType = WrehOrderDetailsEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('wreh_order_details');
        $builder->select([
            'wreh_order_details.order_detail_id AS order_detail_id',
            'wreh_order_details.order_id AS order_id',
            'wreh_order_details.product_id AS product_id',
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('wreh_order_details.order_detail_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'order_detail_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'order_detail_id';
        ...
        $rows = $dataBuilder
            ->orderBy('wreh_order_details.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('wreh_order_details.order_detail_id >', $after);
        }

        return $builder
            ->orderBy('wreh_order_details.order_detail_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'wreh_order_details.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('wreh_order_details.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'order_detail_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'order_detail_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('wreh_order_details.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    /** Restituisce le opzioni della relazione order_id. */
