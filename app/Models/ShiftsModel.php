...
use App\Entities\ShiftsEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per shifts; tutte le query del CRUD sono centralizzate qui. */
final class ShiftsModel extends Model
{
    protected $table = 'shifts';
    protected $primaryKey = 'id';
    protected $returnType = ShiftsEntity::class;
  ...
  array (
    ...
    public function baseBuilder(): BaseBuilder
    {
        ...
        $builder->select([
            'shifts.id AS id',
            'shifts.staff_id AS staff_id',
            'shifts.hotel_id AS hotel_id',
            'shifts.shift_date AS shift_date',
            'shifts.position AS position',
            'shifts.shift_time AS shift_time',
            'shifts.data_record AS data_record',
            'staff__staff_id.nome AS staff__staff_id__label'
        ]);
        $builder->join('staff AS staff__staff_id', 'staff__staff_id.staff_id = shifts.staff_id', 'left');
        return $builder;
    }
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'shifts.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('shifts.' . $field, $value);
            }
        }
        ...
        $rows = $builder->orderBy('shifts.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
