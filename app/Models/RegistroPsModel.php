...
use App\Entities\RegistroPsEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per registro_ps; tutte le query del CRUD sono centralizzate qui. */
final class RegistroPsModel extends Model
{
    protected $table = 'registro_ps';
    protected $primaryKey = 'registro_ps_id';
    protected $returnType = RegistroPsEntity::class;
    protected $useSoftDeletes = false;
    protected $protectFields = true;
    protected $allowedFields = array (
  0 => 'registro_ps_hotel_id',
  1 => 'registro_ps_valore',
  2 => 'registro_ps_utente_id',
);
  ...
  3 => 'registro_ps_utente_id',
    ...
    {
        $builder = $this->db->table('registro_ps');
        $builder->select([
            'registro_ps.registro_ps_id AS registro_ps_id',
            'registro_ps.registro_ps_hotel_id AS registro_ps_hotel_id',
            'registro_ps.registro_ps_valore AS registro_ps_valore',
            'registro_ps.registro_ps_utente_id AS registro_ps_utente_id'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('registro_ps.registro_ps_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'registro_ps_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'registro_ps_id';
        ...
        $rows = $dataBuilder
            ->orderBy('registro_ps.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('registro_ps.registro_ps_id >', $after);
        }

        return $builder
            ->orderBy('registro_ps.registro_ps_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'registro_ps.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('registro_ps.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'registro_ps_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'registro_ps_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('registro_ps.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
