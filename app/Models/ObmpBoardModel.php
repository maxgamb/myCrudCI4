...
use App\Entities\ObmpBoardEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per obmp_board; tutte le query del CRUD sono centralizzate qui. */
final class ObmpBoardModel extends Model
{
    protected $table = 'obmp_board';
    protected $primaryKey = 'obmp_board_id';
    protected $returnType = ObmpBoardEntity::class;
  ...
  4 => 'board_lg',
    ...
    {
        $builder = $this->db->table('obmp_board');
        $builder->select([
            'obmp_board.obmp_board_id AS obmp_board_id',
            'obmp_board.obmp_board_title AS obmp_board_title',
            'obmp_board.obmp_board AS obmp_board',
            'obmp_board.obmp_board_cod AS obmp_board_cod',
            'obmp_board.board_lg AS board_lg'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('obmp_board.obmp_board_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'obmp_board_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'obmp_board_id';
        ...
        $rows = $dataBuilder
            ->orderBy('obmp_board.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('obmp_board.obmp_board_id >', $after);
        }

        return $builder
            ->orderBy('obmp_board.obmp_board_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'obmp_board.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('obmp_board.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'obmp_board_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'obmp_board_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('obmp_board.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getObmpRatesByObmpBoardCod(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('obmp_rates')
            ->where('obmp_board_cod', $parentId)
    ...
    {
        $result = [];
        $result['obmp_rates__obmp_board_cod'] = $this->getObmpRatesByObmpBoardCod($parentId, 20);
        return $result;
    }
