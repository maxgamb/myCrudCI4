...
use App\Entities\CaxMotivoEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per cax_motivo; tutte le query del CRUD sono centralizzate qui. */
final class CaxMotivoModel extends Model
{
    protected $table = 'cax_motivo';
    protected $primaryKey = 'cax_motivo_id';
    protected $returnType = CaxMotivoEntity::class;
  ...
  2 => 'data',
    ...
    {
        $builder = $this->db->table('cax_motivo');
        $builder->select([
            'cax_motivo.cax_motivo_id AS cax_motivo_id',
            'cax_motivo.etichetta AS etichetta',
            'cax_motivo.data AS data'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('cax_motivo.cax_motivo_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'cax_motivo_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'cax_motivo_id';
        ...
        $rows = $dataBuilder
            ->orderBy('cax_motivo.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('cax_motivo.cax_motivo_id >', $after);
        }

        return $builder
            ->orderBy('cax_motivo.cax_motivo_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'cax_motivo.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('cax_motivo.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'cax_motivo_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'cax_motivo_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('cax_motivo.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
