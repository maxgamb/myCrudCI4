...
use App\Entities\LogObmpFullEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per log_obmp_full; tutte le query del CRUD sono centralizzate qui. */
final class LogObmpFullModel extends Model
{
    protected $table = 'log_obmp_full';
    protected $primaryKey = 'log_obmp_id_full';
    protected $returnType = LogObmpFullEntity::class;
  ...
  13 => 'log_obmp_daterecord',
    ...
    {
        $builder = $this->db->table('log_obmp_full');
        $builder->select([
            'log_obmp_full.log_obmp_id_full AS log_obmp_id_full',
            'log_obmp_full.preno_dal AS preno_dal',
            'log_obmp_full.preno_al AS preno_al',
            'log_obmp_full.Q1 AS Q1',
            'log_obmp_full.T1 AS T1',
            'log_obmp_full.hotel_id AS hotel_id',
            'log_obmp_full.ref_site AS ref_site',
            'log_obmp_full.ref_agency AS ref_agency',
            'log_obmp_full.ref_event AS ref_event',
            'log_obmp_full.ref_session AS ref_session',
            'log_obmp_full.ref_cookie AS ref_cookie',
            'log_obmp_full.mygooglekeyword AS mygooglekeyword',
            'log_obmp_full.today AS today',
            'log_obmp_full.log_obmp_daterecord AS log_obmp_daterecord'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('log_obmp_full.log_obmp_id_full', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'log_obmp_id_full',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'log_obmp_id_full';
        ...
        $rows = $dataBuilder
            ->orderBy('log_obmp_full.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('log_obmp_full.log_obmp_id_full >', $after);
        }

        return $builder
            ->orderBy('log_obmp_full.log_obmp_id_full', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'log_obmp_full.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('log_obmp_full.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'log_obmp_id_full');
        $sort = in_array($sort, $sortable, true) ? $sort : 'log_obmp_id_full';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('log_obmp_full.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
