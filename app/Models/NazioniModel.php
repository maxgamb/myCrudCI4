...
use App\Entities\NazioniEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per nazioni; tutte le query del CRUD sono centralizzate qui. */
final class NazioniModel extends Model
{
    protected $table = 'nazioni';
    protected $primaryKey = 'Nazioni_Id_Codice';
    protected $returnType = NazioniEntity::class;
  ...
  5 => 'EN_Country',
    ...
    {
        $builder = $this->db->table('nazioni');
        $builder->select([
            'nazioni.Nazioni_Id_Codice AS Nazioni_Id_Codice',
            'nazioni.Nazioni_Codice AS Nazioni_Codice',
            'nazioni.Nazioni_Descrizione AS Nazioni_Descrizione',
            'nazioni.Nazioni_Targa AS Nazioni_Targa',
            'nazioni.Nazioni_ColExcel AS Nazioni_ColExcel',
            'nazioni.EN_Country AS EN_Country'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('nazioni.Nazioni_Id_Codice', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'Nazioni_Id_Codice',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'Nazioni_Id_Codice';
        ...
        $rows = $dataBuilder
            ->orderBy('nazioni.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('nazioni.Nazioni_Id_Codice >', $after);
        }

        return $builder
            ->orderBy('nazioni.Nazioni_Id_Codice', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'nazioni.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('nazioni.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'Nazioni_Id_Codice');
        $sort = in_array($sort, $sortable, true) ? $sort : 'Nazioni_Id_Codice';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('nazioni.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
