...
use App\Entities\NazioniBandieraEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per nazioni_bandiera; tutte le query del CRUD sono centralizzate qui. */
final class NazioniBandieraModel extends Model
{
    protected $table = 'nazioni_bandiera';
    protected $primaryKey = 'nazione_iso2';
    protected $returnType = NazioniBandieraEntity::class;
  ...
  3 => 'cod_emoji',
    ...
    {
        $builder = $this->db->table('nazioni_bandiera');
        $builder->select([
            'nazioni_bandiera.nazione_iso2 AS nazione_iso2',
            'nazioni_bandiera.Nazioni_Codice AS Nazioni_Codice',
            'nazioni_bandiera.emoji AS emoji',
            'nazioni_bandiera.cod_emoji AS cod_emoji'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('nazioni_bandiera.nazione_iso2', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'nazione_iso2',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'nazione_iso2';
        ...
        $rows = $dataBuilder
            ->orderBy('nazioni_bandiera.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('nazioni_bandiera.nazione_iso2 >', $after);
        }

        return $builder
            ->orderBy('nazioni_bandiera.nazione_iso2', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'nazioni_bandiera.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('nazioni_bandiera.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'nazione_iso2');
        $sort = in_array($sort, $sortable, true) ? $sort : 'nazione_iso2';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('nazioni_bandiera.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
