...
/** Model per language; tutte le query del CRUD sono centralizzate qui. */
final class LanguageModel extends Model
{
  ...
  0 => 'language_id',
    ...
    public function baseBuilder(): BaseBuilder
    {
        ...
        $builder->select([
            ...
            'language.last_update AS last_update'
        ]);
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'language_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'language_id';
        ...
        $rows = $dataBuilder
            ->orderBy('language.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('language.language_id >', $after);
        }
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'language.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('language.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'language_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'language_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('language.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getFilmByLanguageId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('film')
            ->select(array (
  0 => 'film.film_id AS film_id',
  1 => 'film.title AS title',
  2 => 'film.description AS description',
  3 => 'film.release_year AS release_year',
  4 => 'film.language_id AS language_id',
  5 => 'film.original_language_id AS original_language_id',
  6 => 'film.rental_duration AS rental_duration',
  7 => 'film.rental_rate AS rental_rate',
  8 => 'film.length AS length',
  9 => 'film.replacement_cost AS replacement_cost',
  10 => 'film.rating AS rating',
  11 => 'film.special_features AS special_features',
  12 => 'film.last_update AS last_update',
))
            ->where('language_id', $parentId)
    ...
    /** Carica al massimo una riga in più per determinare se esistono altri risultati. */
    public function getFilmByOriginalLanguageId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('film')
            ->select(array (
  ...
  12 => 'film.last_update AS last_update',
            ...
            ->where('original_language_id', $parentId)
            ->orderBy('film_id', 'DESC')
            ->limit($limit + 1)
            ->get()
            ->getResult();
    ...
    {
        $result = [];
        $result['film__language_id'] = $this->getFilmByLanguageId($parentId, 20);

        $result['film__original_language_id'] = $this->getFilmByOriginalLanguageId($parentId, 20);
        return $result;
    }
