...
use App\Entities\PraticheEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per pratiche; tutte le query del CRUD sono centralizzate qui. */
final class PraticheModel extends Model
{
    protected $table = 'pratiche';
    protected $primaryKey = 'pratica_id';
    protected $returnType = PraticheEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('pratiche');
        $builder->select([
            'pratiche.pratica_id AS pratica_id',
            'pratiche.hotel_id AS hotel_id',
            'pratiche.pratica_nome AS pratica_nome',
            'pratiche.pratica_agenzia_id AS pratica_agenzia_id',
            'pratiche.pratica_1 AS pratica_1',
            'pratiche.pratica_2 AS pratica_2',
            'pratiche.pratica_note AS pratica_note',
            'pratiche.pratica_stato AS pratica_stato',
            'pratiche.pratica_data_record AS pratica_data_record',
            'pratiche.pratiche_utente_id AS pratiche_utente_id',
            'agenzie__pratica_agenzia_id.agenzia_tipologia AS agenzie__pratica_agenzia_id__label'
        ]);
        $builder->join('agenzie AS agenzie__pratica_agenzia_id', 'agenzie__pratica_agenzia_id.agenzia_id = pratiche.pratica_agenzia_id', 'left');
        return $builder;
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'pratica_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'pratica_id';
        ...
        $rows = $dataBuilder
            ->orderBy('pratiche.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('pratiche.pratica_id >', $after);
        }

        return $builder
            ->orderBy('pratiche.pratica_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'pratiche.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('pratiche.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'pratica_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'pratica_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('pratiche.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    /** Restituisce le opzioni della relazione pratica_agenzia_id. */
    public function getAgenziePraticaAgenziaIdOptions(): array
    {
  ...
  1 => 'agenzia_tipologia',
    ...
    {
        return [
            'pratica_agenzia_id' => $this->toRelationOptions($this->getAgenziePraticaAgenziaIdOptions(), 'pratica_agenzia_id'),
        ];
    }
    ...
    public function getPraticheRifByPraticheRifId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('pratiche_rif')
            ->where('pratiche_rif_id', $parentId)
            ->orderBy('pratica_rif_pratica_id', 'DESC')
            ->limit($limit + 1)
            ->get()
            ->getResult();
    ...
    /** Carica al massimo una riga in più per determinare se esistono altri risultati. */
    public function getSospesiBySospesoPraticaId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('sospesi')
            ->where('sospeso_pratica_id', $parentId)
    ...
    {
        $result = [];
        $result['pratiche_rif__pratiche_rif_id'] = $this->getPraticheRifByPraticheRifId($parentId, 20);

        $result['sospesi__sospeso_pratica_id'] = $this->getSospesiBySospesoPraticaId($parentId, 20);
        return $result;
    }
