...
use App\Entities\ObmpClientiEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per obmp_clienti; tutte le query del CRUD sono centralizzate qui. */
final class ObmpClientiModel extends Model
{
    protected $table = 'obmp_clienti';
    protected $primaryKey = 'obm_cliente_id';
    protected $returnType = ObmpClientiEntity::class;
  ...
  15 => 'obm_cliente_cc_security',
    ...
    {
        $builder = $this->db->table('obmp_clienti');
        $builder->select([
            'obmp_clienti.obm_cliente_id AS obm_cliente_id',
            'obmp_clienti.obm_cliente_first_name AS obm_cliente_first_name',
            'obmp_clienti.obm_cliente_last_name AS obm_cliente_last_name',
            'obmp_clienti.obm_cliente_email AS obm_cliente_email',
            'obmp_clienti.obm_cliente_city AS obm_cliente_city',
            'obmp_clienti.obm_cliente_country AS obm_cliente_country',
            'obmp_clienti.lingua AS lingua',
            'obmp_clienti.obm_cliente_phone AS obm_cliente_phone',
            'obmp_clienti.obm_cliente_newsletter AS obm_cliente_newsletter',
            'obmp_clienti.obm_cliente_pass AS obm_cliente_pass',
            'obmp_clienti.obm_cliente_data_insert AS obm_cliente_data_insert',
            'obmp_clienti.obm_cliente_data_record AS obm_cliente_data_record',
            'obmp_clienti.obm_cliente_cc_type AS obm_cliente_cc_type',
            'obmp_clienti.obm_cliente_cc_number AS obm_cliente_cc_number',
            'obmp_clienti.obm_cliente_holder AS obm_cliente_holder',
            'obmp_clienti.obm_cliente_cc_expire AS obm_cliente_cc_expire',
            'obmp_clienti.obm_cliente_cc_security AS obm_cliente_cc_security'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('obmp_clienti.obm_cliente_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'obm_cliente_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'obm_cliente_id';
        ...
        $rows = $dataBuilder
            ->orderBy('obmp_clienti.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('obmp_clienti.obm_cliente_id >', $after);
        }

        return $builder
            ->orderBy('obmp_clienti.obm_cliente_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'obmp_clienti.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('obmp_clienti.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'obm_cliente_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'obm_cliente_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('obmp_clienti.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getRefObmpBookingByObmClienteId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('ref_obmp_booking')
            ->where('obm_cliente_id', $parentId)
    ...
    {
        $result = [];
        $result['ref_obmp_booking__obm_cliente_id'] = $this->getRefObmpBookingByObmClienteId($parentId, 20);
        return $result;
    }
