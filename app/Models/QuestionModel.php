...
/** Model per question; tutte le query del CRUD sono centralizzate qui. */
  ...
  5 => 'tex_no',
    ...
    public function baseBuilder(): BaseBuilder
    {
        ...
        $builder->select([
            ...
            'question.tex_lingue_id_pro AS tex_lingue_id_pro',
            'question.tex_lingue_id_con AS tex_lingue_id_con',
            'question.tex_pro AS tex_pro',
            'question.tex_no AS tex_no'
        ]);
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'question_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'question_id';
        ...
        $rows = $dataBuilder
            ->orderBy('question.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('question.question_id >', $after);
        }
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'question.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('question.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'question_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'question_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('question.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getQuestionRewByQuestionId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('question_rew')
            ->where('question_id', $parentId)
            ->orderBy('question_rew_id', 'DESC')
            ->limit($limit + 1)
            ->get()
            ->getResult();
    ...
    {
        $result = [];
        $result['question_rew__question_id'] = $this->getQuestionRewByQuestionId($parentId, 20);
        return $result;
    }
