...
/** Model per province; tutte le query del CRUD sono centralizzate qui. */
final class ProvinceModel extends Model
{
    protected $table = 'province';
    protected $primaryKey = 'Prov';
    protected $returnType = ProvinceEntity::class;
  ...
  1 => 'ColExcel',
    ...
    public function baseBuilder(): BaseBuilder
    {
        ...
        $builder->select([
            'province.Prov AS Prov',
            'province.ColExcel AS ColExcel'
        ]);
    ...
    public function getListPage(
        ...
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'Prov';
        ...
        $rows = $dataBuilder
            ->orderBy('province.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('province.Prov >', $after);
        }

        return $builder
            ->orderBy('province.Prov', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'province.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('province.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'Prov');
        $sort = in_array($sort, $sortable, true) ? $sort : 'Prov';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('province.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
