...
use App\Entities\SalesByFilmCategoryEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;
use RuntimeException;
use Throwable;

/** Model read-only per SQL VIEW sales_by_film_category; estendere manualmente solo se necessario. */
final class SalesByFilmCategoryModel extends Model
{
    protected $table = 'sales_by_film_category';
    protected $primaryKey = 'category';
    protected $returnType = SalesByFilmCategoryEntity::class;
  ...
  1 => 'total_sales',
    ...
    {
        $builder = $this->db->table('sales_by_film_category');
        $builder->select([
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('sales_by_film_category.category', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'category';
        ...
        $rows = $dataBuilder
            ->orderBy('sales_by_film_category.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('sales_by_film_category.category >', $after);
        }

        return $builder
            ->orderBy('sales_by_film_category.category', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'sales_by_film_category.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('sales_by_film_category.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'category');
        $sort = in_array($sort, $sortable, true) ? $sort : 'category';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('sales_by_film_category.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
