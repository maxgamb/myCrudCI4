...
/** Model per products; tutte le query del CRUD sono centralizzate qui. */
final class ProductsModel extends Model
{
    protected $table = 'products';
    protected $primaryKey = 'product_id';
    protected $returnType = ProductsEntity::class;
  ...
  5 => 'supplier_id',
    ...
    public function getListPage(
        ...
        $rows = $dataBuilder
            ->orderBy('products.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('products.product_id >', $after);
        }
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'products.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        $rows = $builder->orderBy('products.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
