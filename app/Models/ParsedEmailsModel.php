...
use App\Entities\ParsedEmailsEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per parsed_emails; tutte le query del CRUD sono centralizzate qui. */
final class ParsedEmailsModel extends Model
{
    protected $table = 'parsed_emails';
    protected $primaryKey = 'id';
    protected $returnType = ParsedEmailsEntity::class;
  ...
  8 => 'json_parsed',
    ...
    {
        $builder = $this->db->table('parsed_emails');
        $builder->select([
            'parsed_emails.id AS id',
            'parsed_emails.hotel_id AS hotel_id',
            'parsed_emails.category AS category',
            'parsed_emails.referente_tipo AS referente_tipo',
            'parsed_emails.prenotazione_tipo AS prenotazione_tipo',
            'parsed_emails.finalita AS finalita',
            'parsed_emails.segmento_commerciale AS segmento_commerciale',
            'parsed_emails.raw_email AS raw_email',
            'parsed_emails.json_parsed AS json_parsed',
            'parsed_emails.created_at AS created_at'
        ]);
    ...
    public function getListPage(
        ...
        $rows = $dataBuilder
            ->orderBy('parsed_emails.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('parsed_emails.id >', $after);
        }

        return $builder
            ->orderBy('parsed_emails.id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'parsed_emails.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('parsed_emails.' . $field, $value);
            }
        }
        ...
        $rows = $builder->orderBy('parsed_emails.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
