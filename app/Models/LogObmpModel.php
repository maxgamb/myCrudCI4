...
use App\Entities\LogObmpEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per log_obmp; tutte le query del CRUD sono centralizzate qui. */
final class LogObmpModel extends Model
{
    protected $table = 'log_obmp';
    protected $primaryKey = 'log_obmp_id';
    protected $returnType = LogObmpEntity::class;
  ...
  12 => 'log_obmp_daterecord',
    ...
    {
        $builder = $this->db->table('log_obmp');
        $builder->select([
            'log_obmp.log_obmp_id AS log_obmp_id',
            'log_obmp.preno_dal AS preno_dal',
            'log_obmp.preno_al AS preno_al',
            'log_obmp.Q1 AS Q1',
            'log_obmp.T1 AS T1',
            'log_obmp.hotel_id AS hotel_id',
            'log_obmp.ref_site AS ref_site',
            'log_obmp.ref_agency AS ref_agency',
            'log_obmp.ref_event AS ref_event',
            'log_obmp.ref_session AS ref_session',
            'log_obmp.ref_cookie AS ref_cookie',
            'log_obmp.mygooglekeyword AS mygooglekeyword',
            'log_obmp.log_obmp_daterecord AS log_obmp_daterecord'
        ]);
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'log_obmp_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'log_obmp_id';
        ...
        $rows = $dataBuilder
            ->orderBy('log_obmp.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('log_obmp.log_obmp_id >', $after);
        }

        return $builder
            ->orderBy('log_obmp.log_obmp_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'log_obmp.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('log_obmp.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'log_obmp_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'log_obmp_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('log_obmp.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
