...
use App\Entities\NazioniLinqueEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per nazioni_linque; tutte le query del CRUD sono centralizzate qui. */
final class NazioniLinqueModel extends Model
{
    protected $table = 'nazioni_linque';
    protected $primaryKey = 'isoKey';
    protected $returnType = NazioniLinqueEntity::class;
  ...
  7 => 'lg',
    ...
    {
        $builder = $this->db->table('nazioni_linque');
        $builder->select([
            'nazioni_linque.isoKey AS isoKey',
            'nazioni_linque.iso3 AS iso3',
            'nazioni_linque.nazioni_EN AS nazioni_EN',
            'nazioni_linque.nazioni_ES AS nazioni_ES',
            'nazioni_linque.nazioni_FR AS nazioni_FR',
            'nazioni_linque.nazioni_DE AS nazioni_DE',
            'nazioni_linque.nazioni_IT AS nazioni_IT',
            'nazioni_linque.lg AS lg'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('nazioni_linque.isoKey', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'isoKey';
        ...
        $rows = $dataBuilder
            ->orderBy('nazioni_linque.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('nazioni_linque.isoKey >', $after);
        }

        return $builder
            ->orderBy('nazioni_linque.isoKey', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'nazioni_linque.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('nazioni_linque.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'isoKey');
        $sort = in_array($sort, $sortable, true) ? $sort : 'isoKey';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('nazioni_linque.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
