...
/** Model per tipologia_camera; tutte le query del CRUD sono centralizzate qui. */
final class TipologiaCameraModel extends Model
{
    protected $table = 'tipologia_camera';
    protected $primaryKey = 'tipologia_id';
    protected $returnType = TipologiaCameraEntity::class;
  ...
  10 => 'perc_prezzo',
    ...
    {
        $builder = $this->db->table('tipologia_camera');
        $builder->select([
            'tipologia_camera.tipologia_id AS tipologia_id',
            'tipologia_camera.nome_tipologia AS nome_tipologia',
            'tipologia_camera.nome_tipologia_en AS nome_tipologia_en',
            'tipologia_camera.nome_tipologia_fr AS nome_tipologia_fr',
            'tipologia_camera.nome_tipologia_de AS nome_tipologia_de',
            'tipologia_camera.nome_tipologia_sp AS nome_tipologia_sp',
            'tipologia_camera.nome_tipologia_jp AS nome_tipologia_jp',
            'tipologia_camera.tipologia_sigla AS tipologia_sigla',
            'tipologia_camera.numero_pax AS numero_pax',
            'tipologia_camera.tipologia_camera_data_record AS tipologia_camera_data_record',
            'tipologia_camera.tipologia_camera_utente_id AS tipologia_camera_utente_id',
            'tipologia_camera.perc_prezzo AS perc_prezzo'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('tipologia_camera.tipologia_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'tipologia_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'tipologia_id';
        ...
        $rows = $dataBuilder
            ->orderBy('tipologia_camera.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('tipologia_camera.tipologia_id >', $after);
        }

        return $builder
            ->orderBy('tipologia_camera.tipologia_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'tipologia_camera.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        $sort = (string) ($query['sort'] ?? 'tipologia_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'tipologia_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('tipologia_camera.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getCamereByTipologiaId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('camere')
    ...
    /** Carica al massimo una riga in più per determinare se esistono altri risultati. */
    public function getFoglioGiornoByTipologiaId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
    ...
    /** Carica al massimo una riga in più per determinare se esistono altri risultati. */
    public function getObmpCmRoomsByObmpCmRoomsTipologiaId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('obmp_cm_rooms')
            ->where('obmp_cm_rooms_tipologia_id', $parentId)
    ...
    /** Carica al massimo una riga in più per determinare se esistono altri risultati. */
    public function getRefCostiTipologiaByTipologiaId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
    ...
    {
        $result = [];
        $result['camere__tipologia_id'] = $this->getCamereByTipologiaId($parentId, 20);

        $result['foglio_giorno__tipologia_id'] = $this->getFoglioGiornoByTipologiaId($parentId, 20);

        $result['obmp_cm_rooms__obmp_cm_rooms_tipologia_id'] = $this->getObmpCmRoomsByObmpCmRoomsTipologiaId($parentId, 20);

        $result['ref_costi_tipologia__tipologia_id'] = $this->getRefCostiTipologiaByTipologiaId($parentId, 20);
        return $result;
    }
