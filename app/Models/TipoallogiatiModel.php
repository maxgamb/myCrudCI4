...
use App\Entities\TipoallogiatiEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per tipoallogiati; tutte le query del CRUD sono centralizzate qui. */
final class TipoallogiatiModel extends Model
{
    protected $table = 'tipoallogiati';
    protected $primaryKey = 'Tip_all_Cod';
    protected $returnType = TipoallogiatiEntity::class;
    protected $useSoftDeletes = false;
    protected $protectFields = true;
    protected $allowedFields = array (
  0 => 'Tip_all_Cod',
  1 => 'Tip_all_Descrizione',
);
  ...
  1 => 'Tip_all_Descrizione',
    ...
    {
        $builder = $this->db->table('tipoallogiati');
        $builder->select([
            'tipoallogiati.Tip_all_Cod AS Tip_all_Cod',
            'tipoallogiati.Tip_all_Descrizione AS Tip_all_Descrizione'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('tipoallogiati.Tip_all_Cod', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'Tip_all_Cod',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'Tip_all_Cod';
        ...
        $rows = $dataBuilder
            ->orderBy('tipoallogiati.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('tipoallogiati.Tip_all_Cod >', $after);
        }

        return $builder
            ->orderBy('tipoallogiati.Tip_all_Cod', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'tipoallogiati.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('tipoallogiati.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'Tip_all_Cod');
        $sort = in_array($sort, $sortable, true) ? $sort : 'Tip_all_Cod';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('tipoallogiati.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
