...
use App\Entities\AgenziaListiniEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per agenzia_listini; tutte le query del CRUD sono centralizzate qui. */
final class AgenziaListiniModel extends Model
{
    protected $table = 'agenzia_listini';
    protected $primaryKey = 'agenzia_listini_id';
    protected $returnType = AgenziaListiniEntity::class;
    protected $useSoftDeletes = false;
    protected $protectFields = true;
    protected $allowedFields = array (
  0 => 'hotel_id',
  1 => 'agenzia_listini_nome',
  2 => 'agenzia_listini_note',
  3 => 'agenzia_listini_datarecord',
);
  ...
  4 => 'agenzia_listini_datarecord',
    ...
    {
        $builder = $this->db->table('agenzia_listini');
        $builder->select([
            'agenzia_listini.agenzia_listini_id AS agenzia_listini_id',
            'agenzia_listini.hotel_id AS hotel_id',
            'agenzia_listini.agenzia_listini_nome AS agenzia_listini_nome',
            'agenzia_listini.agenzia_listini_note AS agenzia_listini_note',
            'agenzia_listini.agenzia_listini_datarecord AS agenzia_listini_datarecord'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('agenzia_listini.agenzia_listini_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'agenzia_listini_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'agenzia_listini_id';
        ...
        $rows = $dataBuilder
            ->orderBy('agenzia_listini.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('agenzia_listini.agenzia_listini_id >', $after);
        }

        return $builder
            ->orderBy('agenzia_listini.agenzia_listini_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'agenzia_listini.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('agenzia_listini.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'agenzia_listini_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'agenzia_listini_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('agenzia_listini.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getAgenziaPrezziByAgenziaPrezziId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('agenzia_prezzi')
            ->where('agenzia_prezzi_id', $parentId)
            ->orderBy('agenzia_prezzi_id', 'DESC')
            ->limit($limit + 1)
            ->get()
            ->getResult();
    ...
    /** Carica al massimo una riga in più per determinare se esistono altri risultati. */
    public function getRefAgenziaListiniByAgenziaListiniId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('ref_agenzia_listini')
            ->where('agenzia_listini_id', $parentId)
    ...
    {
        $result = [];
        $result['agenzia_prezzi__agenzia_prezzi_id'] = $this->getAgenziaPrezziByAgenziaPrezziId($parentId, 20);

        $result['ref_agenzia_listini__agenzia_listini_id'] = $this->getRefAgenziaListiniByAgenziaListiniId($parentId, 20);
        return $result;
    }
