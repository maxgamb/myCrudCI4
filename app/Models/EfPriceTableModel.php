...
use App\Entities\EfPriceTableEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per ef_price_table; tutte le query del CRUD sono centralizzate qui. */
final class EfPriceTableModel extends Model
{
    protected $table = 'ef_price_table';
    protected $primaryKey = 'price_ef_is';
    protected $returnType = EfPriceTableEntity::class;
  ...
  8 => 'fam_tr',
    ...
    {
        $builder = $this->db->table('ef_price_table');
        $builder->select([
            'ef_price_table.price_ef_is AS price_ef_is',
            'ef_price_table.hotel_id AS hotel_id',
            'ef_price_table.from AS from',
            'ef_price_table.to AS to',
            'ef_price_table.single AS single',
            'ef_price_table.single_plus AS single_plus',
            'ef_price_table.tw_db AS tw_db',
            'ef_price_table.student AS student',
            'ef_price_table.fam_tr AS fam_tr'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('ef_price_table.price_ef_is', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'price_ef_is',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'price_ef_is';
        ...
        $rows = $dataBuilder
            ->orderBy('ef_price_table.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('ef_price_table.price_ef_is >', $after);
        }

        return $builder
            ->orderBy('ef_price_table.price_ef_is', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'ef_price_table.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('ef_price_table.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'price_ef_is');
        $sort = in_array($sort, $sortable, true) ? $sort : 'price_ef_is';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('ef_price_table.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
