...
/** Model per app_ip; tutte le query del CRUD sono centralizzate qui. */
final class AppIpModel extends Model
{
    protected $table = 'app_ip';
    protected $primaryKey = 'app_ip_id';
    protected $returnType = AppIpEntity::class;
  ...
  3 => 'data',
    ...
    {
        $builder = $this->db->table('app_ip');
        $builder->select([
            'app_ip.app_ip_id AS app_ip_id',
            'app_ip.ip_aderss AS ip_aderss',
            'app_ip.Livello AS Livello',
            'app_ip.data AS data'
        ]);
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'app_ip_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'app_ip_id';
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('app_ip.app_ip_id >', $after);
        }

        return $builder
            ->orderBy('app_ip.app_ip_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'app_ip.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        $sort = (string) ($query['sort'] ?? 'app_ip_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'app_ip_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('app_ip.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
