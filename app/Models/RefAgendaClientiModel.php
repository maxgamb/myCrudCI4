...
use App\Entities\RefAgendaClientiEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per ref_agenda_clienti; tutte le query del CRUD sono centralizzate qui. */
final class RefAgendaClientiModel extends Model
{
    protected $table = 'ref_agenda_clienti';
    protected $primaryKey = 'ref_agenda_cliente';
    protected $returnType = RefAgendaClientiEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('ref_agenda_clienti');
        $builder->select([
            'ref_agenda_clienti.ref_agenda_cliente AS ref_agenda_cliente',
            'ref_agenda_clienti.preno_id AS preno_id',
            'ref_agenda_clienti.clienti_id AS clienti_id',
            'ref_agenda_clienti.tipologia_id AS tipologia_id',
            'ref_agenda_clienti.ref_a_c_datarecord AS ref_a_c_datarecord',
            'agenda__preno_id.preno_arr_ore AS agenda__preno_id__label'
        ]);
        $builder->join('agenda AS agenda__preno_id', 'agenda__preno_id.preno_id = ref_agenda_clienti.preno_id', 'left');
        return $builder;
    }
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('ref_agenda_clienti.ref_agenda_cliente', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'ref_agenda_cliente',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'ref_agenda_cliente';
        ...
        $rows = $dataBuilder
            ->orderBy('ref_agenda_clienti.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('ref_agenda_clienti.ref_agenda_cliente >', $after);
        }

        return $builder
            ->orderBy('ref_agenda_clienti.ref_agenda_cliente', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'ref_agenda_clienti.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('ref_agenda_clienti.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'ref_agenda_cliente');
        $sort = in_array($sort, $sortable, true) ? $sort : 'ref_agenda_cliente';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('ref_agenda_clienti.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
