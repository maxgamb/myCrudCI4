...
use App\Entities\ObmpCancellationsEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per obmp_cancellations; tutte le query del CRUD sono centralizzate qui. */
final class ObmpCancellationsModel extends Model
{
    protected $table = 'obmp_cancellations';
    protected $primaryKey = 'obmp_cancellation_id';
    protected $returnType = ObmpCancellationsEntity::class;
  ...
  5 => 'cancellation_lg',
    ...
    {
        $builder = $this->db->table('obmp_cancellations');
        $builder->select([
            'obmp_cancellations.obmp_cancellation_id AS obmp_cancellation_id',
            'obmp_cancellations.obmp_cancellation_cod AS obmp_cancellation_cod',
            'obmp_cancellations.obmp_cancellation_title AS obmp_cancellation_title',
            'obmp_cancellations.obmp_cancellation AS obmp_cancellation',
            'obmp_cancellations.obmp_cancellation_day AS obmp_cancellation_day',
            'obmp_cancellations.cancellation_lg AS cancellation_lg'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('obmp_cancellations.obmp_cancellation_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'obmp_cancellation_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'obmp_cancellation_id';
        ...
        $rows = $dataBuilder
            ->orderBy('obmp_cancellations.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('obmp_cancellations.obmp_cancellation_id >', $after);
        }

        return $builder
            ->orderBy('obmp_cancellations.obmp_cancellation_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'obmp_cancellations.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('obmp_cancellations.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'obmp_cancellation_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'obmp_cancellation_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('obmp_cancellations.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getObmpRatesByObmpCancellationCod(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('obmp_rates')
            ->where('obmp_cancellation_cod', $parentId)
    ...
    {
        $result = [];
        $result['obmp_rates__obmp_cancellation_cod'] = $this->getObmpRatesByObmpCancellationCod($parentId, 20);
        return $result;
    }
