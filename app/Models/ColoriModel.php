...
use App\Entities\ColoriEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per colori; tutte le query del CRUD sono centralizzate qui. */
final class ColoriModel extends Model
{
    protected $table = 'colori';
    protected $primaryKey = 'colore_nome';
    protected $returnType = ColoriEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('colori');
        $builder->select([
            'colori.colore_nome AS colore_nome',
            'colori.colore_codice AS colore_codice',
            'colori.col_preno_id AS col_preno_id',
            'colori.colore_data_record AS colore_data_record',
            'agenda__col_preno_id.preno_arr_ore AS agenda__col_preno_id__label'
        ]);
        $builder->join('agenda AS agenda__col_preno_id', 'agenda__col_preno_id.preno_id = colori.col_preno_id', 'left');
        return $builder;
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'colore_nome',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'colore_nome';
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('colori.colore_nome >', $after);
        }

        return $builder
            ->orderBy('colori.colore_nome', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'colori.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('colori.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'colore_nome');
        $sort = in_array($sort, $sortable, true) ? $sort : 'colore_nome';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('colori.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    /** Restituisce le opzioni della relazione col_preno_id. */
    public function getAgendaColPrenoIdOptions(): array
    {
  ...
  1 => 'preno_arr_ore',
    ...
    {
        return [
            'col_preno_id' => $this->toRelationOptions($this->getAgendaColPrenoIdOptions(), 'col_preno_id'),
        ];
    }
