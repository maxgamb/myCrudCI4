...
use App\Entities\ObmpRefSiteEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per obmp_ref_site; tutte le query del CRUD sono centralizzate qui. */
final class ObmpRefSiteModel extends Model
{
    protected $table = 'obmp_ref_site';
    protected $primaryKey = 'ref_site_id';
    protected $returnType = ObmpRefSiteEntity::class;
  ...
  4 => 'ref_site_date_record',
    ...
    {
        $builder = $this->db->table('obmp_ref_site');
        $builder->select([
            'obmp_ref_site.ref_site_id AS ref_site_id',
            'obmp_ref_site.hotel_id AS hotel_id',
            'obmp_ref_site.obmp_affiliati_id AS obmp_affiliati_id',
            'obmp_ref_site.ref_site_nome AS ref_site_nome',
            'obmp_ref_site.ref_site_date_record AS ref_site_date_record'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('obmp_ref_site.ref_site_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'ref_site_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'ref_site_id';
        ...
        $rows = $dataBuilder
            ->orderBy('obmp_ref_site.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('obmp_ref_site.ref_site_id >', $after);
        }

        return $builder
            ->orderBy('obmp_ref_site.ref_site_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'obmp_ref_site.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('obmp_ref_site.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'ref_site_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'ref_site_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('obmp_ref_site.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getObmpRefEventByRefSiteId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('obmp_ref_event')
            ->where('ref_site_id', $parentId)
    ...
    /** Carica al massimo una riga in più per determinare se esistono altri risultati. */
    public function getRefObmpBookingByRefSite(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('ref_obmp_booking')
            ->where('ref_site', $parentId)
    ...
    {
        $result = [];
        $result['obmp_ref_event__ref_site_id'] = $this->getObmpRefEventByRefSiteId($parentId, 20);

        $result['ref_obmp_booking__ref_site'] = $this->getRefObmpBookingByRefSite($parentId, 20);
        return $result;
    }
