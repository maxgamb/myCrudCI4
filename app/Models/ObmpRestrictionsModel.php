...
use App\Entities\ObmpRestrictionsEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per obmp_restrictions; tutte le query del CRUD sono centralizzate qui. */
final class ObmpRestrictionsModel extends Model
{
    protected $table = 'obmp_restrictions';
    protected $primaryKey = 'obmp_restriction_id';
    protected $returnType = ObmpRestrictionsEntity::class;
  ...
  6 => 'restr_max_bw',
    ...
    {
        $builder = $this->db->table('obmp_restrictions');
        $builder->select([
            'obmp_restrictions.obmp_restriction_id AS obmp_restriction_id',
            'obmp_restrictions.hotel_id AS hotel_id',
            'obmp_restrictions.restr_nama AS restr_nama',
            'obmp_restrictions.restr_min_stay AS restr_min_stay',
            'obmp_restrictions.restr_max_stay AS restr_max_stay',
            'obmp_restrictions.restr_min_bw AS restr_min_bw',
            'obmp_restrictions.restr_max_bw AS restr_max_bw'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('obmp_restrictions.obmp_restriction_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'obmp_restriction_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'obmp_restriction_id';
        ...
        $rows = $dataBuilder
            ->orderBy('obmp_restrictions.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('obmp_restrictions.obmp_restriction_id >', $after);
        }

        return $builder
            ->orderBy('obmp_restrictions.obmp_restriction_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'obmp_restrictions.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('obmp_restrictions.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'obmp_restriction_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'obmp_restriction_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('obmp_restrictions.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getObmpRatesByObmpRestrictionId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('obmp_rates')
            ->where('obmp_restriction_id', $parentId)
    ...
    {
        $result = [];
        $result['obmp_rates__obmp_restriction_id'] = $this->getObmpRatesByObmpRestrictionId($parentId, 20);
        return $result;
    }
