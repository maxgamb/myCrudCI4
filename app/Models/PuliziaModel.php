...
use App\Entities\PuliziaEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per pulizia; tutte le query del CRUD sono centralizzate qui. */
final class PuliziaModel extends Model
{
    protected $table = 'pulizia';
    protected $primaryKey = 'pulizia_id';
    protected $returnType = PuliziaEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('pulizia');
        $builder->select([
            'pulizia.pulizia_id AS pulizia_id',
            'pulizia.hotel_id AS hotel_id',
            'pulizia.conto_id AS conto_id',
            'pulizia.camera_id AS camera_id',
            'pulizia.cambio_biancheria AS cambio_biancheria',
            'pulizia.pulizia_stato AS pulizia_stato',
            'pulizia.pulizia_data AS pulizia_data',
            'pulizia.pulizia_note AS pulizia_note',
            'pulizia.utente_id AS utente_id',
            'pulizia.pulizia_data_record AS pulizia_data_record',
            'conti__conto_id.trattamento_sog AS conti__conto_id__label'
        ]);
        $builder->join('conti AS conti__conto_id', 'conti__conto_id.conto_id = pulizia.conto_id', 'left');
        return $builder;
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'pulizia_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'pulizia_id';
        ...
        $rows = $dataBuilder
            ->orderBy('pulizia.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('pulizia.pulizia_id >', $after);
        }

        return $builder
            ->orderBy('pulizia.pulizia_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'pulizia.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('pulizia.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'pulizia_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'pulizia_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('pulizia.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
