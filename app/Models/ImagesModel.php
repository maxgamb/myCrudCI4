...
use App\Entities\ImagesEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per images; tutte le query del CRUD sono centralizzate qui. */
final class ImagesModel extends Model
{
    protected $table = 'images';
    protected $primaryKey = 'images_id';
    protected $returnType = ImagesEntity::class;
  ...
  array (
    ...
    public function baseBuilder(): BaseBuilder
    {
        ...
        $builder->select([
            ...
            'images.hotel_id AS hotel_id',
            'images.camera_id AS camera_id',
            'images.obmp_cm_rooms_id AS obmp_cm_rooms_id',
            'images.tipologia_id AS tipologia_id',
            'images.img_small AS img_small',
            'images.img_medium AS img_medium',
            'images.img_large AS img_large',
            'images.titolo AS titolo',
            'images.data_record AS data_record',
            'images.utente_id AS utente_id',
            'obmp_cm_rooms__obmp_cm_rooms_id.obmp_cm_rooms_room_note AS obmp_cm_rooms__obmp_cm_rooms_id__label'
        ]);
        $builder->join('obmp_cm_rooms AS obmp_cm_rooms__obmp_cm_rooms_id', 'obmp_cm_rooms__obmp_cm_rooms_id.obmp_cm_rooms_id = images.obmp_cm_rooms_id', 'left');
        return $builder;
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'images_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'images_id';
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('images.images_id >', $after);
        }

        return $builder
            ->orderBy('images.images_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'images.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('images.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'images_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'images_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('images.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
