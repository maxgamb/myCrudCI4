...
use App\Entities\RegioniEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per regioni; tutte le query del CRUD sono centralizzate qui. */
final class RegioniModel extends Model
{
    protected $table = 'regioni';
    protected $primaryKey = 'regione_id';
    protected $returnType = RegioniEntity::class;
  ...
  3 => 'regione',
    ...
    {
        $builder = $this->db->table('regioni');
        $builder->select([
            'regioni.regione_id AS regione_id',
            'regioni.cod_provincia AS cod_provincia',
            'regioni.provincia AS provincia',
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'regione_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'regione_id';
        ...
        $rows = $dataBuilder
            ->orderBy('regioni.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('regioni.regione_id >', $after);
        }

        return $builder
            ->orderBy('regioni.regione_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'regioni.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('regioni.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'regione_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'regione_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('regioni.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
