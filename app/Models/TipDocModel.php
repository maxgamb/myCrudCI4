...
use App\Entities\TipDocEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per tip_doc; tutte le query del CRUD sono centralizzate qui. */
final class TipDocModel extends Model
{
    protected $table = 'tip_doc';
    protected $primaryKey = 'tip_doc_id';
    protected $returnType = TipDocEntity::class;
  ...
  2 => 'Doc_Descrizione',
    ...
    {
        $builder = $this->db->table('tip_doc');
        $builder->select([
            'tip_doc.tip_doc_id AS tip_doc_id',
            'tip_doc.Doc_CodMin AS Doc_CodMin',
            'tip_doc.Doc_Descrizione AS Doc_Descrizione'
        ]);
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'tip_doc_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'tip_doc_id';
        ...
        $rows = $dataBuilder
            ->orderBy('tip_doc.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('tip_doc.tip_doc_id >', $after);
        }

        return $builder
            ->orderBy('tip_doc.tip_doc_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'tip_doc.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('tip_doc.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'tip_doc_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'tip_doc_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('tip_doc.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
