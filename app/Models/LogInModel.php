...
/** Model per log_in; tutte le query del CRUD sono centralizzate qui. */
final class LogInModel extends Model
{
  ...
  5 => 'log_time',
    ...
    {
        $builder = $this->db->table('log_in');
        $builder->select([
            'log_in.log_in_id AS log_in_id',
            'log_in.log_nome AS log_nome',
            'log_in.log_pass AS log_pass',
            'log_in.log_ip AS log_ip',
            'log_in.log_out AS log_out',
            'log_in.log_time AS log_time'
        ]);
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'log_in_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'log_in_id';
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('log_in.log_in_id >', $after);
        }

        return $builder
            ->orderBy('log_in.log_in_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'log_in.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('log_in.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'log_in_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'log_in_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('log_in.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
