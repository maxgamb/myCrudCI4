...
use App\Entities\ObmpRatesEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per obmp_rates; tutte le query del CRUD sono centralizzate qui. */
final class ObmpRatesModel extends Model
{
    protected $table = 'obmp_rates';
    protected $primaryKey = 'obmp_rate_id';
    protected $returnType = ObmpRatesEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('obmp_rates');
        $builder->select([
            'obmp_rates.obmp_rate_id AS obmp_rate_id',
            'obmp_rates.obmp_cm_rooms_id AS obmp_cm_rooms_id',
            'obmp_rates.obmp_restriction_id AS obmp_restriction_id',
            'obmp_rates.hotel_id AS hotel_id',
            'obmp_rates.obmp_board_cod AS obmp_board_cod',
            'obmp_rates.obmp_cancellation_cod AS obmp_cancellation_cod',
            'obmp_rates.obmp_payment_cod AS obmp_payment_cod',
        ...
        $builder->join('obmp_cm_rooms AS obmp_cm_rooms__obmp_cm_rooms_id', 'obmp_cm_rooms__obmp_cm_rooms_id.obmp_cm_rooms_id = obmp_rates.obmp_cm_rooms_id', 'left');
        $builder->join('obmp_payments AS obmp_payments__obmp_payment_cod', 'obmp_payments__obmp_payment_cod.obmp_payment_cod = obmp_rates.obmp_payment_cod', 'left');
        $builder->join('obmp_restrictions AS obmp_restrictions__obmp_restriction_id', 'obmp_restrictions__obmp_restriction_id.obmp_restriction_id = obmp_rates.obmp_restriction_id', 'left');
        return $builder;
    }
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('obmp_rates.obmp_rate_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'obmp_rate_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'obmp_rate_id';
        ...
        $rows = $dataBuilder
            ->orderBy('obmp_rates.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('obmp_rates.obmp_rate_id >', $after);
        }

        return $builder
            ->orderBy('obmp_rates.obmp_rate_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'obmp_rates.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('obmp_rates.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'obmp_rate_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'obmp_rate_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('obmp_rates.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    /** Restituisce le opzioni della relazione obmp_board_cod. */
