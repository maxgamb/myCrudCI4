...
use App\Entities\ContiNoteEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per conti_note; tutte le query del CRUD sono centralizzate qui. */
final class ContiNoteModel extends Model
{
    protected $table = 'conti_note';
    protected $primaryKey = 'conto_nota_id';
    protected $returnType = ContiNoteEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('conti_note');
        $builder->select([
            'conti_note.conto_nota_id AS conto_nota_id',
            'conti_note.conto_id AS conto_id',
            'conti_note.hotel_id AS hotel_id',
            'conti_note.conto_nota_testo AS conto_nota_testo',
            'conti_note.conto_nota_data_record AS conto_nota_data_record',
            'conti_note.note_conto_utente_id AS note_conto_utente_id',
            'conti__conto_id.trattamento_sog AS conti__conto_id__label'
        ]);
        $builder->join('conti AS conti__conto_id', 'conti__conto_id.conto_id = conti_note.conto_id', 'left');
        return $builder;
    }
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('conti_note.conto_nota_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'conto_nota_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'conto_nota_id';
        ...
        $rows = $dataBuilder
            ->orderBy('conti_note.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('conti_note.conto_nota_id >', $after);
        }

        return $builder
            ->orderBy('conti_note.conto_nota_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'conti_note.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('conti_note.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'conto_nota_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'conto_nota_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('conti_note.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
