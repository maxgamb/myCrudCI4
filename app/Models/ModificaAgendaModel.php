...
use App\Entities\ModificaAgendaEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per modifica_agenda; tutte le query del CRUD sono centralizzate qui. */
final class ModificaAgendaModel extends Model
{
    protected $table = 'modifica_agenda';
    protected $primaryKey = 'mod_agenda_id';
    protected $returnType = ModificaAgendaEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('modifica_agenda');
        $builder->select([
            'modifica_agenda.mod_agenda_id AS mod_agenda_id',
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('modifica_agenda.mod_agenda_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'mod_agenda_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'mod_agenda_id';
        ...
        $rows = $dataBuilder
            ->orderBy('modifica_agenda.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('modifica_agenda.mod_agenda_id >', $after);
        }

        return $builder
            ->orderBy('modifica_agenda.mod_agenda_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'modifica_agenda.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('modifica_agenda.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'mod_agenda_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'mod_agenda_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('modifica_agenda.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    /** Restituisce le opzioni della relazione mod_agenda_id. */
