...
use App\Entities\WrehOrdersEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per wreh_orders; tutte le query del CRUD sono centralizzate qui. */
final class WrehOrdersModel extends Model
{
    protected $table = 'wreh_orders';
    protected $primaryKey = 'order_id';
    protected $returnType = WrehOrdersEntity::class;
  ...
  4 => 'utente_id',
    ...
    {
        $builder = $this->db->table('wreh_orders');
        $builder->select([
            'wreh_orders.order_id AS order_id',
            'wreh_orders.hotel_id AS hotel_id',
            'wreh_orders.order_date AS order_date',
            'wreh_orders.status AS status',
            'wreh_orders.utente_id AS utente_id'
        ]);
    ...
    public function getListPage(
        ...
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'order_id';
        ...
        $rows = $dataBuilder
            ->orderBy('wreh_orders.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('wreh_orders.order_id >', $after);
        }

        return $builder
            ->orderBy('wreh_orders.order_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'wreh_orders.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('wreh_orders.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'order_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'order_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('wreh_orders.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getWrehOrderDetailsByOrderId(int|string $parentId, int $limit = 20): array
    {
    ...
    {
        $result = [];
        $result['wreh_order_details__order_id'] = $this->getWrehOrderDetailsByOrderId($parentId, 20);
        return $result;
    }
