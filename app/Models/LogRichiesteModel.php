...
use App\Entities\LogRichiesteEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per log_richieste; tutte le query del CRUD sono centralizzate qui. */
final class LogRichiesteModel extends Model
{
    protected $table = 'log_richieste';
    protected $primaryKey = 'log_ric_id';
    protected $returnType = LogRichiesteEntity::class;
  ...
  7 => 'log_ric_utente_id',
    ...
    {
        $builder = $this->db->table('log_richieste');
        $builder->select([
            'log_richieste.log_ric_id AS log_ric_id',
            'log_richieste.log_ric_hotel_id AS log_ric_hotel_id',
            'log_richieste.log_ric_dal AS log_ric_dal',
            'log_richieste.log_ric_al AS log_ric_al',
            'log_richieste.log_ric_data AS log_ric_data',
            'log_richieste.log_ric_notti AS log_ric_notti',
            'log_richieste.log_ric_wind AS log_ric_wind',
            'log_richieste.log_ric_utente_id AS log_ric_utente_id'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('log_richieste.log_ric_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'log_ric_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'log_ric_id';
        ...
        $rows = $dataBuilder
            ->orderBy('log_richieste.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('log_richieste.log_ric_id >', $after);
        }

        return $builder
            ->orderBy('log_richieste.log_ric_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'log_richieste.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('log_richieste.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'log_ric_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'log_ric_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('log_richieste.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
