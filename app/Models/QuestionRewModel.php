...
use App\Entities\QuestionRewEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per question_rew; tutte le query del CRUD sono centralizzate qui. */
final class QuestionRewModel extends Model
{
    protected $table = 'question_rew';
    protected $primaryKey = 'question_rew_id';
    protected $returnType = QuestionRewEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('question_rew');
        $builder->select([
            'question_rew.question_rew_id AS question_rew_id',
            'question_rew.question_id AS question_id',
            'question_rew.hotel_id AS hotel_id',
            'question_rew.conto_id AS conto_id',
            'question_rew.clienti_id AS clienti_id',
            'question_rew.valore AS valore',
            'question_rew.data AS data',
            'question__question_id.title AS question__question_id__label'
        ]);
        $builder->join('question AS question__question_id', 'question__question_id.question_id = question_rew.question_id', 'left');
        return $builder;
    }
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('question_rew.question_rew_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'question_rew_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'question_rew_id';
        ...
        $rows = $dataBuilder
            ->orderBy('question_rew.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('question_rew.question_rew_id >', $after);
        }

        return $builder
            ->orderBy('question_rew.question_rew_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'question_rew.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('question_rew.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'question_rew_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'question_rew_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('question_rew.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    /** Restituisce le opzioni della relazione question_id. */
    public function getQuestionQuestionIdOptions(): array
    {
  ...
  1 => 'title',
    ...
    {
        return [
            'question_id' => $this->toRelationOptions($this->getQuestionQuestionIdOptions(), 'question_id'),
        ];
    }
