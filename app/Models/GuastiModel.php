...
use App\Entities\GuastiEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per guasti; tutte le query del CRUD sono centralizzate qui. */
final class GuastiModel extends Model
{
    protected $table = 'guasti';
    protected $primaryKey = 'guasto_id';
    protected $returnType = GuastiEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('guasti');
        $builder->select([
            'guasti.guasto_id AS guasto_id',
            'guasti.hotel_id AS hotel_id',
            'guasti.camera_id AS camera_id',
            'guasti.guasto_priorita AS guasto_priorita',
            'guasti.guasto_area AS guasto_area',
            'guasti.guasto_piano AS guasto_piano',
            'guasti.guasto_note AS guasto_note',
            'guasti.guasto_stato AS guasto_stato',
            'guasti.guasto_data AS guasto_data',
            'guasti.guasto_utente_id AS guasto_utente_id',
            'guasti.guasto_data_record AS guasto_data_record',
            'camere__camera_id.tipologia_camera AS camere__camera_id__label',
            'hotels__hotel_id.nome_hotel AS hotels__hotel_id__label'
        ]);
        $builder->join('camere AS camere__camera_id', 'camere__camera_id.camera_id = guasti.camera_id', 'left');
        $builder->join('hotels AS hotels__hotel_id', 'hotels__hotel_id.hotel_id = guasti.hotel_id', 'left');
        return $builder;
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'guasto_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'guasto_id';
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('guasti.guasto_id >', $after);
        }

        return $builder
            ->orderBy('guasti.guasto_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'guasti.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('guasti.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'guasto_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'guasto_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('guasti.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
  ...
  1 => 'tipologia_camera',
    ...
    /** Restituisce le opzioni della relazione hotel_id. */
    public function getHotelsHotelIdOptions(): array
    {
  ...
  1 => 'nome_hotel',
    ...
    public function relationOptions(): array
    {
        return [
            ...
            'hotel_id' => $this->toRelationOptions($this->getHotelsHotelIdOptions(), 'hotel_id'),
        ];
    }
