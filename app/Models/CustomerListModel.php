...
use App\Entities\CustomerListEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;
use RuntimeException;
use Throwable;

/** Model read-only per SQL VIEW customer_list; estendere manualmente solo se necessario. */
final class CustomerListModel extends Model
{
    protected $table = 'customer_list';
    protected $primaryKey = 'ID';
    protected $returnType = CustomerListEntity::class;
  ...
  8 => 'SID',
    ...
    {
        $builder = $this->db->table('customer_list');
        $builder->select([
    ...
    public function getListPage(
        ...
        $rows = $dataBuilder
            ->orderBy('customer_list.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('customer_list.ID >', $after);
        }

        return $builder
            ->orderBy('customer_list.ID', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'customer_list.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('customer_list.' . $field, $value);
            }
        }
        ...
        $rows = $builder->orderBy('customer_list.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
