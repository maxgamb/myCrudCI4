...
use App\Entities\CodiciStatoEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per codici_stato; tutte le query del CRUD sono centralizzate qui. */
final class CodiciStatoModel extends Model
{
    protected $table = 'codici_stato';
    protected $primaryKey = 'cod_stato';
    protected $returnType = CodiciStatoEntity::class;
  ...
  2 => 'stato_commento',
    ...
    {
        $builder = $this->db->table('codici_stato');
        $builder->select([
            'codici_stato.cod_stato AS cod_stato',
            'codici_stato.stato AS stato',
            'codici_stato.stato_commento AS stato_commento'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('codici_stato.cod_stato', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'cod_stato',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'cod_stato';
        ...
        $rows = $dataBuilder
            ->orderBy('codici_stato.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('codici_stato.cod_stato >', $after);
        }

        return $builder
            ->orderBy('codici_stato.cod_stato', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'codici_stato.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('codici_stato.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'cod_stato');
        $sort = in_array($sort, $sortable, true) ? $sort : 'cod_stato';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('codici_stato.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
