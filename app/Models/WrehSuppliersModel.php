...
use App\Entities\WrehSuppliersEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per wreh_suppliers; tutte le query del CRUD sono centralizzate qui. */
final class WrehSuppliersModel extends Model
{
    protected $table = 'wreh_suppliers';
    protected $primaryKey = 'supplier_id';
    protected $returnType = WrehSuppliersEntity::class;
  ...
  6 => 'utente_id',
    ...
    {
        $builder = $this->db->table('wreh_suppliers');
        $builder->select([
            'wreh_suppliers.supplier_id AS supplier_id',
            'wreh_suppliers.company AS company',
            'wreh_suppliers.contact_name AS contact_name',
            'wreh_suppliers.phone AS phone',
            'wreh_suppliers.email AS email',
            'wreh_suppliers.address AS address',
            'wreh_suppliers.utente_id AS utente_id'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('wreh_suppliers.supplier_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'supplier_id';
        ...
        $rows = $dataBuilder
            ->orderBy('wreh_suppliers.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('wreh_suppliers.supplier_id >', $after);
        }

        return $builder
            ->orderBy('wreh_suppliers.supplier_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'wreh_suppliers.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('wreh_suppliers.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'supplier_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'supplier_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('wreh_suppliers.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getWrehProductsBySupplierId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('wreh_products')
            ->where('supplier_id', $parentId)
    ...
    {
        $result = [];
        $result['wreh_products__supplier_id'] = $this->getWrehProductsBySupplierId($parentId, 20);
        return $result;
    }
