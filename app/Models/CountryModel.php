...
/**
 * Model for `country`. Centralizes CRUD queries, filters, relations, and persistence.
 *
 ...
 */
final class CountryModel extends BaseCrudModel
{
  ...
  2 => 'last_update',
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'country_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'country_id';
        ...
        $rows = $dataBuilder
            ->orderBy('country.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('country.country_id >', $after);
        }

        return $builder
            ->orderBy('country.country_id', 'ASC')
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('country.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'country_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'country_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('country.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    /**
     * HasMany scaffolding delegated to the Model that owns table city.
     * The current Model only names the relation; it no longer composes child SQL.
     */
    public function getCityByCountryId(int|string $parentId, int $limit = 20): array
    {
        return (new CityModel())->childrenByForeignKey(
            'country_id',
            $parentId,
            array (
  0 => 'city_id',
  ...
  3 => 'last_update',
    ...
    {
        $result = [];
        $result['city__country_id'] = $this->getCityByCountryId($parentId, 20);
        return $result;
    }
}
