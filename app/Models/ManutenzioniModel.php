...
use App\Entities\ManutenzioniEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per manutenzioni; tutte le query del CRUD sono centralizzate qui. */
final class ManutenzioniModel extends Model
{
    protected $table = 'manutenzioni';
    protected $primaryKey = 'manutenzione_id';
    protected $returnType = ManutenzioniEntity::class;
  ...
  8 => 'manut_stato',
    ...
    {
        $builder = $this->db->table('manutenzioni');
        $builder->select([
            'manutenzioni.manutenzione_id AS manutenzione_id',
            'manutenzioni.hotel_id AS hotel_id',
            'manutenzioni.manut_priorita AS manut_priorita',
            'manutenzioni.manut_area_guasto AS manut_area_guasto',
            'manutenzioni.manut_piano AS manut_piano',
            'manutenzioni.manut_camera AS manut_camera',
            'manutenzioni.manut_descrizione AS manut_descrizione',
            'manutenzioni.manut_data_segnalazione AS manut_data_segnalazione',
            'manutenzioni.manut_stato AS manut_stato',
            'manutenzioni.manut_data_record AS manut_data_record'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('manutenzioni.manutenzione_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'manutenzione_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'manutenzione_id';
        ...
        $rows = $dataBuilder
            ->orderBy('manutenzioni.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('manutenzioni.manutenzione_id >', $after);
        }

        return $builder
            ->orderBy('manutenzioni.manutenzione_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'manutenzioni.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('manutenzioni.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'manutenzione_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'manutenzione_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('manutenzioni.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
