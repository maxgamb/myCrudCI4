...
use App\Entities\RefAgenziaPrenoEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per ref_agenzia_preno; tutte le query del CRUD sono centralizzate qui. */
final class RefAgenziaPrenoModel extends Model
{
    protected $table = 'ref_agenzia_preno';
    protected $primaryKey = 'ref_agenzia_preno';
    protected $returnType = RefAgenziaPrenoEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('ref_agenzia_preno');
        $builder->select([
            'ref_agenzia_preno.ref_agenzia_preno AS ref_agenzia_preno',
            'ref_agenzia_preno.agenzia_id AS agenzia_id',
            'ref_agenzia_preno.preno_id AS preno_id',
            'ref_agenzia_preno.ref_a_p_datarecord AS ref_a_p_datarecord',
            'agenda__preno_id.preno_arr_ore AS agenda_preno_arr_ore',
            'agenzie__agenzia_id.agenzia_tipologia AS agenzie_agenzia_tipologia'
        ]);
        $builder->join('agenda AS agenda__preno_id', 'agenda__preno_id.preno_id = ref_agenzia_preno.preno_id', 'left');
        $builder->join('agenzie AS agenzie__agenzia_id', 'agenzie__agenzia_id.agenzia_id = ref_agenzia_preno.agenzia_id', 'left');
        return $builder;
    }
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('ref_agenzia_preno.ref_agenzia_preno', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'ref_agenzia_preno',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'ref_agenzia_preno';
        ...
        $rows = $dataBuilder
            ->orderBy('ref_agenzia_preno.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('ref_agenzia_preno.ref_agenzia_preno >', $after);
        }

        return $builder
            ->orderBy('ref_agenzia_preno.ref_agenzia_preno', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'ref_agenzia_preno.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('ref_agenzia_preno.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'ref_agenzia_preno');
        $sort = in_array($sort, $sortable, true) ? $sort : 'ref_agenzia_preno';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('ref_agenzia_preno.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
