...
use App\Entities\PrezziEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per prezzi; tutte le query del CRUD sono centralizzate qui. */
final class PrezziModel extends Model
{
    protected $table = 'prezzi';
    protected $primaryKey = 'prezzo_id';
    protected $returnType = PrezziEntity::class;
  ...
  7 => 'prezzi_utente_id',
    ...
    {
        $builder = $this->db->table('prezzi');
        $builder->select([
            'prezzi.prezzo_id AS prezzo_id',
            'prezzi.hotel_id AS hotel_id',
            'prezzi.conto_id AS conto_id',
            'prezzi.prezzo_dal AS prezzo_dal',
            'prezzi.prezzo_al AS prezzo_al',
            'prezzi.prezzo_valore AS prezzo_valore',
            'prezzi.prezzo_data_record AS prezzo_data_record',
            'prezzi.libero AS libero',
            'prezzi.prezzi_utente_id AS prezzi_utente_id'
        ]);
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'prezzo_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'prezzo_id';
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('prezzi.prezzo_id >', $after);
        }

        return $builder
            ->orderBy('prezzi.prezzo_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'prezzi.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('prezzi.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'prezzo_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'prezzo_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('prezzi.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
