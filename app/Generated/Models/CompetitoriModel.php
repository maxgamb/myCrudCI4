...
use App\Entities\CompetitoriEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per competitori; tutte le query del CRUD sono centralizzate qui. */
final class CompetitoriModel extends Model
{
    protected $table = 'competitori';
    protected $primaryKey = 'competitore_id';
    protected $returnType = CompetitoriEntity::class;
  ...
  5 => 'qualita_trivago',
    ...
    {
        $builder = $this->db->table('competitori');
        $builder->select([
            'competitori.competitore_id AS competitore_id',
            'competitori.hotel_id AS hotel_id',
            'competitori.livello_dicompetizione AS livello_dicompetizione',
            'competitori.competitore_nome AS competitore_nome',
            'competitori.competitore_venere_id AS competitore_venere_id',
            'competitori.qualita_trivago AS qualita_trivago',
            'competitori.competitore_data_record AS competitore_data_record'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('competitori.competitore_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'competitore_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'competitore_id';
        ...
        $rows = $dataBuilder
            ->orderBy('competitori.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('competitori.competitore_id >', $after);
        }

        return $builder
            ->orderBy('competitori.competitore_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'competitori.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('competitori.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'competitore_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'competitore_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('competitori.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
