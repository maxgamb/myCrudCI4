...
use App\Entities\ChecklistPrenoEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per checklist_preno; tutte le query del CRUD sono centralizzate qui. */
final class ChecklistPrenoModel extends Model
{
    protected $table = 'checklist_preno';
    protected $primaryKey = 'checklist_id';
    protected $returnType = ChecklistPrenoEntity::class;
  ...
  16 => 'utente_id',
    ...
    {
        $builder = $this->db->table('checklist_preno');
        $builder->select([
            'checklist_preno.checklist_id AS checklist_id',
            'checklist_preno.hotel_id AS hotel_id',
            'checklist_preno.preno_id AS preno_id',
            'checklist_preno.preno_dal AS preno_dal',
            'checklist_preno.email AS email',
            'checklist_preno.email_pms AS email_pms',
            'checklist_preno.lista AS lista',
            'checklist_preno.lista_pms AS lista_pms',
            'checklist_preno.pagamento AS pagamento',
            'checklist_preno.tassa AS tassa',
            'checklist_preno.proforma AS proforma',
            'checklist_preno.proforma_pms AS proforma_pms',
            'checklist_preno.bonifico AS bonifico',
            'checklist_preno.importo AS importo',
            'checklist_preno.note AS note',
            'checklist_preno.data_check AS data_check',
            'checklist_preno.utente_id AS utente_id',
            'checklist_preno.data_record AS data_record'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('checklist_preno.checklist_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'checklist_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'checklist_id';
        ...
        $rows = $dataBuilder
            ->orderBy('checklist_preno.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('checklist_preno.checklist_id >', $after);
        }

        return $builder
            ->orderBy('checklist_preno.checklist_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'checklist_preno.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('checklist_preno.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'checklist_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'checklist_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('checklist_preno.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
