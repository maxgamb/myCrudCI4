...
use App\Entities\LettereEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per lettere; tutte le query del CRUD sono centralizzate qui. */
final class LettereModel extends Model
{
    protected $table = 'lettere';
    protected $primaryKey = 'lettere_id';
    protected $returnType = LettereEntity::class;
  ...
  11 => 'data_stamp',
    ...
    {
        $builder = $this->db->table('lettere');
        $builder->select([
            'lettere.lettere_id AS lettere_id',
            'lettere.etichetta AS etichetta',
            'lettere.hotel_id AS hotel_id',
            'lettere.titolo AS titolo',
            'lettere.reparto AS reparto',
            'lettere.contoller AS contoller',
            'lettere.en AS en',
            'lettere.it AS it',
            'lettere.es AS es',
            'lettere.fr AS fr',
            'lettere.de AS de',
            'lettere.data_stamp AS data_stamp'
        ]);
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'lettere_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'lettere_id';
        ...
        $rows = $dataBuilder
            ->orderBy('lettere.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('lettere.lettere_id >', $after);
        }

        return $builder
            ->orderBy('lettere.lettere_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'lettere.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('lettere.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'lettere_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'lettere_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('lettere.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
