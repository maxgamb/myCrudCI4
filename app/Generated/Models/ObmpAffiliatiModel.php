...
use App\Entities\ObmpAffiliatiEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per obmp_affiliati; tutte le query del CRUD sono centralizzate qui. */
final class ObmpAffiliatiModel extends Model
{
    protected $table = 'obmp_affiliati';
    protected $primaryKey = 'obmp_affiliati_id';
    protected $returnType = ObmpAffiliatiEntity::class;
  ...
  7 => 'obmp_aff_mark_up',
    ...
    {
        $builder = $this->db->table('obmp_affiliati');
        $builder->select([
            'obmp_affiliati.obmp_affiliati_id AS obmp_affiliati_id',
            'obmp_affiliati.obmp_aff_societa AS obmp_aff_societa',
            'obmp_affiliati.obmp_aff_sito AS obmp_aff_sito',
            'obmp_affiliati.obmp_aff_email AS obmp_aff_email',
            'obmp_affiliati.obmp_aff_pasword AS obmp_aff_pasword',
            'obmp_affiliati.obmp_aff_cookies AS obmp_aff_cookies',
            'obmp_affiliati.obmp_aff_commisione AS obmp_aff_commisione',
            'obmp_affiliati.obmp_aff_mark_up AS obmp_aff_mark_up',
            'obmp_affiliati.obmp_aff_data_record AS obmp_aff_data_record'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('obmp_affiliati.obmp_affiliati_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'obmp_affiliati_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'obmp_affiliati_id';
        ...
        $rows = $dataBuilder
            ->orderBy('obmp_affiliati.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('obmp_affiliati.obmp_affiliati_id >', $after);
        }

        return $builder
            ->orderBy('obmp_affiliati.obmp_affiliati_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'obmp_affiliati.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('obmp_affiliati.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'obmp_affiliati_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'obmp_affiliati_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('obmp_affiliati.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
