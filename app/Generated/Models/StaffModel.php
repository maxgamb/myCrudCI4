  ...
  14 => 'utente_id',
    ...
    public function baseBuilder(): BaseBuilder
    {
        ...
        $builder->select([
            ...
            'staff.cognome AS cognome',
            'staff.nome AS nome',
            'staff.citta AS citta',
            'staff.provincia AS provincia',
            'staff.staff_nazione AS staff_nazione',
            'staff.indirizzo AS indirizzo',
            'staff.telefono AS telefono',
            'staff.cellulare AS cellulare',
            'staff.email AS email',
            'staff.genere AS genere',
            'staff.reparto_id AS reparto_id',
            'staff.staff_stato AS staff_stato',
            'staff.staff_datarecod AS staff_datarecod',
            'staff.utente_id AS utente_id'
        ]);
    ...
    public function getShiftsByStaffId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('shifts')
    ...
    /** Carica al massimo una riga in più per determinare se esistono altri risultati. */
    public function getUtentiByStaffId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('utenti')
            ->where('staff_id', $parentId)
            ->orderBy('Utente_id', 'DESC')
            ->limit($limit + 1)
            ->get()
            ->getResult();
        $hasMore = count($rows) > $limit;
    ...
    {
        $result = [];
        $result['shifts__staff_id'] = $this->getShiftsByStaffId($parentId, 20);

        $result['utenti__staff_id'] = $this->getUtentiByStaffId($parentId, 20);
        return $result;
    }
