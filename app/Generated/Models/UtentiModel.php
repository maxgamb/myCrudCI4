...
use App\Entities\UtentiEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per utenti; tutte le query del CRUD sono centralizzate qui. */
final class UtentiModel extends Model
{
    protected $table = 'utenti';
    protected $primaryKey = 'Utente_id';
    protected $returnType = UtentiEntity::class;
  ...
  array (
    ...
    public function baseBuilder(): BaseBuilder
    {
        ...
        $builder->select([
            'utenti.Utente_id AS Utente_id',
            'utenti.staff_id AS staff_id',
            'utenti.Nome_Utente AS Nome_Utente',
            'utenti.Pass_Utente AS Pass_Utente',
            'utenti.Email_Utente AS Email_Utente',
            'utenti.hotel_id AS hotel_id',
            'utenti.utenti_livello AS utenti_livello',
            'utenti.utenti_data_record AS utenti_data_record',
            'utenti.utenti_Utente_id AS utenti_Utente_id',
            'staff__staff_id.nome AS staff_nome'
        ]);
        $builder->join('staff AS staff__staff_id', 'staff__staff_id.staff_id = utenti.staff_id', 'left');
        return $builder;
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'Utente_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'Utente_id';
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('utenti.Utente_id >', $after);
        }

        return $builder
            ->orderBy('utenti.Utente_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'utenti.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('utenti.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'Utente_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'Utente_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('utenti.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    {
        return [
            'staff_id' => $this->toOptions($this->getStaffStaffIdOptions(), 'staff_id', 'nome'),
        ];
    }
    ...
    public function getNoteUtenteByUtenteId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('note_utente')
            ->where('Utente_id', $parentId)
            ->orderBy('note_utente_id', 'DESC')
            ->limit($limit + 1)
            ->get()
            ->getResult();
    ...
    {
        $result = [];
        $result['note_utente__Utente_id'] = $this->getNoteUtenteByUtenteId($parentId, 20);
        return $result;
    }
