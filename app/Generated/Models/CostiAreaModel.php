...
use App\Entities\CostiAreaEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per costi_area; tutte le query del CRUD sono centralizzate qui. */
final class CostiAreaModel extends Model
{
    protected $table = 'costi_area';
    protected $primaryKey = 'costi_area_id';
    protected $returnType = CostiAreaEntity::class;
  ...
  3 => 'utente_id',
    ...
    {
        $builder = $this->db->table('costi_area');
        $builder->select([
            'costi_area.costi_area_id AS costi_area_id',
            'costi_area.costi_area_nome AS costi_area_nome',
            'costi_area.costi_area AS costi_area',
            'costi_area.utente_id AS utente_id',
            'costi_area.costi_area_data_record AS costi_area_data_record'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('costi_area.costi_area_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'costi_area_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'costi_area_id';
        ...
        $rows = $dataBuilder
            ->orderBy('costi_area.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('costi_area.costi_area_id >', $after);
        }

        return $builder
            ->orderBy('costi_area.costi_area_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'costi_area.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('costi_area.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'costi_area_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'costi_area_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('costi_area.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getCostiVarByCostiAreaId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('costi_var')
            ->where('costi_area_id', $parentId)
            ->orderBy('costi_var_id', 'DESC')
            ->limit($limit + 1)
            ->get()
            ->getResult();
    ...
    /** Carica al massimo una riga in più per determinare se esistono altri risultati. */
    public function getWrehProductsByCostiAreaId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('wreh_products')
            ->where('costi_area_id', $parentId)
    ...
    {
        $result = [];
        $result['costi_var__costi_area_id'] = $this->getCostiVarByCostiAreaId($parentId, 20);

        $result['wreh_products__costi_area_id'] = $this->getWrehProductsByCostiAreaId($parentId, 20);
        return $result;
    }
