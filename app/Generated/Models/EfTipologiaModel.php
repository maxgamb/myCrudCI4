...
use App\Entities\EfTipologiaEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per ef_tipologia; tutte le query del CRUD sono centralizzate qui. */
final class EfTipologiaModel extends Model
{
    protected $table = 'ef_tipologia';
    protected $primaryKey = 'pax';
    protected $returnType = EfTipologiaEntity::class;
  ...
  4 => '1',
    ...
    {
        $builder = $this->db->table('ef_tipologia');
        $builder->select([
            'ef_tipologia.pax AS pax',
            'ef_tipologia.4 AS 4',
            'ef_tipologia.3 AS 3',
            'ef_tipologia.2 AS 2',
            'ef_tipologia.1 AS 1'
    ...
    public function getListPage(
        ...
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'pax';
        ...
        $rows = $dataBuilder
            ->orderBy('ef_tipologia.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('ef_tipologia.pax >', $after);
        }

        return $builder
            ->orderBy('ef_tipologia.pax', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'ef_tipologia.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('ef_tipologia.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'pax');
        $sort = in_array($sort, $sortable, true) ? $sort : 'pax';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('ef_tipologia.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
