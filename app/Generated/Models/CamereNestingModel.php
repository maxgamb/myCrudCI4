...
use App\Entities\CamereNestingEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per camere_nesting; tutte le query del CRUD sono centralizzate qui. */
final class CamereNestingModel extends Model
{
    protected $table = 'camere_nesting';
    protected $primaryKey = 'nesting_id';
    protected $returnType = CamereNestingEntity::class;
  ...
  4 => 'nesting_utente_id',
    ...
    {
        $builder = $this->db->table('camere_nesting');
        $builder->select([
            'camere_nesting.nesting_id AS nesting_id',
            'camere_nesting.camara_id AS camara_id',
            'camere_nesting.tipologia_id AS tipologia_id',
            'camere_nesting.voto AS voto',
            'camere_nesting.nesting_data_record AS nesting_data_record',
            'camere_nesting.nesting_utente_id AS nesting_utente_id'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('camere_nesting.nesting_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'nesting_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'nesting_id';
        ...
        $rows = $dataBuilder
            ->orderBy('camere_nesting.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('camere_nesting.nesting_id >', $after);
        }

        return $builder
            ->orderBy('camere_nesting.nesting_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'camere_nesting.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('camere_nesting.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'nesting_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'nesting_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('camere_nesting.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
