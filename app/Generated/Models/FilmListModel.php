...
use App\Entities\FilmListEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;
use RuntimeException;
use Throwable;

/** Model per film_list; tutte le query del CRUD sono centralizzate qui. */
final class FilmListModel extends Model
{
    protected $table = 'film_list';
    protected $primaryKey = 'FID';
    protected $returnType = FilmListEntity::class;
  ...
  7 => 'actors',
    ...
    {
        $builder = $this->db->table('film_list');
        $builder->select([
    ...
    public function getListPage(
        ...
        $rows = $dataBuilder
            ->orderBy('film_list.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('film_list.FID >', $after);
        }

        return $builder
            ->orderBy('film_list.FID', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'film_list.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('film_list.' . $field, $value);
            }
        }
        ...
        $rows = $builder->orderBy('film_list.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
