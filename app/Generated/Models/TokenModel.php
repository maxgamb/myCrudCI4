...
/** Model per token; tutte le query del CRUD sono centralizzate qui. */
final class TokenModel extends Model
{
    protected $table = 'token';
    protected $primaryKey = 'token_id';
    protected $returnType = TokenEntity::class;
  ...
  3 => 'token_data',
    ...
    public function baseBuilder(): BaseBuilder
    {
        ...
        $builder->select([
            ...
            'token.hotel_id AS hotel_id',
            'token.token AS token',
            'token.token_data AS token_data',
            'token.token_data_record AS token_data_record'
        ]);
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'token_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'token_id';
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('token.token_id >', $after);
        }

        return $builder
            ->orderBy('token.token_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'token.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('token.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'token_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'token_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('token.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
