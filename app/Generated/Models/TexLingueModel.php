...
use App\Entities\TexLingueEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per tex_lingue; tutte le query del CRUD sono centralizzate qui. */
final class TexLingueModel extends Model
{
    protected $table = 'tex_lingue';
    protected $primaryKey = 'tex_lingue_id';
    protected $returnType = TexLingueEntity::class;
  ...
  7 => 'reparto_id',
    ...
    {
        $builder = $this->db->table('tex_lingue');
        $builder->select([
            'tex_lingue.tex_lingue_id AS tex_lingue_id',
            'tex_lingue.etichetta_lg AS etichetta_lg',
            'tex_lingue.en AS en',
            'tex_lingue.it AS it',
            'tex_lingue.es AS es',
            'tex_lingue.fr AS fr',
            'tex_lingue.de AS de',
            'tex_lingue.reparto_id AS reparto_id'
        ]);
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('tex_lingue.tex_lingue_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'tex_lingue_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'tex_lingue_id';
        ...
        $rows = $dataBuilder
            ->orderBy('tex_lingue.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('tex_lingue.tex_lingue_id >', $after);
        }

        return $builder
            ->orderBy('tex_lingue.tex_lingue_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'tex_lingue.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('tex_lingue.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'tex_lingue_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'tex_lingue_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('tex_lingue.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
