...
use App\Entities\ContiTrasferisciEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per conti_trasferisci; tutte le query del CRUD sono centralizzate qui. */
final class ContiTrasferisciModel extends Model
{
    protected $table = 'conti_trasferisci';
    protected $primaryKey = 'conti_trasferisci_id';
    protected $returnType = ContiTrasferisciEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('conti_trasferisci');
        $builder->select([
            'conti_trasferisci.conti_trasferisci_id AS conti_trasferisci_id',
            'conti_trasferisci.conto_id_ex AS conto_id_ex',
            'conti_trasferisci.conto_id_new AS conto_id_new',
            'conti_trasferisci.hotel_id AS hotel_id',
            'conti_trasferisci.adebito_id AS adebito_id',
            'conti_trasferisci.conti_tra_data AS conti_tra_data',
            'adebiti__adebito_id.descrizione AS adebiti_descrizione'
        ]);
        $builder->join('adebiti AS adebiti__adebito_id', 'adebiti__adebito_id.adebito_id = conti_trasferisci.adebito_id', 'left');
        return $builder;
    }
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('conti_trasferisci.conti_trasferisci_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        int $page = 1,
        int $perPage = 25,
        string $sort = 'conti_trasferisci_id',
        string $direction = 'desc'
    ): array {
        $page = max(1, $page);
        $perPage = max(25, min(100, $perPage));
        $sort = in_array($sort, self::SORTABLE_FIELDS, true) ? $sort : 'conti_trasferisci_id';
        ...
        $rows = $dataBuilder
            ->orderBy('conti_trasferisci.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('conti_trasferisci.conti_trasferisci_id >', $after);
        }

        return $builder
            ->orderBy('conti_trasferisci.conti_trasferisci_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'conti_trasferisci.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('conti_trasferisci.' . $field, $value);
            }
        }

        $sort = (string) ($query['sort'] ?? 'conti_trasferisci_id');
        $sort = in_array($sort, $sortable, true) ? $sort : 'conti_trasferisci_id';
        $direction = strtolower((string) ($query['direction'] ?? 'asc')) === 'desc' ? 'DESC' : 'ASC';
        $total = (clone $builder)->countAllResults(false);
        $rows = $builder->orderBy('conti_trasferisci.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    /** Restituisce le opzioni della relazione adebito_id. */
    public function getAdebitiAdebitoIdOptions(): array
    {
        return $this->db->table('adebiti')
            ->select(['adebito_id', 'descrizione'])
    ...
    {
        return [
            'adebito_id' => $this->toOptions($this->getAdebitiAdebitoIdOptions(), 'adebito_id', 'descrizione'),
        ];
    }
