...
use App\Entities\ReferClientiEntity;
use CodeIgniter\Database\BaseBuilder;
use CodeIgniter\Model;

/** Model per refer_clienti; tutte le query del CRUD sono centralizzate qui. */
final class ReferClientiModel extends Model
{
    protected $table = 'refer_clienti';
    protected $primaryKey = 'conto_id';
    protected $returnType = ReferClientiEntity::class;
  ...
  array (
    ...
    {
        $builder = $this->db->table('refer_clienti');
        $builder->select([
            'refer_clienti.conto_id AS conto_id',
            'refer_clienti.clienti_id AS clienti_id',
            'refer_clienti.hotel_id AS hotel_id',
            'refer_clienti.ps_valore AS ps_valore',
            'refer_clienti.ref_clinti_data_record AS ref_clinti_data_record',
            'refer_clienti.refer_clienti_utente_id AS refer_clienti_utente_id',
            'refer_clienti.refer_clienti_conto_id AS refer_clienti_conto_id',
            'conti__conto_id.trattamento_sog AS conti_trattamento_sog'
        ]);
        $builder->join('conti AS conti__conto_id', 'conti__conto_id.conto_id = refer_clienti.conto_id', 'left');
        return $builder;
    }
    ...
    public function getDetail(int|string $id): ?object
    {
        return $this->baseBuilder()
            ->where('refer_clienti.conto_id', $id)
            ->get()
            ->getRow();
    }
    ...
    public function getListPage(
        ...
        $rows = $dataBuilder
            ->orderBy('refer_clienti.' . $sort, $direction)
    ...
    public function getExportRows(array $filters, int $limit = 2000, int|string|null $after = null): array
    {
        ...
        if ($after !== null && $after !== '') {
            $builder->where('refer_clienti.conto_id >', $after);
        }

        return $builder
            ->orderBy('refer_clienti.conto_id', 'ASC')
    ...
    private function applyListFilters(BaseBuilder $builder, array $filters, bool $qualified): void
    {
        ...
        foreach (array_values($filters) as $filter) {
            ...
            $column = $qualified ? 'refer_clienti.' . $field : $field;
    ...
    public function apiList(array $query, array $filterable, array $sortable): array
    {
        ...
        foreach ((array) ($query['filter'] ?? []) as $field => $value) {
            if (is_scalar($value) && in_array($field, $filterable, true) && (string) $value !== '') {
                $builder->where('refer_clienti.' . $field, $value);
            }
        }
        ...
        $rows = $builder->orderBy('refer_clienti.' . $sort, $direction)
            ->limit($perPage, ($page - 1) * $perPage)
            ->get()
            ->getResult();
    ...
    public function getClientiByClientiId(int|string $parentId, int $limit = 20): array
    {
        $limit = max(1, min(200, $limit));
        $rows = $this->db->table('clienti')
            ->where('clienti_id', $parentId)
            ->orderBy('clienti_id', 'DESC')
            ->limit($limit + 1)
            ->get()
            ->getResult();
    ...
    {
        $result = [];
        $result['clienti__clienti_id'] = $this->getClientiByClientiId($parentId, 20);
        return $result;
    }
