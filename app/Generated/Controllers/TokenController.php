...
use App\Validation\TokenRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per token.
 *
...
final class TokenController extends BaseController
{
  ...
  2 => 100,
    ...
    public function store()
    {
        ...
        if (!$this->validate(TokenRules::createRules(), TokenRules::messages())) {
        ...
        return redirect()->to(site_url('token'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(TokenRules::updateRules($id), TokenRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('token'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('token'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('token'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'token_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('token') . '?' . http_build_query((array) $this->request->getGet()))
