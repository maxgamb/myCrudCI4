...
use App\Services\CostiVarService;
use App\Validation\CostiVarRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per costi_var.
 *
 ...
 */
final class CostiVarController extends BaseController
{
    ...
    private CostiVarService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new CostiVarService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('costi_var/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('costi_var/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(CostiVarRules::createRules(), CostiVarRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['costi_var_id']);
        ...
        return redirect()->to(site_url('costi_var'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('costi_var/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CostiVarRules::updateRules($id), CostiVarRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('costi_var'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('costi_var'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('costi_var'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'costi_var_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('costi_var') . '?' . http_build_query((array) $this->request->getGet()))
