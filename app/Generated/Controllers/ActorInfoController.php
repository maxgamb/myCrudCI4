...
use App\Validation\ActorInfoRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per actor_info.
 *
 ...
 */
final class ActorInfoController extends BaseController
{
    ...
    private ActorInfoService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ActorInfoService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('actor_info/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('actor_info/index', $data);
    }
  ...
  2 => 100,
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = ActorInfoRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('actor_info') . ($query === [] ? '' : '?' . http_build_query($query));
