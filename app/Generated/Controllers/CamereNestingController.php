...
use App\Services\CamereNestingService;
use App\Validation\CamereNestingRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per camere_nesting.
 *
 ...
 */
final class CamereNestingController extends BaseController
{
    ...
    private CamereNestingService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new CamereNestingService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('camere_nesting/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('camere_nesting/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('camere_nesting/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('camere_nesting/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(CamereNestingRules::createRules(), CamereNestingRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['nesting_id']);
        ...
        return redirect()->to(site_url('camere_nesting'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('camere_nesting/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CamereNestingRules::updateRules($id), CamereNestingRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('camere_nesting'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('camere_nesting'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('camere_nesting'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'nesting_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'camere_nesting',
                languageGroup: 'CamereNesting',
    ...
    {
        return redirect()->to(site_url('camere_nesting') . '?' . http_build_query((array) $this->request->getGet()))
