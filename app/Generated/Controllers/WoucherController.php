...
use App\Services\WoucherService;
use App\Validation\WoucherRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per woucher.
 *
 ...
 */
final class WoucherController extends BaseController
{
    ...
    private WoucherService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new WoucherService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('woucher/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('woucher/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(WoucherRules::createRules(), WoucherRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['woucher_id']);
        ...
        return redirect()->to(site_url('woucher'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('woucher/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(WoucherRules::updateRules($id), WoucherRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('woucher'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('woucher'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('woucher'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'woucher_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('woucher') . '?' . http_build_query((array) $this->request->getGet()))
