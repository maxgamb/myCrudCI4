...
/**
 * Controller CRUD full per app_ip.
 *
 ...
 */
final class AppIpController extends BaseController
{
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('app_ip/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('app_ip/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(AppIpRules::createRules(), AppIpRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['app_ip_id']);
        ...
        return redirect()->to(site_url('app_ip'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(AppIpRules::updateRules($id), AppIpRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('app_ip'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('app_ip'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('app_ip'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'app_ip_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('app_ip') . '?' . http_build_query((array) $this->request->getGet()))
