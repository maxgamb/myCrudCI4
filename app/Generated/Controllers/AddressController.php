...
/**
 * Controller CRUD full per address.
 *
...
final class AddressController extends BaseController
{
  ...
  array (
    ...
    private AddressService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new AddressService();
  ...
  1 => 'city_id',
        ...
        if ($this->request->isAJAX()) {
            return view('address/_table', $data);
        }
  ...
  0 => 'city_id',
    ...
    public function store()
    {
        ...
        $createRules = AddressRules::createRules();
        ...
        if (!$this->validate($createRules, AddressRules::messages())) {
        ...
        return redirect()->to($this->contextUrl('address', $navigationContext))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(AddressRules::updateRules($id), AddressRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to($this->contextUrl('address', $navigationContext))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        ...
        try {
            ...
            return redirect()->to($this->contextUrl('address', $navigationContext))->with('error', $e->getMessage());
        }
        return redirect()->to($this->contextUrl('address', $navigationContext))->with('message', 'Record eliminato correttamente.');
    }
    /**
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'address_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
), array (
  ...
  0 => 'last_update',
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = AddressRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('address') . ($query === [] ? '' : '?' . http_build_query($query));
