...
use App\Services\ObmpAffiliatiService;
use App\Validation\ObmpAffiliatiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_affiliati.
 *
 ...
 */
final class ObmpAffiliatiController extends BaseController
{
    ...
    private ObmpAffiliatiService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpAffiliatiService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_affiliati/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_affiliati/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('obmp_affiliati/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('obmp_affiliati/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpAffiliatiRules::createRules(), ObmpAffiliatiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['obmp_affiliati_id']);
        ...
        return redirect()->to(site_url('obmp_affiliati'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_affiliati/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpAffiliatiRules::updateRules($id), ObmpAffiliatiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_affiliati'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_affiliati'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_affiliati'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'obmp_affiliati_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'obmp_affiliati',
                languageGroup: 'ObmpAffiliati',
    ...
    {
        return redirect()->to(site_url('obmp_affiliati') . '?' . http_build_query((array) $this->request->getGet()))
