...
use App\Services\CompetitoriService;
use App\Validation\CompetitoriRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per competitori.
 *
 ...
 */
final class CompetitoriController extends BaseController
{
    ...
    private CompetitoriService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new CompetitoriService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('competitori/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('competitori/index', $data);
    }
    ...
    {
        return view('competitori/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(CompetitoriRules::createRules(), CompetitoriRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['competitore_id']);
        ...
        return redirect()->to(site_url('competitori'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('competitori/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CompetitoriRules::updateRules($id), CompetitoriRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('competitori'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('competitori'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('competitori'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'competitore_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'competitori',
                languageGroup: 'Competitori',
    ...
    {
        return redirect()->to(site_url('competitori') . '?' . http_build_query((array) $this->request->getGet()))
