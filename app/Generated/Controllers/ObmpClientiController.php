...
use App\Services\ObmpClientiService;
use App\Validation\ObmpClientiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_clienti.
 *
 ...
 */
final class ObmpClientiController extends BaseController
{
    ...
    private ObmpClientiService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpClientiService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_clienti/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_clienti/index', $data);
    }
    ...
    {
        return view('obmp_clienti/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpClientiRules::createRules(), ObmpClientiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['obm_cliente_id']);
        ...
        return redirect()->to(site_url('obmp_clienti'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_clienti/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpClientiRules::updateRules($id), ObmpClientiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_clienti'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_clienti'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_clienti'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'obm_cliente_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'obmp_clienti',
                languageGroup: 'ObmpClienti',
    ...
    {
        return redirect()->to(site_url('obmp_clienti') . '?' . http_build_query((array) $this->request->getGet()))
