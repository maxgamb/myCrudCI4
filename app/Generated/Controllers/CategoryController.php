...
/**
 * Controller CRUD full per category.
 *
...
final class CategoryController extends BaseController
{
    ...
    private CategoryService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new CategoryService();
  ...
  0 => 'category_id',
        ...
        if ($this->request->isAJAX()) {
            return view('category/_table', $data);
        }
    ...
    public function store()
    {
        ...
        $createRules = CategoryRules::createRules();
        ...
        if (!$this->validate($createRules, CategoryRules::messages())) {
        ...
        return redirect()->to($this->contextUrl('category', $navigationContext))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CategoryRules::updateRules($id), CategoryRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to($this->contextUrl('category', $navigationContext))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        ...
        try {
            ...
            return redirect()->to($this->contextUrl('category', $navigationContext))->with('error', $e->getMessage());
        }
        return redirect()->to($this->contextUrl('category', $navigationContext))->with('message', 'Record eliminato correttamente.');
    }
    /**
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'category_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
), array (
  ...
  0 => 'last_update',
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = CategoryRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('category') . ($query === [] ? '' : '?' . http_build_query($query));
