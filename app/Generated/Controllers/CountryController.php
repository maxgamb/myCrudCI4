...
use App\Validation\CountryRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per country.
 *
...
final class CountryController extends BaseController
{
    ...
    private CountryService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new CountryService();
  ...
  0 => 'country_id',
        ...
        if ($this->request->isAJAX()) {
            return view('country/_table', $data);
        }
    ...
    public function store()
    {
        ...
        $createRules = CountryRules::createRules();
        ...
        if (!$this->validate($createRules, CountryRules::messages())) {
        ...
        return redirect()->to($this->contextUrl('country', $navigationContext))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CountryRules::updateRules($id), CountryRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to($this->contextUrl('country', $navigationContext))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        ...
        try {
            ...
            return redirect()->to($this->contextUrl('country', $navigationContext))->with('error', $e->getMessage());
        }
        return redirect()->to($this->contextUrl('country', $navigationContext))->with('message', 'Record eliminato correttamente.');
    }
    /**
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'country_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
), array (
  ...
  0 => 'last_update',
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = CountryRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('country') . ($query === [] ? '' : '?' . http_build_query($query));
