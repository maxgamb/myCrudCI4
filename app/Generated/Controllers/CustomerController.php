...
/**
 * Controller CRUD full per customer.
 *
...
final class CustomerController extends BaseController
{
  ...
  array (
    ...
    private CustomerService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new CustomerService();
  ...
  3 => 'address_id',
        ...
        if ($this->request->isAJAX()) {
            return view('customer/_table', $data);
        }
  ...
  1 => 'address_id',
    ...
    public function store()
    {
        ...
        $createRules = CustomerRules::createRules();
        ...
        if (!$this->validate($createRules, CustomerRules::messages())) {
        ...
        return redirect()->to($this->contextUrl('customer', $navigationContext))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CustomerRules::updateRules($id), CustomerRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to($this->contextUrl('customer', $navigationContext))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        ...
        try {
            ...
            return redirect()->to($this->contextUrl('customer', $navigationContext))->with('error', $e->getMessage());
        }
        return redirect()->to($this->contextUrl('customer', $navigationContext))->with('message', 'Record eliminato correttamente.');
    }
    /**
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'customer_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
), array (
  ...
  0 => 'last_update',
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = CustomerRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('customer') . ($query === [] ? '' : '?' . http_build_query($query));
