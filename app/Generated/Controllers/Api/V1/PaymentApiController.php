...
use App\Validation\PaymentApiRules;
use RuntimeException;
use Throwable;

/** API REST v1 per la risorsa payment. */
final class PaymentApiController extends BaseApiController
{
    ...
    public function index()
    {
        try {
            ...
            $result = $this->service->apiList($query, PaymentResource::filterableFields(), PaymentResource::sortableFields());
            return $this->success(PaymentResource::collection($result['rows']), $result['meta'], $result['links']);
        } catch (Throwable $e) {
    ...
    {
        try {
            return $this->success(PaymentResource::make($this->service->find($id)));
    ...
    {
        $data = PaymentResource::writableData($this->payload());
        if ($data === []) {
            return $this->error('EMPTY_PAYLOAD', 'Nessun campo scrivibile ricevuto.', 422);
        }
        if (!$this->validateData($data, PaymentApiRules::createRules(), PaymentApiRules::messages())) {
            return $this->error('VALIDATION_ERROR', 'Dati non validi.', 422, $this->validator->getErrors());
        }
        try {
            $id = $this->service->create($data);
            return $this->success(['payment_id' => $id], [], [], 201);
    ...
    private function writeUpdate(int|string $id, bool $partial)
    {
        ...
        $rules = PaymentApiRules::updateRules($id);
        if ($partial) {
            $rules = array_intersect_key($rules, $data);
        }
        if ($rules !== [] && !$this->validateData($data, $rules, PaymentApiRules::messages())) {
            return $this->error('VALIDATION_ERROR', 'Dati non validi.', 422, $this->validator->getErrors());
        }
        try {
