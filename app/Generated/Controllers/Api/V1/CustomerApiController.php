...
use App\Validation\CustomerApiRules;
use RuntimeException;
use Throwable;

/** API REST v1 per la risorsa customer. */
final class CustomerApiController extends BaseApiController
{
    ...
    public function index()
    {
        try {
            ...
            $result = $this->service->apiList($query, CustomerResource::filterableFields(), CustomerResource::sortableFields());
            return $this->success(CustomerResource::collection($result['rows']), $result['meta'], $result['links']);
        } catch (Throwable $e) {
    ...
    {
        try {
            return $this->success(CustomerResource::make($this->service->find($id)));
    ...
    {
        $data = CustomerResource::writableData($this->payload());
        if ($data === []) {
            return $this->error('EMPTY_PAYLOAD', 'Nessun campo scrivibile ricevuto.', 422);
        }
        if (!$this->validateData($data, CustomerApiRules::createRules(), CustomerApiRules::messages())) {
            return $this->error('VALIDATION_ERROR', 'Dati non validi.', 422, $this->validator->getErrors());
        }
        try {
            $id = $this->service->create($data);
            return $this->success(['customer_id' => $id], [], [], 201);
    ...
    private function writeUpdate(int|string $id, bool $partial)
    {
        ...
        $rules = CustomerApiRules::updateRules($id);
        if ($partial) {
            $rules = array_intersect_key($rules, $data);
        }
        if ($rules !== [] && !$this->validateData($data, $rules, CustomerApiRules::messages())) {
            return $this->error('VALIDATION_ERROR', 'Dati non validi.', 422, $this->validator->getErrors());
        }
        try {
