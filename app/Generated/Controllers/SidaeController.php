...
use App\Services\SidaeService;
use App\Validation\SidaeRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per sidae.
 *
 ...
 */
final class SidaeController extends BaseController
{
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('sidae/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('sidae/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(SidaeRules::createRules(), SidaeRules::messages())) {
        ...
        return redirect()->to(site_url('sidae'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('sidae/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(SidaeRules::updateRules($id), SidaeRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('sidae'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('sidae'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('sidae'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'sidae_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('sidae') . '?' . http_build_query((array) $this->request->getGet()))
