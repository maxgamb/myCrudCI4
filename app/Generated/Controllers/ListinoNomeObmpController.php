...
use App\Services\ListinoNomeObmpService;
use App\Validation\ListinoNomeObmpRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per listino_nome_obmp.
 *
 ...
 */
final class ListinoNomeObmpController extends BaseController
{
    ...
    private ListinoNomeObmpService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ListinoNomeObmpService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('listino_nome_obmp/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('listino_nome_obmp/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('listino_nome_obmp/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('listino_nome_obmp/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ListinoNomeObmpRules::createRules(), ListinoNomeObmpRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['listino_nome_id']);
        ...
        return redirect()->to(site_url('listino_nome_obmp'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('listino_nome_obmp/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ListinoNomeObmpRules::updateRules($id), ListinoNomeObmpRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('listino_nome_obmp'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('listino_nome_obmp'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('listino_nome_obmp'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'listino_nome_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'listino_nome_obmp',
                languageGroup: 'ListinoNomeObmp',
    ...
    {
        return redirect()->to(site_url('listino_nome_obmp') . '?' . http_build_query((array) $this->request->getGet()))
