...
use App\Validation\BlackListRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per black_list.
 *
 ...
 */
final class BlackListController extends BaseController
{
    ...
    private BlackListService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new BlackListService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('black_list/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('black_list/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(BlackListRules::createRules(), BlackListRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['black_list_id']);
        ...
        return redirect()->to(site_url('black_list'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('black_list/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(BlackListRules::updateRules($id), BlackListRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('black_list'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('black_list'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('black_list'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'black_list_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('black_list') . '?' . http_build_query((array) $this->request->getGet()))
