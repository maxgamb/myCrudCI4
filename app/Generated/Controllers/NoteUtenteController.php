...
use App\Services\NoteUtenteService;
use App\Validation\NoteUtenteRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per note_utente.
 *
 ...
 */
final class NoteUtenteController extends BaseController
{
    ...
    private NoteUtenteService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new NoteUtenteService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('note_utente/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('note_utente/index', $data);
    }
    ...
    {
        return view('note_utente/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(NoteUtenteRules::createRules(), NoteUtenteRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['note_utente_id']);
        ...
        return redirect()->to(site_url('note_utente'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('note_utente/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(NoteUtenteRules::updateRules($id), NoteUtenteRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('note_utente'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('note_utente'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('note_utente'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'note_utente_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'note_utente',
                languageGroup: 'NoteUtente',
    ...
    {
        return redirect()->to(site_url('note_utente') . '?' . http_build_query((array) $this->request->getGet()))
