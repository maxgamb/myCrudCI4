...
use App\Services\ManutenzioniService;
use App\Validation\ManutenzioniRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per manutenzioni.
 *
 ...
 */
final class ManutenzioniController extends BaseController
{
    ...
    private ManutenzioniService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ManutenzioniService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('manutenzioni/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('manutenzioni/index', $data);
    }
    ...
    {
        return view('manutenzioni/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ManutenzioniRules::createRules(), ManutenzioniRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['manutenzione_id']);
        ...
        return redirect()->to(site_url('manutenzioni'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('manutenzioni/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ManutenzioniRules::updateRules($id), ManutenzioniRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('manutenzioni'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('manutenzioni'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('manutenzioni'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'manutenzione_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'manutenzioni',
                languageGroup: 'Manutenzioni',
    ...
    {
        return redirect()->to(site_url('manutenzioni') . '?' . http_build_query((array) $this->request->getGet()))
