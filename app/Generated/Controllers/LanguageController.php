...
/**
 * Controller CRUD full per language.
 *
...
final class LanguageController extends BaseController
{
    ...
    private LanguageService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new LanguageService();
  ...
  0 => 'language_id',
        ...
        if ($this->request->isAJAX()) {
            return view('language/_table', $data);
        }
    ...
    public function store()
    {
        ...
        $createRules = LanguageRules::createRules();
        ...
        if (!$this->validate($createRules, LanguageRules::messages())) {
        ...
        return redirect()->to($this->contextUrl('language', $navigationContext))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(LanguageRules::updateRules($id), LanguageRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to($this->contextUrl('language', $navigationContext))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        ...
        try {
            ...
            return redirect()->to($this->contextUrl('language', $navigationContext))->with('error', $e->getMessage());
        }
        return redirect()->to($this->contextUrl('language', $navigationContext))->with('message', 'Record eliminato correttamente.');
    }
    /**
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'language_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
), array (
  ...
  0 => 'last_update',
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = LanguageRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('language') . ($query === [] ? '' : '?' . http_build_query($query));
