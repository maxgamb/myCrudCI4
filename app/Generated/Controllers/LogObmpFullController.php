...
use App\Validation\LogObmpFullRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per log_obmp_full.
 *
 ...
 */
final class LogObmpFullController extends BaseController
{
    ...
    private LogObmpFullService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new LogObmpFullService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('log_obmp_full/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('log_obmp_full/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('log_obmp_full/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('log_obmp_full/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(LogObmpFullRules::createRules(), LogObmpFullRules::messages())) {
        ...
        return redirect()->to(site_url('log_obmp_full'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('log_obmp_full/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(LogObmpFullRules::updateRules($id), LogObmpFullRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('log_obmp_full'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('log_obmp_full'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('log_obmp_full'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'log_obmp_id_full', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'log_obmp_full',
                languageGroup: 'LogObmpFull',
    ...
    {
        return redirect()->to(site_url('log_obmp_full') . '?' . http_build_query((array) $this->request->getGet()))
