...
/**
 * Controller CRUD full per staff.
 *
...
final class StaffController extends BaseController
{
  ...
  0 => 'store_id',
    ...
    public function store()
    {
        ...
        $createRules = StaffRules::createRules();
        ...
        if (!$this->validate($createRules, StaffRules::messages())) {
        ...
        return redirect()->to($this->contextUrl('staff', $navigationContext))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(StaffRules::updateRules($id), StaffRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to($this->contextUrl('staff', $navigationContext))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        ...
        try {
            ...
            return redirect()->to($this->contextUrl('staff', $navigationContext))->with('error', $e->getMessage());
        }
        return redirect()->to($this->contextUrl('staff', $navigationContext))->with('message', 'Record eliminato correttamente.');
    }
    /**
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'staff_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
), array (
  ...
  0 => 'last_update',
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = StaffRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('staff') . ($query === [] ? '' : '?' . http_build_query($query));
