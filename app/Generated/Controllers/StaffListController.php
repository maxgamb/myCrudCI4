...
use App\Validation\StaffListRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per staff_list.
 *
 ...
 */
final class StaffListController extends BaseController
{
    ...
    private StaffListService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new StaffListService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('staff_list/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('staff_list/index', $data);
    }
  ...
  2 => 100,
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = StaffListRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('staff_list') . ($query === [] ? '' : '?' . http_build_query($query));
