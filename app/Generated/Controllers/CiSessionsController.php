...
use App\Services\CiSessionsService;
use App\Validation\CiSessionsRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per ci_sessions.
 *
 ...
 */
final class CiSessionsController extends BaseController
{
    ...
    private CiSessionsService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new CiSessionsService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('ci_sessions/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('ci_sessions/index', $data);
    }
    ...
    {
        return view('ci_sessions/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(CiSessionsRules::createRules(), CiSessionsRules::messages())) {
        ...
        return redirect()->to(site_url('ci_sessions'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('ci_sessions/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CiSessionsRules::updateRules($id), CiSessionsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('ci_sessions'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('ci_sessions'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('ci_sessions'))->with('message', 'Record eliminato correttamente.');
    }
  ...
  2 => 100,
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'ci_sessions',
                languageGroup: 'CiSessions',
    ...
    {
        return redirect()->to(site_url('ci_sessions') . '?' . http_build_query((array) $this->request->getGet()))
