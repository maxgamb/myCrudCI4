...
use App\Services\ProdottiListaService;
use App\Validation\ProdottiListaRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per prodotti_lista.
 *
 ...
 */
final class ProdottiListaController extends BaseController
{
    ...
    private ProdottiListaService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ProdottiListaService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('prodotti_lista/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('prodotti_lista/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('prodotti_lista/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('prodotti_lista/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ProdottiListaRules::createRules(), ProdottiListaRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['prodotti_lista_id']);
        ...
        return redirect()->to(site_url('prodotti_lista'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('prodotti_lista/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ProdottiListaRules::updateRules($id), ProdottiListaRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('prodotti_lista'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('prodotti_lista'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('prodotti_lista'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'prodotti_lista_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'prodotti_lista',
                languageGroup: 'ProdottiLista',
    ...
    {
        return redirect()->to(site_url('prodotti_lista') . '?' . http_build_query((array) $this->request->getGet()))
