...
use App\Services\ContiTrasferisciService;
use App\Validation\ContiTrasferisciRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per conti_trasferisci.
 *
 ...
 */
final class ContiTrasferisciController extends BaseController
{
    ...
    private ContiTrasferisciService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ContiTrasferisciService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('conti_trasferisci/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('conti_trasferisci/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('conti_trasferisci/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('conti_trasferisci/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ContiTrasferisciRules::createRules(), ContiTrasferisciRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['conti_trasferisci_id']);
        ...
        return redirect()->to(site_url('conti_trasferisci'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('conti_trasferisci/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ContiTrasferisciRules::updateRules($id), ContiTrasferisciRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('conti_trasferisci'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('conti_trasferisci'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('conti_trasferisci'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'conti_trasferisci_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'conti_trasferisci',
                languageGroup: 'ContiTrasferisci',
    ...
    {
        return redirect()->to(site_url('conti_trasferisci') . '?' . http_build_query((array) $this->request->getGet()))
