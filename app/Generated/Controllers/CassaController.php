...
use App\Services\CassaService;
use App\Validation\CassaRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per cassa.
 *
 ...
 */
final class CassaController extends BaseController
{
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('cassa/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('cassa/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(CassaRules::createRules(), CassaRules::messages())) {
        ...
        return redirect()->to(site_url('cassa'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('cassa/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CassaRules::updateRules($id), CassaRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('cassa'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('cassa'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('cassa'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'cassa_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('cassa') . '?' . http_build_query((array) $this->request->getGet()))
