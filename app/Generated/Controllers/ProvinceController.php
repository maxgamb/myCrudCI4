...
use App\Validation\ProvinceRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per province.
 *
 ...
 */
final class ProvinceController extends BaseController
{
    ...
    private ProvinceService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ProvinceService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('province/_table', $data);
        }
    ...
    public function store()
    {
        ...
        if (!$this->validate(ProvinceRules::createRules(), ProvinceRules::messages())) {
        ...
        return redirect()->to(site_url('province'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ProvinceRules::updateRules($id), ProvinceRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('province'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('province'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('province'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'Prov', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('province') . '?' . http_build_query((array) $this->request->getGet()))
