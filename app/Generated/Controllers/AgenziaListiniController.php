...
use App\Services\AgenziaListiniService;
use App\Validation\AgenziaListiniRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per agenzia_listini.
 *
 ...
 */
final class AgenziaListiniController extends BaseController
{
    ...
    private AgenziaListiniService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new AgenziaListiniService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('agenzia_listini/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('agenzia_listini/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('agenzia_listini/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('agenzia_listini/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(AgenziaListiniRules::createRules(), AgenziaListiniRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['agenzia_listini_id']);
        ...
        return redirect()->to(site_url('agenzia_listini'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('agenzia_listini/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(AgenziaListiniRules::updateRules($id), AgenziaListiniRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('agenzia_listini'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('agenzia_listini'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('agenzia_listini'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'agenzia_listini_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'agenzia_listini',
                languageGroup: 'AgenziaListini',
    ...
    {
        return redirect()->to(site_url('agenzia_listini') . '?' . http_build_query((array) $this->request->getGet()))
