...
/**
 * CRUD full controller for resource `payment`.
 *
...
final class PaymentController extends BaseController
{
  ...
  3 => 'rental_id',
        ...
        if ($this->request->isAJAX()) {
            return view('payment/_table', $data);
        }
  ...
  2 => 'rental_id',
    ...
    public function store()
    {
        ...
        $createRules = PaymentRules::createRules();
        ...
        }        if (!$this->validate($createRules, PaymentRules::messages())) {
        ...
        $redirectUrl = $parentContext['url'] ?? $this->contextUrl('payment', $navigationContext, $cascadeTrail);
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(PaymentRules::updateRules($id), PaymentRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to($this->contextUrl('payment', $navigationContext, $cascadeTrail))->with('message', 'Record updated successfully.');
    }
    ...
    public function delete(int|string $id)
    {
        ...
        try {
            ...
            return redirect()->to($this->contextUrl('payment', $navigationContext))->with('error', $e->getMessage());
        }
        return redirect()->to($this->contextUrl('payment', $navigationContext))->with('message', 'Record deleted successfully.');
    }
    /**
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'payment_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
), array (
  ...
  0 => 'rental_id',
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = PaymentRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('payment') . ($query === [] ? '' : '?' . http_build_query($query));
