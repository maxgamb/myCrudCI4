...
use App\Validation\CustomerListRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per customer_list.
 *
 ...
 */
final class CustomerListController extends BaseController
{
    ...
    private CustomerListService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new CustomerListService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('customer_list/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('customer_list/index', $data);
    }
  ...
  2 => 100,
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = CustomerListRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('customer_list') . ($query === [] ? '' : '?' . http_build_query($query));
