...
use App\Models\CostiAreaModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class CostiAreaService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'costi_area_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly CostiAreaModel $model = new CostiAreaModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new CostiAreaEntity($data), true);
        if ($id === false) {
