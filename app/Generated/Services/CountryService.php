...
use App\Services\Extensions\CountryServiceExtension;
use App\Models\CountryModel;
use App\Validation\CountryRules;
use RuntimeException;

/**
 * Service applicativo per la risorsa `country`.
 *
 * Responsabilità:
 ...
 *
 * Queries remain the responsibility of CountryModel.
 */
final class CountryService
{
    use CountryServiceExtension;

    private const DATABASE_MANAGED_FIELDS = array (
  0 => 'last_update',
);

    public function __construct(private readonly CountryModel $model = new CountryModel())
    {
    }
    ...
    {
        $this->validatePayload($data, CountryRules::createRules(), CountryRules::messages(), 'Create validation failed.');
    }
    /** @param array<string,mixed> $data */
    private function validateUpdatePayload(int|string $id, array $data): void
    {
        $this->validatePayload($data, CountryRules::updateRules($id), CountryRules::messages(), 'Update validation failed.');
    }
    /**
    ...
    {
        $data = $this->prepareData($data);
        $rules = array_intersect_key(CountryRules::updateRules($id), $data);
        if ($rules !== []) {
            $this->validatePayload($data, $rules, CountryRules::messages(), 'Patch validation failed.');
        }
        $data = $this->beforeUpdate($id, $data);
        if (!$this->model->updateRecord($id, $data)) {
