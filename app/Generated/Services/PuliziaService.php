...
use App\Models\PuliziaModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class PuliziaService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'pulizia_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly PuliziaModel $model = new PuliziaModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new PuliziaEntity($data), true);
        if ($id === false) {
