...
use App\Models\SospesiModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class SospesiService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'sospeso_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly SospesiModel $model = new SospesiModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new SospesiEntity($data), true);
        if ($id === false) {
