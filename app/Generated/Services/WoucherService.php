...
use App\Models\WoucherModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class WoucherService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'woucher_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly WoucherModel $model = new WoucherModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new WoucherEntity($data), true);
        if ($id === false) {
