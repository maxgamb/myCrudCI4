...
use App\Models\ChecklistPrenoModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class ChecklistPrenoService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly ChecklistPrenoModel $model = new ChecklistPrenoModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ChecklistPrenoEntity($data), true);
        if ($id === false) {
