...
use App\Models\TaxPagamentoModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class TaxPagamentoService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'tax_pagamento_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly TaxPagamentoModel $model = new TaxPagamentoModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new TaxPagamentoEntity($data), true);
        if ($id === false) {
