...
use App\Models\ObmpClientiModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class ObmpClientiService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'obm_cliente_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly ObmpClientiModel $model = new ObmpClientiModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ObmpClientiEntity($data), true);
        if ($id === false) {
