...
use App\Models\GuastiModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class GuastiService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'guasto_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly GuastiModel $model = new GuastiModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new GuastiEntity($data), true);
        if ($id === false) {
