...
use App\Models\ColoriModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class ColoriService
{
    private const PASSWORD_FIELDS = array (
);
    private const AUTOMATIC_DATE_FIELDS = array (
  'colore_data_record' => 'Y-m-d H:i:s',
);

    public function __construct(private readonly ColoriModel $model = new ColoriModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new ColoriEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('colore_nome', $data) && (is_int($data['colore_nome']) || is_string($data['colore_nome']))) {
            return $data['colore_nome'];
        }
