...
final class InventoryService
{
  ...
  0 => 'last_update',
    ...
    {
        $this->validatePayload($data, InventoryRules::createRules(), InventoryRules::messages(), 'Create validation failed.');
    }
    /** @param array<string,mixed> $data */
    private function validateUpdatePayload(int|string $id, array $data): void
    {
        $this->validatePayload($data, InventoryRules::updateRules($id), InventoryRules::messages(), 'Update validation failed.');
    }
    /**
    ...
    {
        $data = $this->prepareData($data);
        $rules = array_intersect_key(InventoryRules::updateRules($id), $data);
        if ($rules !== []) {
            $this->validatePayload($data, $rules, InventoryRules::messages(), 'Patch validation failed.');
        }
        $data = $this->beforeUpdate($id, $data);
        if (!$this->model->updateRecord($id, $data)) {
