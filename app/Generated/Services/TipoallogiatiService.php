...
use App\Models\TipoallogiatiModel;
use RuntimeException;

/** Coordina la logica applicativa senza comporre query SQL. */
final class TipoallogiatiService
{
    ...
    public function __construct(private readonly TipoallogiatiModel $model = new TipoallogiatiModel())
    {
    }
    ...
    public function create(array $data): int|string
    {
        ...
        $id = $this->model->insert(new TipoallogiatiEntity($data), true);
        if ($id === false) {
            throw new RuntimeException(implode(' ', $this->model->errors()) ?: 'Inserimento non riuscito.');
        }
        $this->model->clearListCountCache();
        if (array_key_exists('Tip_all_Cod', $data) && (is_int($data['Tip_all_Cod']) || is_string($data['Tip_all_Cod']))) {
            return $data['Tip_all_Cod'];
        }
