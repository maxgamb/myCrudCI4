...
/**
 * Write Service for `film_category`.
 *
 * The table exposes Create but no record-level Update/Delete identity.
 * Read/query operations remain in FilmCategoryModel; this Service owns write preparation,
 * validation and orchestration only.
 */
final class FilmCategoryService
{
    private const DATABASE_MANAGED_FIELDS = array (
  0 => 'last_update',
);

    public function __construct(private readonly FilmCategoryModel $model = new FilmCategoryModel())
    {
    }
    ...
    /**
     * Creates the parent resource for relation category_id.
     *
     * Delegates the write to CategoryService; this Service only orchestrates the FK assignment.
     *
     * @param array<string,mixed> $payload Parent resource payload.
     * @return int|string Created parent identifier.
     */
    private function createCategoryForCategoryId(array $payload): int|string
    {
    ...
    {
        $this->validatePayload($data, FilmCategoryRules::createRules(), FilmCategoryRules::messages(), 'Create validation failed.');
    }
    /**
    ...
    public function create(
        ...
        try {
            ...
            if (isset($related['category_id']) && is_array($related['category_id'])) {
                $data['category_id'] = $this->createCategoryForCategoryId($related['category_id']);
            }
