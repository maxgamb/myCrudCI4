...
final class FilmCategoryApiArchitectureContractTest extends CIUnitTestCase
{
    public function testApiControllerUsesModelForReadsAndServiceForWritesWithoutSql(): void
    {
        $path = APPPATH . 'Controllers/Api/V1/FilmCategoryApiController.php';
        $this->assertFileExists($path);
        $php = (string) file_get_contents($path);

        $this->assertStringContainsString('private readonly FilmCategoryModel $model', $php);
        $this->assertStringContainsString('private readonly FilmCategoryService $service', $php);
    ...
    {
        $resource = (string) file_get_contents(APPPATH . 'API/Resources/FilmCategoryResource.php');
