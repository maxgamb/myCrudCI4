                        ...
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                <td>
                                    ...
                                    <?php if ((string) ($row->{'inventory_id'} ?? '') !== ''): ?><?php $parentTrailEncoded = \App\Libraries\Crud\CrudNavigationTrail::encode((array) ($cascadeTrail ?? [])); $parentHref = site_url('inventory/view/' . rawurlencode((string) $row->{'inventory_id'})); if ($parentTrailEncoded !== '') $parentHref .= '?_trail=' . rawurlencode($parentTrailEncoded); ?><a href="<?= esc($parentHref) ?>" class="text-decoration-none"><?= esc($row->{'inventory_id__label'} ?? $row->{'inventory_id'} ?? '') ?></a><?php else: ?><?= esc($row->{'inventory_id__label'} ?? '') ?><?php endif; ?>
                                    <?php
                                    $quickQuery = (array) ($query ?? []);
                                ...
                                <td class="text-end text-nowrap">
                                    ...
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Record actions">
                                        ...
                                        <a href="<?= site_url('rental/edit/' . rawurlencode((string) $id)) . ($navigationQuery ?? '') ?>" class="btn btn-outline-warning" title="Edit">
                                            <i class="bi bi-pencil-square" aria-hidden="true"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('rental/delete/' . rawurlencode((string) $id)) . ($navigationQuery ?? '') ?>" class="d-inline" onsubmit="return confirm('Delete this record?')">
                                            <?= csrf_field() ?>
