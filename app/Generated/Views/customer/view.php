...
<div class="container py-4">
    <div class="d-print-none">
        ...
        </a>        <a href="<?= site_url('customer') . ($navigationQuery ?? '') ?>" class="btn btn-outline-secondary" title="Torna alla lista">
            <i class="bi bi-list-ul me-1" aria-hidden="true"></i> Lista
        </a>
        <a href="<?= site_url('customer/edit/' . rawurlencode((string) ($row->{'customer_id'} ?? ''))) . ($navigationQuery ?? '') ?>" class="btn btn-outline-warning" title="Modifica record">
            <i class="bi bi-pencil-square me-1" aria-hidden="true"></i> Modifica
        </a>
        <form method="post" action="<?= site_url('customer/delete/' . rawurlencode((string) ($row->{'customer_id'} ?? ''))) . ($navigationQuery ?? '') ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
            <?= csrf_field() ?>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-sm table-bordered table-striped align-middle">
                        <tbody>
                        <tr>
                            <th class="w-25"><?= esc(lang('Customer.customer_id')) ?></th>
                            <td><?= esc($row->{'customer_id'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Customer.store_id')) ?></th>
                            ...
                            <th class="w-25"><?= esc(lang('Customer.first_name')) ?></th>
                            <td><?= esc($row->{'first_name'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Customer.last_name')) ?></th>
                            <td><?= esc($row->{'last_name'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Customer.email')) ?></th>
                            <td><?= esc($row->{'email'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Customer.address_id')) ?></th>
                            ...
                            <th class="w-25"><?= esc(lang('Customer.active')) ?></th>
                            <td><?= esc($row->{'active'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Customer.create_date')) ?></th>
                            <td><?= esc($row->{'create_date'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Customer.last_update')) ?></th>
...
<div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between align-items-center gap-2">
            ...
            <div class="d-flex align-items-center gap-2 d-print-none">
                <a href="<?= site_url('payment/create') . '?' . http_build_query(['customer_id' => $row->{'customer_id'} ?? '']) ?>" class="btn btn-sm btn-primary" title="Nuovo record collegato"><i class="bi bi-plus-circle me-1" aria-hidden="true"></i> Nuovo</a>
                <span class="badge bg-secondary"><?= (int) ($children['payment__customer_id']['count'] ?? 0) ?><?= !empty($children['payment__customer_id']['hasMore']) ? '+' : '' ?></span>
            </div>
        </div>
        <div class="card-body">
            <?php $relatedRows = $children['payment__customer_id']['rows'] ?? []; ?>
                        ...
                        <tbody>
                            <?php foreach ($relatedRows as $child): ?>
                                ...
                                <td><?= esc($child->{'last_update'} ?? '') ?></td>
                                    <td class="d-print-none"><a href="<?= site_url('payment/view/' . rawurlencode((string) ($child->{'payment_id'} ?? ''))) . '?' . http_build_query(['customer_id' => $row->{'customer_id'} ?? '']) ?>" class="btn btn-sm btn-outline-info" title="Visualizza record"><i class="bi bi-eye" aria-hidden="true"></i></a></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>
                <?php if (!empty($children['payment__customer_id']['hasMore'])): ?>
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 d-print-none">
                        ...
                        <a href="<?= site_url('payment') . '?' . http_build_query(['customer_id' => $row->{'customer_id'} ?? '']) ?>" class="btn btn-sm btn-outline-primary">
...
<div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between align-items-center gap-2">
            ...
            <div class="d-flex align-items-center gap-2 d-print-none">
                <a href="<?= site_url('rental/create') . '?' . http_build_query(['customer_id' => $row->{'customer_id'} ?? '']) ?>" class="btn btn-sm btn-primary" title="Nuovo record collegato"><i class="bi bi-plus-circle me-1" aria-hidden="true"></i> Nuovo</a>
                <span class="badge bg-secondary"><?= (int) ($children['rental__customer_id']['count'] ?? 0) ?><?= !empty($children['rental__customer_id']['hasMore']) ? '+' : '' ?></span>
            </div>
        </div>
        <div class="card-body">
            <?php $relatedRows = $children['rental__customer_id']['rows'] ?? []; ?>
            <?php if (empty($relatedRows)): ?>
                <div class="alert alert-light border mb-0">Nessun record collegato.</div>
            <?php else: ?>
                <div class="table-responsive">
                    <table class="table table-sm table-striped table-hover align-middle">
                        ...
                        <tbody>
                            <?php foreach ($relatedRows as $child): ?>
                                ...
                                <td><?= esc($child->{'staff_id'} ?? '') ?></td>
                                <td><?= esc($child->{'last_update'} ?? '') ?></td>
                                    <td class="d-print-none"><a href="<?= site_url('rental/view/' . rawurlencode((string) ($child->{'rental_id'} ?? ''))) . '?' . http_build_query(['customer_id' => $row->{'customer_id'} ?? '']) ?>" class="btn btn-sm btn-outline-info" title="Visualizza record"><i class="bi bi-eye" aria-hidden="true"></i></a></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>
                <?php if (!empty($children['rental__customer_id']['hasMore'])): ?>
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 d-print-none">
                        <div class="small text-muted">Visualizzati i primi 20 record.</div>
                        <a href="<?= site_url('rental') . '?' . http_build_query(['customer_id' => $row->{'customer_id'} ?? '']) ?>" class="btn btn-sm btn-outline-primary">
                            Vedi tutti <i class="bi bi-arrow-right"></i>
                        </a>
                    </div>
                <?php endif; ?>
