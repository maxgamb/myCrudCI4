...
<div class="container py-4">
    <div class="d-print-none">
        ...
        <a href="<?= site_url('customer/create') . ($navigationQuery ?? '') ?>" class="btn btn-primary" title="New record">
            <i class="bi bi-plus-circle me-1" aria-hidden="true"></i> New
        </a>        <a href="<?= site_url('customer') . ($navigationQuery ?? '') ?>" class="btn btn-outline-secondary" title="Back to list">
            <i class="bi bi-list-ul me-1" aria-hidden="true"></i> List
        </a>
        <a href="<?= site_url('customer/edit/' . rawurlencode((string) ($row->{'customer_id'} ?? ''))) . ($navigationQuery ?? '') ?>" class="btn btn-outline-warning" title="Edit record">
            <i class="bi bi-pencil-square me-1" aria-hidden="true"></i> Edit
        </a>
        <form method="post" action="<?= site_url('customer/delete/' . rawurlencode((string) ($row->{'customer_id'} ?? ''))) . ($navigationQuery ?? '') ?>" class="d-inline" onsubmit="return confirm('Delete this record?')">
            <?= csrf_field() ?>
...
if ($parentTrail === (array) ($cascadeTrail ?? [])) {
    $parentTrail = \App\Libraries\Crud\CrudNavigationTrail::append($parentTrail, "customer", (string) ($row->{'customer_id'} ?? ''), trim((string) ($row->{'first_name'} ?? '') . ' ' . (string) ($row->{'last_name'} ?? '')) ?: 'Customer' . ' #' . (string) ($row->{'customer_id'} ?? ''));
}
...
<?= view('customer/_children_payment__customer_id', ['row' => $row, 'children' => $children, 'cascadeTrail' => $cascadeTrail ?? []]) ?>
<?= view('customer/_children_rental__customer_id', ['row' => $row, 'children' => $children, 'cascadeTrail' => $cascadeTrail ?? []]) ?>
