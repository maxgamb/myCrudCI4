...
<div class="container-fluid px-0">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            <a id="createRecordButton" data-base-url="<?= site_url('customer/create') ?>" href="<?= site_url('customer/create') . ($navigationQuery ?? '') ?>" class="btn btn-primary" title="Nuovo record">
                <i class="bi bi-plus-circle me-1" aria-hidden="true"></i> Nuovo
            </a>            <a id="exportCsvButton" href="<?= site_url('customer/export-csv') . $navigationQuery ?>" class="btn btn-outline-success" title="Esporta i risultati filtrati in CSV">
                <i class="bi bi-filetype-csv me-1" aria-hidden="true"></i> CSV
            </a>
            <a id="exportWordButton" href="<?= site_url('customer/export-word') . $navigationQuery ?>" class="btn btn-outline-primary" title="Esporta i risultati filtrati in Word">
    ...
    <details class="mb-3" <?= $hasActiveFilters ? 'open' : '' ?>>
        <summary class="fw-semibold"><?= esc(lang('Customer.filtersSummary')) ?></summary>
        <div class="card card-body mt-2">
            <?= view('customer/_filters', [
                'filters' => $filters ?? [],
                'options' => $options ?? [],
                'perPage' => $perPage ?? 25,
                'sort' => $sort ?? 'customer_id',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const navigationContextFields = ["store_id","address_id"];
    const simpleFilterFields = ["customer_id","store_id","last_name","address_id"];
    ...
    const updateActionUrls = (source = window.location.href) => {
        ...
        const csvUrl = new URL("<?= site_url('customer/export-csv') ?>", window.location.origin);
        const wordUrl = new URL("<?= site_url('customer/export-word') ?>", window.location.origin);
