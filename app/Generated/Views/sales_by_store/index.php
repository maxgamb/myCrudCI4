...
<div class="container-fluid px-0">
    <nav aria-label="breadcrumb" class="mb-2">
        <ol class="breadcrumb mb-0">
            ...
            <?php endforeach; ?>
            <li class="breadcrumb-item active" aria-current="page">sales_by_store</li>
        </ol>
    </nav>

    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        <div>
            <h1 class="h3 mb-0">sales_by_store</h1>
            <small class="text-muted">Elenco</small>
        </div>
        <div class="d-flex flex-wrap gap-2">
            <a id="exportCsvButton" href="<?= site_url('sales_by_store/export-csv') . $navigationQuery ?>" class="btn btn-outline-success" title="Esporta i risultati filtrati in CSV">
                <i class="bi bi-filetype-csv me-1" aria-hidden="true"></i> CSV
            </a>
            <a id="exportWordButton" href="<?= site_url('sales_by_store/export-word') . $navigationQuery ?>" class="btn btn-outline-primary" title="Esporta i risultati filtrati in Word">
    ...
    <details class="mb-3" <?= $hasActiveFilters ? 'open' : '' ?>>
        <summary class="fw-semibold"><?= esc(lang('SalesByStore.filtersSummary')) ?></summary>
        <div class="card card-body mt-2">
            <?= view('sales_by_store/_filters', [
                'filters' => $filters ?? [],
                'options' => $options ?? [],
                'perPage' => $perPage ?? 25,
                'sort' => $sort ?? 'store',
    ...
    <div id="crudTableContainer" aria-live="polite" aria-busy="false">
        <?= view('sales_by_store/_table', [
            'rows' => $rows ?? [],
            'total' => $total ?? 0,
            'page' => $page ?? 1,
            'perPage' => $perPage ?? 25,
            'pagerLinks' => $pagerLinks ?? '',
            'primaryKey' => $primaryKey ?? 'store',
            'sort' => $sort ?? 'store',
            'direction' => $direction ?? 'desc',
            'query' => $query ?? [],
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateActionUrls = (source = window.location.href) => {
        ...
        const csvUrl = new URL("<?= site_url('sales_by_store/export-csv') ?>", window.location.origin);
        const wordUrl = new URL("<?= site_url('sales_by_store/export-word') ?>", window.location.origin);
