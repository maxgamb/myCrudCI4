    ...
    <div class="d-print-none">
        <nav aria-label="breadcrumb" class="mb-2">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item"><a href="<?= site_url('staff') . $navigationQuery ?>">staff</a></li>
        ...
        <a href="<?= site_url('staff/create') . ($navigationQuery ?? '') ?>" class="btn btn-primary" title="Nuovo record">
            <i class="bi bi-plus-circle me-1" aria-hidden="true"></i> Nuovo
        </a>        <a href="<?= site_url('staff') . ($navigationQuery ?? '') ?>" class="btn btn-outline-secondary" title="Torna alla lista">
            <i class="bi bi-list-ul me-1" aria-hidden="true"></i> Lista
        </a>
        <a href="<?= site_url('staff/edit/' . rawurlencode((string) ($row->{'staff_id'} ?? ''))) . ($navigationQuery ?? '') ?>" class="btn btn-outline-warning" title="Modifica record">
            <i class="bi bi-pencil-square me-1" aria-hidden="true"></i> Modifica
        </a>
        <form method="post" action="<?= site_url('staff/delete/' . rawurlencode((string) ($row->{'staff_id'} ?? ''))) . ($navigationQuery ?? '') ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
            <?= csrf_field() ?>
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-bordered table-striped align-middle">
                        ...
                        <tr>
                            ...
                            <th class="w-25"><?= esc(lang('Staff.first_name')) ?></th>
                            <td><?= esc($row->{'first_name'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Staff.last_name')) ?></th>
                            <td><?= esc($row->{'last_name'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Staff.address_id')) ?></th>
                            ...
                            <th class="w-25"><?= esc(lang('Staff.store_id')) ?></th>
                            ...
                            <th class="w-25"><?= esc(lang('Staff.active')) ?></th>
                            <td><?= esc($row->{'active'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Staff.username')) ?></th>
                            <td><?= esc($row->{'username'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Staff.password')) ?></th>
                            <td><?= esc($row->{'password'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Staff.last_update')) ?></th>
...
<div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between align-items-center gap-2">
            ...
            <div class="d-flex align-items-center gap-2 d-print-none">
                <a href="<?= site_url('payment/create') . '?' . http_build_query(['staff_id' => $row->{'staff_id'} ?? '']) ?>" class="btn btn-sm btn-primary" title="Nuovo record collegato"><i class="bi bi-plus-circle me-1" aria-hidden="true"></i> Nuovo</a>
                <span class="badge bg-secondary"><?= (int) ($children['payment__staff_id']['count'] ?? 0) ?><?= !empty($children['payment__staff_id']['hasMore']) ? '+' : '' ?></span>
            </div>
        </div>
        <div class="card-body">
            <?php $relatedRows = $children['payment__staff_id']['rows'] ?? []; ?>
                        ...
                        <tbody>
                            <?php foreach ($relatedRows as $child): ?>
                                ...
                                <td><?= esc($child->{'last_update'} ?? '') ?></td>
                                    <td class="d-print-none"><a href="<?= site_url('payment/view/' . rawurlencode((string) ($child->{'payment_id'} ?? ''))) . '?' . http_build_query(['staff_id' => $row->{'staff_id'} ?? '']) ?>" class="btn btn-sm btn-outline-info" title="Visualizza record"><i class="bi bi-eye" aria-hidden="true"></i></a></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>
                <?php if (!empty($children['payment__staff_id']['hasMore'])): ?>
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 d-print-none">
                        ...
                        <a href="<?= site_url('payment') . '?' . http_build_query(['staff_id' => $row->{'staff_id'} ?? '']) ?>" class="btn btn-sm btn-outline-primary">
...
<div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between align-items-center gap-2">
            ...
            <div class="d-flex align-items-center gap-2 d-print-none">
                <a href="<?= site_url('rental/create') . '?' . http_build_query(['staff_id' => $row->{'staff_id'} ?? '']) ?>" class="btn btn-sm btn-primary" title="Nuovo record collegato"><i class="bi bi-plus-circle me-1" aria-hidden="true"></i> Nuovo</a>
                <span class="badge bg-secondary"><?= (int) ($children['rental__staff_id']['count'] ?? 0) ?><?= !empty($children['rental__staff_id']['hasMore']) ? '+' : '' ?></span>
            </div>
        </div>
        <div class="card-body">
            <?php $relatedRows = $children['rental__staff_id']['rows'] ?? []; ?>
            <?php if (empty($relatedRows)): ?>
                <div class="alert alert-light border mb-0">Nessun record collegato.</div>
            <?php else: ?>
                <div class="table-responsive">
                    <table class="table table-sm table-striped table-hover align-middle">
                        ...
                        <tbody>
                            <?php foreach ($relatedRows as $child): ?>
                                ...
                                <td><?= esc($child->{'staff_id'} ?? '') ?></td>
                                <td><?= esc($child->{'last_update'} ?? '') ?></td>
                                    <td class="d-print-none"><a href="<?= site_url('rental/view/' . rawurlencode((string) ($child->{'rental_id'} ?? ''))) . '?' . http_build_query(['staff_id' => $row->{'staff_id'} ?? '']) ?>" class="btn btn-sm btn-outline-info" title="Visualizza record"><i class="bi bi-eye" aria-hidden="true"></i></a></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>
                <?php if (!empty($children['rental__staff_id']['hasMore'])): ?>
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 d-print-none">
                        <div class="small text-muted">Visualizzati i primi 20 record.</div>
                        <a href="<?= site_url('rental') . '?' . http_build_query(['staff_id' => $row->{'staff_id'} ?? '']) ?>" class="btn btn-sm btn-outline-primary">
                            Vedi tutti <i class="bi bi-arrow-right"></i>
                        </a>
                    </div>
                <?php endif; ?>
...
<div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between align-items-center gap-2">
            ...
            <div class="d-flex align-items-center gap-2 d-print-none">
                <a href="<?= site_url('store/create') . '?' . http_build_query(['manager_staff_id' => $row->{'staff_id'} ?? '']) ?>" class="btn btn-sm btn-primary" title="Nuovo record collegato"><i class="bi bi-plus-circle me-1" aria-hidden="true"></i> Nuovo</a>
        ...
        <div class="card-body">
            ...
            <?php else: ?>
                    ...
                    <table class="table table-sm table-striped table-hover align-middle">
                        ...
                        <tbody>
                            <?php foreach ($relatedRows as $child): ?>
                                ...
                                <td><?= esc($child->{'last_update'} ?? '') ?></td>
                                    <td class="d-print-none"><a href="<?= site_url('store/view/' . rawurlencode((string) ($child->{'store_id'} ?? ''))) . '?' . http_build_query(['manager_staff_id' => $row->{'staff_id'} ?? '']) ?>" class="btn btn-sm btn-outline-info" title="Visualizza record"><i class="bi bi-eye" aria-hidden="true"></i></a></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                ...
                <?php if (!empty($children['store__manager_staff_id']['hasMore'])): ?>
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 d-print-none">
                        <div class="small text-muted">Visualizzati i primi 20 record.</div>
                        <a href="<?= site_url('store') . '?' . http_build_query(['manager_staff_id' => $row->{'staff_id'} ?? '']) ?>" class="btn btn-sm btn-outline-primary">
                            Vedi tutti <i class="bi bi-arrow-right"></i>
                        </a>
                    </div>
                <?php endif; ?>
