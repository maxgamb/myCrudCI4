                ...
                <thead class="table-light">
                    <tr>
                        ...
                        <th>
                            ...
                            >
                                <?= esc(lang('Country.country_id')) ?>
                                <?php if (($sort ?? '') === 'country_id'): ?>
                                    <i class="bi bi-sort-<?= ($direction ?? 'desc') === 'asc' ? 'up' : 'down' ?>"></i>
                                <?php endif; ?>
                            </a>
                        </th>                        <th><?= esc(lang('Country.country')) ?></th>
                        <th><?= esc(lang('Country.last_update')) ?></th>
                            ...
                            <tr>
                                <td>
                                    <?php if ((string) ($row->{'country_id'} ?? '') !== ''): ?>
                                        ...
                                        ><?= esc($row->{'country_id'} ?? '') ?></a>
                                    <?php endif; ?>
                                </td>                                <td><?= esc($row->{'country'} ?? '') ?></td>
                                <td><?= esc($row->{'last_update'} ?? '') ?></td>
                                <td class="text-end text-nowrap">
                                    <?php $id = $row->{'country_id'} ?? ''; ?>
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Record actions">
                                        <a href="<?= site_url('country/view/' . rawurlencode((string) $id)) . ($navigationQuery ?? '') ?>" class="btn btn-outline-info" title="Visualizza">
                                            <i class="bi bi-eye" aria-hidden="true"></i>
                                        </a>
                                        <a href="<?= site_url('country/edit/' . rawurlencode((string) $id)) . ($navigationQuery ?? '') ?>" class="btn btn-outline-warning" title="Edit">
                                            <i class="bi bi-pencil-square" aria-hidden="true"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('country/delete/' . rawurlencode((string) $id)) . ($navigationQuery ?? '') ?>" class="d-inline" onsubmit="return confirm('Delete this record?')">
                                            <?= csrf_field() ?>
