...
<div class="container-fluid px-0">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        <div>
            <h1 class="h3 mb-0">obmp_rates</h1>
            <small class="text-muted">Tabella Bootstrap, Pager CI4 e caricamento AJAX</small>
        </div>
        <div class="d-flex flex-wrap gap-2">
            <a href="<?= site_url('obmp_rates/create') ?>" class="btn btn-primary">
                <i class="bi bi-plus-circle"></i> Nuovo
            </a>
            <a id="exportCsvButton" href="<?= site_url('obmp_rates/export-csv') ?>" class="btn btn-success">
                <i class="bi bi-filetype-csv"></i> Esporta CSV
            </a>
            <a id="exportWordButton" href="<?= site_url('obmp_rates/export-word') ?>" class="btn btn-outline-primary">
    ...
    <details class="mb-3" <?= $hasActiveFilters ? 'open' : '' ?>>
        <summary class="fw-semibold"><?= esc(lang('ObmpRates.filtersSummary')) ?></summary>
        <div class="card card-body mt-2">
            <?= view('obmp_rates/_filters', [
                'filters' => $filters ?? [],
                'options' => $options ?? [],
                'perPage' => $perPage ?? 25,
                'sort' => $sort ?? 'obmp_rate_id',
    ...
    <div id="crudTableContainer" aria-live="polite" aria-busy="false">
        <?= view('obmp_rates/_table', [
            'rows' => $rows ?? [],
            'total' => $total ?? 0,
            'page' => $page ?? 1,
            'perPage' => $perPage ?? 25,
            'pagerLinks' => $pagerLinks ?? '',
            'primaryKey' => $primaryKey ?? 'obmp_rate_id',
            'sort' => $sort ?? 'obmp_rate_id',
            'direction' => $direction ?? 'desc',
            'query' => $query ?? [],
        ]) ?>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateExportUrls = () => {
        ...
        const csvUrl = new URL("<?= site_url('obmp_rates/export-csv') ?>", window.location.origin);
        const wordUrl = new URL("<?= site_url('obmp_rates/export-word') ?>", window.location.origin);
