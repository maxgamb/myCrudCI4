            ...
            <?= form_open($formAction, [
                ...
                <div class="col-md-6">
                    <label for="actor_id" class="form-label">
                        <?= esc(lang('FilmActor.actor_id')) ?>
                    </label>
<div class="input-group crud-relation-input-group">
                    <select
                        name="actor_id"
                        id="actor_id"
                        class="form-select <?= isset($errors['actor_id']) ? 'is-invalid' : '' ?>"
                        aria-describedby="actor_id-error"
                        aria-invalid="<?= isset($errors['actor_id']) ? 'true' : 'false' ?>"
                        required
                    >
                        <option value="">Seleziona...</option>
                        <?php foreach (($options['actor_id'] ?? []) as $optionValue => $optionLabel): ?>
                            <option
                                value="<?= esc($optionValue) ?>"
                                <?= (string) old('actor_id', $row->{'actor_id'} ?? ($context['actor_id'] ?? '')) === (string) $optionValue ? 'selected' : '' ?>
                            >
                                <?= esc($optionLabel) ?>
                            </option>
                        <?php endforeach; ?>
                        ...
                        <a
                            ...
                            data-value-source="actor_id"
                            data-base-url="<?= site_url('actor/view') ?>"
                        ...
                        <button
                            ...
                            id="related_create_actor_id_toggle"
                            data-bs-toggle="offcanvas"
                            data-bs-target="#related_create_actor_id"
                            aria-controls="related_create_actor_id"
                            data-related-field="actor_id"
                            data-panel-target="related_create_actor_id"
                            data-state-target="related_create_actor_id_state"
                            title="Crea nuovo Actor"
                            aria-label="Crea nuovo Actor"
                        >
                ...
                <?php if ($row === null): ?>
                    ...
                    $relatedCreateActive = !empty($relatedNewState['actor_id']);
                    ?>
                    <div class="col-12">
                        <input
                            type="hidden"
                            name="_related_new[actor_id]"
                            id="related_create_actor_id_state"
                            value="<?= $relatedCreateActive ? '1' : '0' ?>"
                        >
                        <div
                            id="related_create_actor_id"
                            class="offcanvas offcanvas-end crud-related-create-panel"
                            tabindex="-1"
                            aria-labelledby="related_create_actor_id_label"
                            data-related-field="actor_id"
                            data-state-target="related_create_actor_id_state"
                            data-toggle-target="related_create_actor_id_toggle"
                            data-bs-backdrop="static"
                        >
                            <div class="offcanvas-header border-bottom">
                                <div>
                                    <h2 class="offcanvas-title h5 mb-0" id="related_create_actor_id_label">Nuovo Actor</h2>
                                    <small class="text-muted">Relazione actor_id</small>
                                </div>
                                <button
                                    type="button"
                                    ...
                                    aria-label="Annulla nuovo Actor"
                                ></button>
                            </div>
                            <div class="offcanvas-body">
                                <div class="alert alert-light border small" role="note">
                                    Compila i dati del nuovo Actor. Il record collegato e questo record verranno salvati insieme al submit del form principale, nella stessa transazione.
                                </div>
                                <?= view('film_actor/_related_create_actor_id', [
                                    'relatedField'        => 'actor_id',
...
<div class="input-group crud-relation-input-group">
                    ...
                    >
                        ...
                        <button
                            ...
                            title="Crea nuovo Film"
                            aria-label="Crea nuovo Film"
                        >
                            <i class="bi bi-plus-circle me-1" aria-hidden="true"></i>
                            Nuovo
                        </button>
                ...
                <?php if ($row === null): ?>
                    ...
                    <div class="col-12">
                        ...
                        >
                            <div class="offcanvas-header border-bottom">
                                <div>
                                    <h2 class="offcanvas-title h5 mb-0" id="related_create_film_id_label">Nuovo Film</h2>
                                    <small class="text-muted">Relazione film_id</small>
                                </div>
                                <button
                                    type="button"
                                    class="btn-close crud-related-create-cancel"
                                    ...
                                    data-state-target="related_create_film_id_state"
                                    data-bs-dismiss="offcanvas"
                                    aria-label="Annulla nuovo Film"
                                ></button>
                            </div>
                            <div class="offcanvas-body">
                                <div class="alert alert-light border small" role="note">
                                    Compila i dati del nuovo Film. Il record collegato e questo record verranno salvati insieme al submit del form principale, nella stessa transazione.
                                </div>
                                <?= view('film_actor/_related_create_film_id', [
