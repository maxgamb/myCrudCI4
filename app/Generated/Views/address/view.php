...
<div class="container py-4">
    <div class="d-print-none">
        ...
        <a href="<?= site_url('address/create') . ($navigationQuery ?? '') ?>" class="btn btn-primary" title="New record">
            <i class="bi bi-plus-circle me-1" aria-hidden="true"></i> New
        </a>        <a href="<?= site_url('address') . ($navigationQuery ?? '') ?>" class="btn btn-outline-secondary" title="Back to list">
            <i class="bi bi-list-ul me-1" aria-hidden="true"></i> List
        </a>
        <a href="<?= site_url('address/edit/' . rawurlencode((string) ($row->{'address_id'} ?? ''))) . ($navigationQuery ?? '') ?>" class="btn btn-outline-warning" title="Edit record">
            <i class="bi bi-pencil-square me-1" aria-hidden="true"></i> Edit
        </a>
        <form method="post" action="<?= site_url('address/delete/' . rawurlencode((string) ($row->{'address_id'} ?? ''))) . ($navigationQuery ?? '') ?>" class="d-inline" onsubmit="return confirm('Delete this record?')">
            <?= csrf_field() ?>
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-bordered table-striped align-middle">
                        ...
                        <tr>
                            ...
                            <td><?php
$parentTargetId = (string) ($row->{'city_id'} ?? '');
$parentTrail = \App\Libraries\Crud\CrudNavigationTrail::ancestorsForParent((array) ($cascadeTrail ?? []), 'city', $parentTargetId);
if ($parentTrail === (array) ($cascadeTrail ?? [])) {
    $parentTrail = \App\Libraries\Crud\CrudNavigationTrail::append($parentTrail, "address", (string) ($row->{'address_id'} ?? ''), trim((string) ($row->{'address'} ?? '')) ?: 'Address' . ' #' . (string) ($row->{'address_id'} ?? ''));
}
$parentTrailEncoded = \App\Libraries\Crud\CrudNavigationTrail::encode($parentTrail);
$parentUrl = site_url('city' . '/view/' . rawurlencode($parentTargetId));
if ($parentTrailEncoded !== '') $parentUrl .= '?_trail=' . rawurlencode($parentTrailEncoded);
?>
<a href="<?= esc($parentUrl) ?>" class="text-decoration-none"><?= esc($row->{'city_id__label'} ?? $row->{'city_id'} ?? '') ?></a></td>
                        </tr>                        <tr>
...
<?= view('address/_children_customer__address_id', ['row' => $row, 'children' => $children, 'cascadeTrail' => $cascadeTrail ?? []]) ?>
<?= view('address/_children_staff__address_id', ['row' => $row, 'children' => $children, 'cascadeTrail' => $cascadeTrail ?? []]) ?>
<?= view('address/_children_store__address_id', ['row' => $row, 'children' => $children, 'cascadeTrail' => $cascadeTrail ?? []]) ?>
