        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-sm table-bordered table-striped align-middle">
                    <tbody>
                        <tr>
                            <th class="w-25"><?= esc(lang('ObmpCancellations.obmp_cancellation_id')) ?></th>
                            <td><?= esc($row->obmp_cancellation_id ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('ObmpCancellations.obmp_cancellation_cod')) ?></th>
                            <td><?= esc($row->obmp_cancellation_cod ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('ObmpCancellations.obmp_cancellation_title')) ?></th>
                            <td><?= esc($row->obmp_cancellation_title ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('ObmpCancellations.obmp_cancellation')) ?></th>
                            <td><?= esc($row->obmp_cancellation ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('ObmpCancellations.obmp_cancellation_day')) ?></th>
                            <td><?= esc($row->obmp_cancellation_day ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('ObmpCancellations.cancellation_lg')) ?></th>
...
<div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <span class="badge bg-secondary"><?= (int) ($children['obmp_rates__obmp_cancellation_cod']['count'] ?? 0) ?><?= !empty($children['obmp_rates__obmp_cancellation_cod']['hasMore']) ? '+' : '' ?></span>
        </div>
        <div class="card-body">
            <?php $relatedRows = $children['obmp_rates__obmp_cancellation_cod']['rows'] ?? []; ?>
            ...
            <?php else: ?>
                ...
                <?php if (!empty($children['obmp_rates__obmp_cancellation_cod']['hasMore'])): ?>
