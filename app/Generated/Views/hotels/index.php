...
<div class="container-fluid px-0">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            <a href="<?= site_url('hotels/create') ?>" class="btn btn-primary">
                <i class="bi bi-plus-circle"></i> Nuovo
            </a>
            <a id="exportCsvButton" href="<?= site_url('hotels/export-csv') ?>" class="btn btn-success">
                <i class="bi bi-filetype-csv"></i> Esporta CSV
            </a>
            <a id="exportWordButton" href="<?= site_url('hotels/export-word') ?>" class="btn btn-outline-primary">
    ...
    <details class="mb-3" <?= $hasActiveFilters ? 'open' : '' ?>>
        <summary class="fw-semibold"><?= esc(lang('Hotels.filtersSummary')) ?></summary>
        <div class="card card-body mt-2">
            <?= view('hotels/_filters', [
                'filters' => $filters ?? [],
                'options' => $options ?? [],
                'perPage' => $perPage ?? 25,
                'sort' => $sort ?? 'hotel_id',
    ...
    <div id="crudTableContainer" aria-live="polite" aria-busy="false">
        <?= view('hotels/_table', [
            ...
            'primaryKey' => $primaryKey ?? 'hotel_id',
            'sort' => $sort ?? 'hotel_id',
            'direction' => $direction ?? 'desc',
            'query' => $query ?? [],
        ]) ?>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateExportUrls = () => {
        ...
        const csvUrl = new URL("<?= site_url('hotels/export-csv') ?>", window.location.origin);
        const wordUrl = new URL("<?= site_url('hotels/export-word') ?>", window.location.origin);
