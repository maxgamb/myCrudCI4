    ...
    <div class="d-print-none">
        <nav aria-label="breadcrumb" class="mb-2">
            <ol class="breadcrumb mb-0">
                ...
                <?php endforeach; ?>
                <li class="breadcrumb-item"><a href="<?= site_url('payment') . $navigationQuery ?>">payment</a></li>
        ...
        <a href="<?= site_url('payment/create') . ($navigationQuery ?? '') ?>" class="btn btn-primary" title="New record">
            <i class="bi bi-plus-circle me-1" aria-hidden="true"></i> New
        </a>        <a href="<?= site_url('payment') . ($navigationQuery ?? '') ?>" class="btn btn-outline-secondary" title="Back to list">
            <i class="bi bi-list-ul me-1" aria-hidden="true"></i> List
        </a>
        <a href="<?= site_url('payment/edit/' . rawurlencode((string) ($row->{'payment_id'} ?? ''))) . ($navigationQuery ?? '') ?>" class="btn btn-outline-warning" title="Edit record">
            <i class="bi bi-pencil-square me-1" aria-hidden="true"></i> Edit
        </a>
        <form method="post" action="<?= site_url('payment/delete/' . rawurlencode((string) ($row->{'payment_id'} ?? ''))) . ($navigationQuery ?? '') ?>" class="d-inline" onsubmit="return confirm('Delete this record?')">
            <?= csrf_field() ?>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-sm table-bordered table-striped align-middle">
                        <tbody>
                        <tr>
                            <th class="w-25"><?= esc(lang('Payment.payment_id')) ?></th>
                            <td><?= esc($row->{'payment_id'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Payment.customer_id')) ?></th>
                            <td><?php
...
if ($parentTrail === (array) ($cascadeTrail ?? [])) {
    $parentTrail = \App\Libraries\Crud\CrudNavigationTrail::append($parentTrail, "payment", (string) ($row->{'payment_id'} ?? ''), 'Payment' . ' #' . (string) ($row->{'payment_id'} ?? ''));
}
...
<a href="<?= esc($parentUrl) ?>" class="text-decoration-none"><?= esc($row->{'customer_id__label'} ?? $row->{'customer_id'} ?? '') ?></a></td>
                            ...
                            <th class="w-25"><?= esc(lang('Payment.staff_id')) ?></th>
                            <td><?php
...
<a href="<?= esc($parentUrl) ?>" class="text-decoration-none"><?= esc($row->{'staff_id__label'} ?? $row->{'staff_id'} ?? '') ?></a></td>
                            ...
                            <th class="w-25"><?= esc(lang('Payment.rental_id')) ?></th>
                            <td><?php
$parentTargetId = (string) ($row->{'rental_id'} ?? '');
$parentTrail = \App\Libraries\Crud\CrudNavigationTrail::ancestorsForParent((array) ($cascadeTrail ?? []), 'rental', $parentTargetId);
if ($parentTrail === (array) ($cascadeTrail ?? [])) {
    $parentTrail = \App\Libraries\Crud\CrudNavigationTrail::append($parentTrail, "payment", (string) ($row->{'payment_id'} ?? ''), 'Payment' . ' #' . (string) ($row->{'payment_id'} ?? ''));
}
$parentTrailEncoded = \App\Libraries\Crud\CrudNavigationTrail::encode($parentTrail);
$parentUrl = site_url('rental' . '/view/' . rawurlencode($parentTargetId));
if ($parentTrailEncoded !== '') $parentUrl .= '?_trail=' . rawurlencode($parentTrailEncoded);
?>
<a href="<?= esc($parentUrl) ?>" class="text-decoration-none"><?= esc($row->{'rental_id__label'} ?? $row->{'rental_id'} ?? '') ?></a></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Payment.amount')) ?></th>
                            <td><?= esc($row->{'amount'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Payment.payment_date')) ?></th>
                            <td><?= esc($row->{'payment_date'} ?? '') ?></td>
                        </tr>                        <tr>
                            <th class="w-25"><?= esc(lang('Payment.last_update')) ?></th>
