...
<div class="container py-4">
    <div class="d-print-none">
        ...
        <a href="<?= site_url('payment/create') . ($navigationQuery ?? '') ?>" class="btn btn-primary" title="Nuovo record">
            <i class="bi bi-plus-circle me-1" aria-hidden="true"></i> Nuovo
        </a>        <a href="<?= site_url('payment') . ($navigationQuery ?? '') ?>" class="btn btn-outline-secondary" title="Torna alla lista">
            <i class="bi bi-list-ul me-1" aria-hidden="true"></i> Lista
        </a>
        <a href="<?= site_url('payment/edit/' . rawurlencode((string) ($row->{'payment_id'} ?? ''))) . ($navigationQuery ?? '') ?>" class="btn btn-outline-warning" title="Modifica record">
            <i class="bi bi-pencil-square me-1" aria-hidden="true"></i> Modifica
        </a>
        <form method="post" action="<?= site_url('payment/delete/' . rawurlencode((string) ($row->{'payment_id'} ?? ''))) . ($navigationQuery ?? '') ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
            <?= csrf_field() ?>
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-bordered table-striped align-middle">
                        ...
                        <tr>
                            ...
                            <td><a href="<?= site_url('rental/view/' . rawurlencode((string) ($row->{'rental_id'} ?? ''))) ?>" class="text-decoration-none"><?= esc($row->{'rental_id__label'} ?? $row->{'rental_id'} ?? '') ?></a></td>
                        </tr>                        <tr>
