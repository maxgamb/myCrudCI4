<?php
    $hasManyCollapseId = 'hasmany_' . preg_replace('/[^A-Za-z0-9_]/', '_', 'rental__inventory_id');
    ...
    <div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between align-items-center gap-2">
            ...
            <div class="d-flex align-items-center gap-2 d-print-none">
                <a href="<?= site_url('rental/create') . '?' . http_build_query(['inventory_id' => $row->{'inventory_id'} ?? '', '_parent_field' => 'inventory_id', '_trail' => \App\Libraries\Crud\CrudNavigationTrail::encode(\App\Libraries\Crud\CrudNavigationTrail::append((array) ($cascadeTrail ?? []), 'inventory', (string) ($row->{'inventory_id'} ?? ''), 'Inventory' . ' #' . (string) ($row->{'inventory_id'} ?? '')))]) ?>" class="btn btn-sm btn-primary" title="New related record"><i class="bi bi-plus-circle me-1" aria-hidden="true"></i> New</a>
                <a href="<?= site_url('rental') . '?' . http_build_query(['inventory_id' => $row->{'inventory_id'} ?? '', '_trail' => \App\Libraries\Crud\CrudNavigationTrail::encode(\App\Libraries\Crud\CrudNavigationTrail::append((array) ($cascadeTrail ?? []), 'inventory', (string) ($row->{'inventory_id'} ?? ''), 'Inventory' . ' #' . (string) ($row->{'inventory_id'} ?? '')))]) ?>" class="btn btn-sm btn-outline-primary" title="View all related records"><i class="bi bi-list-ul me-1" aria-hidden="true"></i> View all</a>
                        ...
                        <table class="table table-sm table-striped table-hover align-middle">
                            ...
                            <tbody>
                                ...
                                <td><?= esc($child->{'last_update'} ?? '') ?></td>
                                    <td class="d-print-none"><a href="<?= site_url('rental/view/' . rawurlencode((string) ($child->{'rental_id'} ?? ''))) . '?' . http_build_query(['inventory_id' => $row->{'inventory_id'} ?? '', '_trail' => \App\Libraries\Crud\CrudNavigationTrail::encode(\App\Libraries\Crud\CrudNavigationTrail::append((array) ($cascadeTrail ?? []), 'inventory', (string) ($row->{'inventory_id'} ?? ''), 'Inventory' . ' #' . (string) ($row->{'inventory_id'} ?? '')))]) ?>" class="btn btn-sm btn-outline-info" title="Visualizza record"><i class="bi bi-eye" aria-hidden="true"></i></a></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
