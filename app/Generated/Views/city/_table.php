                    ...
                    <?php else: ?>
                        <?php foreach ($rows as $row): ?>
                            <tr>
                                ...
                                <td>
                                    <?php if ((string) ($row->{'country_id'} ?? '') !== ''): ?><a href="<?= site_url('country/view/' . rawurlencode((string) $row->{'country_id'})) ?>" class="text-decoration-none"><?= esc($row->{'country_id__label'} ?? $row->{'country_id'} ?? '') ?></a><?php else: ?><?= esc($row->{'country_id__label'} ?? '') ?><?php endif; ?>
                                    <?php
                                    $quickQuery = (array) ($query ?? []);
                                ...
                                <td class="text-end text-nowrap">
                                    ...
                                    <div class="btn-group btn-group-sm" role="group" aria-label="Azioni record">
                                        ...
                                        <a href="<?= site_url('city/edit/' . rawurlencode((string) $id)) . ($navigationQuery ?? '') ?>" class="btn btn-outline-warning" title="Modifica">
                                            <i class="bi bi-pencil-square" aria-hidden="true"></i>
                                        </a>
                                        <form method="post" action="<?= site_url('city/delete/' . rawurlencode((string) $id)) . ($navigationQuery ?? '') ?>" class="d-inline" onsubmit="return confirm('Eliminare questo record?')">
                                            <?= csrf_field() ?>
