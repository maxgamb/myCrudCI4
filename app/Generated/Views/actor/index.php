...
<div class="container-fluid px-0">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            <a id="createRecordButton" data-base-url="<?= site_url('actor/create') ?>" href="<?= site_url('actor/create') . ($navigationQuery ?? '') ?>" class="btn btn-primary" title="Nuovo record">
                <i class="bi bi-plus-circle me-1" aria-hidden="true"></i> Nuovo
            </a>            <a id="exportCsvButton" href="<?= site_url('actor/export-csv') . $navigationQuery ?>" class="btn btn-outline-success" title="Esporta i risultati filtrati in CSV">
                <i class="bi bi-filetype-csv me-1" aria-hidden="true"></i> CSV
            </a>
            <a id="exportWordButton" href="<?= site_url('actor/export-word') . $navigationQuery ?>" class="btn btn-outline-primary" title="Esporta i risultati filtrati in Word">
    ...
    <details class="mb-3" <?= $hasActiveFilters ? 'open' : '' ?>>
        <summary class="fw-semibold"><?= esc(lang('Actor.filtersSummary')) ?></summary>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const simpleFilterFields = ["actor_id","last_name"];
    ...
    const updateActionUrls = (source = window.location.href) => {
        ...
        const csvUrl = new URL("<?= site_url('actor/export-csv') ?>", window.location.origin);
        const wordUrl = new URL("<?= site_url('actor/export-word') ?>", window.location.origin);
