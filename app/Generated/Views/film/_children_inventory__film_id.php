<?php
    $hasManyCollapseId = 'hasmany_' . preg_replace('/[^A-Za-z0-9_]/', '_', 'inventory__film_id');
                        ...
                        <table class="table table-sm table-striped table-hover align-middle">
                            ...
                            <tbody>
                                ...
                                <td><?= esc($child->{'last_update'} ?? '') ?></td>
                                    <td class="d-print-none"><a href="<?= site_url('inventory/view/' . rawurlencode((string) ($child->{'inventory_id'} ?? ''))) . '?' . http_build_query(['film_id' => $row->{'film_id'} ?? '', '_trail' => \App\Libraries\Crud\CrudNavigationTrail::encode(\App\Libraries\Crud\CrudNavigationTrail::append((array) ($cascadeTrail ?? []), 'film', (string) ($row->{'film_id'} ?? ''), trim((string) ($row->{'title'} ?? '')) ?: 'Film' . ' #' . (string) ($row->{'film_id'} ?? '')))]) ?>" class="btn btn-sm btn-outline-info" title="Visualizza record"><i class="bi bi-eye" aria-hidden="true"></i></a></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
