...
<div class="container-fluid px-0">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            ...
            <a id="exportCsvButton" href="<?= site_url('tipologia_camera/export-csv') ?>" class="btn btn-success">
                <i class="bi bi-filetype-csv"></i> Esporta CSV
            </a>
            <a id="exportWordButton" href="<?= site_url('tipologia_camera/export-word') ?>" class="btn btn-outline-primary">
    ...
    <details class="mb-3" <?= $hasActiveFilters ? 'open' : '' ?>>
        <summary class="fw-semibold"><?= esc(lang('TipologiaCamera.filtersSummary')) ?></summary>
        <div class="card card-body mt-2">
            <?= view('tipologia_camera/_filters', [
                'filters' => $filters ?? [],
                'options' => $options ?? [],
                'perPage' => $perPage ?? 25,
                'sort' => $sort ?? 'tipologia_id',
    ...
    <div id="crudTableContainer" aria-live="polite" aria-busy="false">
        <?= view('tipologia_camera/_table', [
            'rows' => $rows ?? [],
            'total' => $total ?? 0,
            'page' => $page ?? 1,
            'perPage' => $perPage ?? 25,
            'pagerLinks' => $pagerLinks ?? '',
            'primaryKey' => $primaryKey ?? 'tipologia_id',
            'sort' => $sort ?? 'tipologia_id',
            'direction' => $direction ?? 'desc',
            'query' => $query ?? [],
        ]) ?>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateExportUrls = () => {
        ...
        const csvUrl = new URL("<?= site_url('tipologia_camera/export-csv') ?>", window.location.origin);
        const wordUrl = new URL("<?= site_url('tipologia_camera/export-word') ?>", window.location.origin);
