...
use App\Services\PraticheRifService;
use App\Validation\PraticheRifRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per pratiche_rif.
 *
 ...
 */
final class PraticheRifController extends BaseController
{
    ...
    private PraticheRifService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new PraticheRifService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('pratiche_rif/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('pratiche_rif/index', $data);
    }
  ...
  0 => 'pratiche_rif_id',
        ...
        return view('pratiche_rif/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(PraticheRifRules::createRules(), PraticheRifRules::messages())) {
        ...
        return redirect()->to(site_url('pratiche_rif'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('pratiche_rif/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(PraticheRifRules::updateRules($id), PraticheRifRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(true);
        unset($data['pratica_rif_pratica_id']);
        ...
        return redirect()->to(site_url('pratiche_rif'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('pratiche_rif'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('pratiche_rif'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'pratica_rif_pratica_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'pratiche_rif',
                languageGroup: 'PraticheRif',
    ...
    {
        return redirect()->to(site_url('pratiche_rif') . '?' . http_build_query((array) $this->request->getGet()))
