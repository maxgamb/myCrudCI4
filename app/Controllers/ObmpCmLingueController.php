...
use App\Services\ObmpCmLingueService;
use App\Validation\ObmpCmLingueRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_cm_lingue.
 *
 ...
 */
final class ObmpCmLingueController extends BaseController
{
    ...
    private ObmpCmLingueService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpCmLingueService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_cm_lingue/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_cm_lingue/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('obmp_cm_lingue/view', [
            'title' => 'Dettaglio',
            'row' => $row,
  ...
  0 => 'obmp_cm_rooms_id',
        ...
        return view('obmp_cm_lingue/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpCmLingueRules::createRules(), ObmpCmLingueRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['obmp_cm_lingue_id']);
        ...
        return redirect()->to(site_url('obmp_cm_lingue'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_cm_lingue/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpCmLingueRules::updateRules($id), ObmpCmLingueRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_cm_lingue'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_cm_lingue'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_cm_lingue'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'obmp_cm_lingue_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'obmp_cm_lingue',
                languageGroup: 'ObmpCmLingue',
    ...
    {
        return redirect()->to(site_url('obmp_cm_lingue') . '?' . http_build_query((array) $this->request->getGet()))
