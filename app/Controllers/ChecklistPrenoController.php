...
use App\Services\ChecklistPrenoService;
use App\Validation\ChecklistPrenoRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per checklist_preno.
 *
 ...
 */
final class ChecklistPrenoController extends BaseController
{
    ...
    private ChecklistPrenoService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ChecklistPrenoService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('checklist_preno/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('checklist_preno/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('checklist_preno/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    public function create()
    {
        ...
        return view('checklist_preno/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ChecklistPrenoRules::createRules(), ChecklistPrenoRules::messages())) {
        ...
        return redirect()->to(site_url('checklist_preno'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('checklist_preno/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ChecklistPrenoRules::updateRules($id), ChecklistPrenoRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('checklist_preno'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('checklist_preno'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('checklist_preno'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'checklist_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'checklist_preno',
                languageGroup: 'ChecklistPreno',
    ...
    {
        return redirect()->to(site_url('checklist_preno') . '?' . http_build_query((array) $this->request->getGet()))
