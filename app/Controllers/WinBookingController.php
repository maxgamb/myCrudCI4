...
use App\Services\WinBookingService;
use App\Validation\WinBookingRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per win_booking.
 *
 ...
 */
final class WinBookingController extends BaseController
{
    ...
    private WinBookingService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new WinBookingService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('win_booking/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('win_booking/index', $data);
    }
    ...
    public function create()
    {
        ...
        return view('win_booking/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(WinBookingRules::createRules(), WinBookingRules::messages())) {
        ...
        return redirect()->to(site_url('win_booking'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('win_booking/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(WinBookingRules::updateRules($id), WinBookingRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('win_booking'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('win_booking'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('win_booking'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'win_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'win_booking',
                languageGroup: 'WinBooking',
    ...
    {
        return redirect()->to(site_url('win_booking') . '?' . http_build_query((array) $this->request->getGet()))
