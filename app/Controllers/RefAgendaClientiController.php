...
use App\Services\RefAgendaClientiService;
use App\Validation\RefAgendaClientiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per ref_agenda_clienti.
 *
 ...
 */
final class RefAgendaClientiController extends BaseController
{
    ...
    private RefAgendaClientiService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new RefAgendaClientiService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('ref_agenda_clienti/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('ref_agenda_clienti/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('ref_agenda_clienti/view', [
            'title' => 'Dettaglio',
            'row' => $row,
  ...
  0 => 'preno_id',
        ...
        return view('ref_agenda_clienti/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(RefAgendaClientiRules::createRules(), RefAgendaClientiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['ref_agenda_cliente']);
        ...
        return redirect()->to(site_url('ref_agenda_clienti'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('ref_agenda_clienti/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(RefAgendaClientiRules::updateRules($id), RefAgendaClientiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('ref_agenda_clienti'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('ref_agenda_clienti'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('ref_agenda_clienti'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'ref_agenda_cliente', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'ref_agenda_clienti',
                languageGroup: 'RefAgendaClienti',
    ...
    {
        return redirect()->to(site_url('ref_agenda_clienti') . '?' . http_build_query((array) $this->request->getGet()))
