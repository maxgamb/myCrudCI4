...
use App\Services\ObmpRestrictionsService;
use App\Validation\ObmpRestrictionsRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_restrictions.
 *
 ...
 */
final class ObmpRestrictionsController extends BaseController
{
    ...
    private ObmpRestrictionsService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpRestrictionsService();
  ...
  2 => 100,
        ...
        $data += [
            'title' => 'obmp_restrictions',
            'primaryKey' => 'obmp_restriction_id',
            'filters' => $listRequest->filters,
            'query' => $listRequest->query,
        ];

        if ($this->request->isAJAX()) {
            return view('obmp_restrictions/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_restrictions/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('obmp_restrictions/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    public function create()
    {
        ...
        return view('obmp_restrictions/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpRestrictionsRules::createRules(), ObmpRestrictionsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['obmp_restriction_id']);
        ...
        return redirect()->to(site_url('obmp_restrictions'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_restrictions/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpRestrictionsRules::updateRules($id), ObmpRestrictionsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_restrictions'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_restrictions'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_restrictions'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'obmp_restriction_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'obmp_restrictions',
                languageGroup: 'ObmpRestrictions',
    ...
    {
        return redirect()->to(site_url('obmp_restrictions') . '?' . http_build_query((array) $this->request->getGet()))
