...
use App\Services\GuastiService;
use App\Validation\GuastiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per guasti.
 *
 ...
 */
final class GuastiController extends BaseController
{
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new GuastiService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('guasti/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('guasti/index', $data);
    }
  ...
  1 => 'camera_id',
    ...
    public function store()
    {
        ...
        if (!$this->validate(GuastiRules::createRules(), GuastiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['guasto_id']);
        ...
        return redirect()->to(site_url('guasti'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('guasti/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(GuastiRules::updateRules($id), GuastiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('guasti'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('guasti'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('guasti'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'guasto_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('guasti') . '?' . http_build_query((array) $this->request->getGet()))
