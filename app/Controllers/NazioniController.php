...
use App\Services\NazioniService;
use App\Validation\NazioniRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per nazioni.
 *
 ...
 */
final class NazioniController extends BaseController
{
    ...
    private NazioniService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new NazioniService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('nazioni/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('nazioni/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(NazioniRules::createRules(), NazioniRules::messages())) {
        ...
        return redirect()->to(site_url('nazioni'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('nazioni/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(NazioniRules::updateRules($id), NazioniRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(true);
        unset($data['Nazioni_Id_Codice']);
        ...
        return redirect()->to(site_url('nazioni'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('nazioni'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('nazioni'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'Nazioni_Id_Codice', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('nazioni') . '?' . http_build_query((array) $this->request->getGet()))
