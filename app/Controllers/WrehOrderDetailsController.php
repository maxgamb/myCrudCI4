...
use App\Validation\WrehOrderDetailsRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per wreh_order_details.
 *
 ...
 */
final class WrehOrderDetailsController extends BaseController
{
    ...
    private WrehOrderDetailsService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new WrehOrderDetailsService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('wreh_order_details/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('wreh_order_details/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('wreh_order_details/view', [
            'title' => 'Dettaglio',
            'row' => $row,
  ...
  1 => 'product_id',
        ...
        return view('wreh_order_details/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(WrehOrderDetailsRules::createRules(), WrehOrderDetailsRules::messages())) {
        ...
        return redirect()->to(site_url('wreh_order_details'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('wreh_order_details/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(WrehOrderDetailsRules::updateRules($id), WrehOrderDetailsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('wreh_order_details'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('wreh_order_details'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('wreh_order_details'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'order_detail_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'wreh_order_details',
                languageGroup: 'WrehOrderDetails',
    ...
    {
        return redirect()->to(site_url('wreh_order_details') . '?' . http_build_query((array) $this->request->getGet()))
