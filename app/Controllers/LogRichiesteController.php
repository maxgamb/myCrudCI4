...
use App\Services\LogRichiesteService;
use App\Validation\LogRichiesteRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per log_richieste.
 *
 ...
 */
final class LogRichiesteController extends BaseController
{
    ...
    private LogRichiesteService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new LogRichiesteService();
  ...
  2 => 100,
    ...
    public function view(int|string $id)
    {
        ...
        return view('log_richieste/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('log_richieste/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(LogRichiesteRules::createRules(), LogRichiesteRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['log_ric_id']);
        ...
        return redirect()->to(site_url('log_richieste'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('log_richieste/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(LogRichiesteRules::updateRules($id), LogRichiesteRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('log_richieste'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('log_richieste'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('log_richieste'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'log_ric_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'log_richieste',
                languageGroup: 'LogRichieste',
    ...
    {
        return redirect()->to(site_url('log_richieste') . '?' . http_build_query((array) $this->request->getGet()))
