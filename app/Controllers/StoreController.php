...
/**
 * Controller CRUD full per store.
 *
...
final class StoreController extends BaseController
{
  ...
  0 => 'manager_staff_id',
    ...
    public function store()
    {
        ...
        $createRules = StoreRules::createRules();
        ...
        if (!$this->validate($createRules, StoreRules::messages())) {
        ...
        return redirect()->to($this->contextUrl('store', $navigationContext))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(StoreRules::updateRules($id), StoreRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to($this->contextUrl('store', $navigationContext))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        ...
        try {
            ...
            return redirect()->to($this->contextUrl('store', $navigationContext))->with('error', $e->getMessage());
        }
        return redirect()->to($this->contextUrl('store', $navigationContext))->with('message', 'Record eliminato correttamente.');
    }
    /**
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'store_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
), array (
  ...
  0 => 'last_update',
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = StoreRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('store') . ($query === [] ? '' : '?' . http_build_query($query));
