...
use App\Services\ModificaContiService;
use App\Validation\ModificaContiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per modifica_conti.
 *
 ...
 */
final class ModificaContiController extends BaseController
{
    ...
    private ModificaContiService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ModificaContiService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('modifica_conti/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('modifica_conti/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('modifica_conti/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('modifica_conti/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ModificaContiRules::createRules(), ModificaContiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['id_mod_conto']);
        ...
        return redirect()->to(site_url('modifica_conti'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('modifica_conti/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ModificaContiRules::updateRules($id), ModificaContiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('modifica_conti'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('modifica_conti'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('modifica_conti'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'id_mod_conto', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'modifica_conti',
                languageGroup: 'ModificaConti',
    ...
    {
        return redirect()->to(site_url('modifica_conti') . '?' . http_build_query((array) $this->request->getGet()))
