...
use App\Services\PuliziaService;
use App\Validation\PuliziaRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per pulizia.
 *
 ...
 */
final class PuliziaController extends BaseController
{
    ...
    private PuliziaService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new PuliziaService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('pulizia/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('pulizia/index', $data);
    }
  ...
  0 => 'conto_id',
    ...
    public function store()
    {
        ...
        if (!$this->validate(PuliziaRules::createRules(), PuliziaRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['pulizia_id']);
        ...
        return redirect()->to(site_url('pulizia'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('pulizia/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(PuliziaRules::updateRules($id), PuliziaRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('pulizia'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('pulizia'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('pulizia'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'pulizia_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('pulizia') . '?' . http_build_query((array) $this->request->getGet()))
