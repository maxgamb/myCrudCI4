...
use App\Services\NazioniBandieraService;
use App\Validation\NazioniBandieraRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per nazioni_bandiera.
 *
 ...
 */
final class NazioniBandieraController extends BaseController
{
    ...
    private NazioniBandieraService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new NazioniBandieraService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('nazioni_bandiera/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('nazioni_bandiera/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('nazioni_bandiera/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('nazioni_bandiera/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(NazioniBandieraRules::createRules(), NazioniBandieraRules::messages())) {
        ...
        return redirect()->to(site_url('nazioni_bandiera'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('nazioni_bandiera/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(NazioniBandieraRules::updateRules($id), NazioniBandieraRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(true);
        unset($data['nazione_iso2']);
        ...
        return redirect()->to(site_url('nazioni_bandiera'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('nazioni_bandiera'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('nazioni_bandiera'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'nazione_iso2', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'nazioni_bandiera',
                languageGroup: 'NazioniBandiera',
    ...
    {
        return redirect()->to(site_url('nazioni_bandiera') . '?' . http_build_query((array) $this->request->getGet()))
