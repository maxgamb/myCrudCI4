...
use App\Services\RefObmpBookingService;
use App\Validation\RefObmpBookingRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per ref_obmp_booking.
 *
 ...
 */
final class RefObmpBookingController extends BaseController
{
    ...
    private RefObmpBookingService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new RefObmpBookingService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('ref_obmp_booking/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('ref_obmp_booking/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('ref_obmp_booking/view', [
            'title' => 'Dettaglio',
            'row' => $row,
  ...
  3 => 'quote_id',
    ...
    public function store()
    {
        ...
        if (!$this->validate(RefObmpBookingRules::createRules(), RefObmpBookingRules::messages())) {
        ...
        return redirect()->to(site_url('ref_obmp_booking'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('ref_obmp_booking/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(RefObmpBookingRules::updateRules($id), RefObmpBookingRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(true);
        unset($data['ref_obm_data']);
        ...
        return redirect()->to(site_url('ref_obmp_booking'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('ref_obmp_booking'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('ref_obmp_booking'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'ref_obm_data', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'ref_obmp_booking',
                languageGroup: 'RefObmpBooking',
    ...
    {
        return redirect()->to(site_url('ref_obmp_booking') . '?' . http_build_query((array) $this->request->getGet()))
