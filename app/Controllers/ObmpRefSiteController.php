...
use App\Services\ObmpRefSiteService;
use App\Validation\ObmpRefSiteRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_ref_site.
 *
 ...
 */
final class ObmpRefSiteController extends BaseController
{
    ...
    private ObmpRefSiteService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpRefSiteService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_ref_site/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_ref_site/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('obmp_ref_site/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    public function create()
    {
        ...
        return view('obmp_ref_site/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpRefSiteRules::createRules(), ObmpRefSiteRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['ref_site_id']);
        ...
        return redirect()->to(site_url('obmp_ref_site'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_ref_site/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpRefSiteRules::updateRules($id), ObmpRefSiteRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_ref_site'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_ref_site'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_ref_site'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'ref_site_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'obmp_ref_site',
                languageGroup: 'ObmpRefSite',
    ...
    {
        return redirect()->to(site_url('obmp_ref_site') . '?' . http_build_query((array) $this->request->getGet()))
