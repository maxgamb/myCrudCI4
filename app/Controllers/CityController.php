...
final class CityController extends BaseController
{
  ...
  0 => 'country_id',
    ...
    public function store()
    {
        ...
        $createRules = CityRules::createRules();
        ...
        if (!$this->validate($createRules, CityRules::messages())) {
        ...
        return redirect()->to($this->contextUrl('city', $navigationContext))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CityRules::updateRules($id), CityRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to($this->contextUrl('city', $navigationContext))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        ...
        try {
            ...
            return redirect()->to($this->contextUrl('city', $navigationContext))->with('error', $e->getMessage());
        }
        return redirect()->to($this->contextUrl('city', $navigationContext))->with('message', 'Record eliminato correttamente.');
    }
    /**
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'city_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
), array (
  ...
  0 => 'last_update',
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = CityRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('city') . ($query === [] ? '' : '?' . http_build_query($query));
