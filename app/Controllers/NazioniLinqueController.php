...
use App\Services\NazioniLinqueService;
use App\Validation\NazioniLinqueRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per nazioni_linque.
 *
 ...
 */
final class NazioniLinqueController extends BaseController
{
    ...
    private NazioniLinqueService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new NazioniLinqueService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('nazioni_linque/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('nazioni_linque/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('nazioni_linque/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    public function create()
    {
        ...
        return view('nazioni_linque/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(NazioniLinqueRules::createRules(), NazioniLinqueRules::messages())) {
        ...
        return redirect()->to(site_url('nazioni_linque'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('nazioni_linque/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(NazioniLinqueRules::updateRules($id), NazioniLinqueRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('nazioni_linque'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('nazioni_linque'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('nazioni_linque'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'isoKey', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'nazioni_linque',
                languageGroup: 'NazioniLinque',
    ...
    {
        return redirect()->to(site_url('nazioni_linque') . '?' . http_build_query((array) $this->request->getGet()))
