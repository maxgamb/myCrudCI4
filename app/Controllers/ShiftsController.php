...
use App\Validation\ShiftsRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per shifts.
 *
 ...
 */
final class ShiftsController extends BaseController
{
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ShiftsService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('shifts/_table', $data);
        }
  ...
  0 => 'staff_id',
    ...
    public function store()
    {
        ...
        if (!$this->validate(ShiftsRules::createRules(), ShiftsRules::messages())) {
        ...
        return redirect()->to(site_url('shifts'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ShiftsRules::updateRules($id), ShiftsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('shifts'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('shifts'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('shifts'))->with('message', 'Record eliminato correttamente.');
    }
  ...
  2 => 100,
    ...
    {
        return redirect()->to(site_url('shifts') . '?' . http_build_query((array) $this->request->getGet()))
