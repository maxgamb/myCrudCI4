...
use App\Validation\CodiciStatoRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per codici_stato.
 *
 ...
 */
final class CodiciStatoController extends BaseController
{
    ...
    private CodiciStatoService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new CodiciStatoService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('codici_stato/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('codici_stato/index', $data);
    }
    ...
    public function create()
    {
        ...
        return view('codici_stato/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(CodiciStatoRules::createRules(), CodiciStatoRules::messages())) {
        ...
        return redirect()->to(site_url('codici_stato'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('codici_stato/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CodiciStatoRules::updateRules($id), CodiciStatoRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('codici_stato'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('codici_stato'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('codici_stato'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'cod_stato', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'codici_stato',
                languageGroup: 'CodiciStato',
    ...
    {
        return redirect()->to(site_url('codici_stato') . '?' . http_build_query((array) $this->request->getGet()))
