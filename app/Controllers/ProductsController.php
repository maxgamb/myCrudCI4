...
use App\Validation\ProductsRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per products.
 *
...
final class ProductsController extends BaseController
{
    ...
    private ProductsService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ProductsService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('products/_table', $data);
        }
    ...
    public function store()
    {
        ...
        if (!$this->validate(ProductsRules::createRules(), ProductsRules::messages())) {
        ...
        return redirect()->to(site_url('products'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ProductsRules::updateRules($id), ProductsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('products'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('products'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('products'))->with('message', 'Record eliminato correttamente.');
    }
  ...
  2 => 100,
    ...
    {
        return redirect()->to(site_url('products') . '?' . http_build_query((array) $this->request->getGet()))
