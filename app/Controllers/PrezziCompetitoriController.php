...
use App\Services\PrezziCompetitoriService;
use App\Validation\PrezziCompetitoriRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per prezzi_competitori.
 *
 ...
 */
final class PrezziCompetitoriController extends BaseController
{
    ...
    private PrezziCompetitoriService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new PrezziCompetitoriService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('prezzi_competitori/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('prezzi_competitori/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('prezzi_competitori/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('prezzi_competitori/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(PrezziCompetitoriRules::createRules(), PrezziCompetitoriRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['prezzi_competitori_id']);
        ...
        return redirect()->to(site_url('prezzi_competitori'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('prezzi_competitori/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(PrezziCompetitoriRules::updateRules($id), PrezziCompetitoriRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('prezzi_competitori'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('prezzi_competitori'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('prezzi_competitori'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'prezzi_competitori_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'prezzi_competitori',
                languageGroup: 'PrezziCompetitori',
    ...
    {
        return redirect()->to(site_url('prezzi_competitori') . '?' . http_build_query((array) $this->request->getGet()))
