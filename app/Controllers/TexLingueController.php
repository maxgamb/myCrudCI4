...
use App\Services\TexLingueService;
use App\Validation\TexLingueRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per tex_lingue.
 *
 ...
 */
final class TexLingueController extends BaseController
{
    ...
    private TexLingueService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new TexLingueService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('tex_lingue/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('tex_lingue/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(TexLingueRules::createRules(), TexLingueRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['tex_lingue_id']);
        ...
        return redirect()->to(site_url('tex_lingue'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('tex_lingue/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(TexLingueRules::updateRules($id), TexLingueRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('tex_lingue'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('tex_lingue'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('tex_lingue'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'tex_lingue_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('tex_lingue') . '?' . http_build_query((array) $this->request->getGet()))
