...
use App\Services\AdebitiService;
use App\Validation\AdebitiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per adebiti.
 *
 ...
 */
final class AdebitiController extends BaseController
{
    ...
    private AdebitiService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new AdebitiService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('adebiti/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('adebiti/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(AdebitiRules::createRules(), AdebitiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['adebito_id']);
        ...
        return redirect()->to(site_url('adebiti'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('adebiti/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(AdebitiRules::updateRules($id), AdebitiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('adebiti'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('adebiti'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('adebiti'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'adebito_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('adebiti') . '?' . http_build_query((array) $this->request->getGet()))
