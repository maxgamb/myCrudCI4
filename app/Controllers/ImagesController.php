...
use App\Validation\ImagesRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per images.
 *
...
final class ImagesController extends BaseController
{
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ImagesService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('images/_table', $data);
        }
  ...
  0 => 'obmp_cm_rooms_id',
    ...
    public function store()
    {
        ...
        if (!$this->validate(ImagesRules::createRules(), ImagesRules::messages())) {
        ...
        return redirect()->to(site_url('images'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ImagesRules::updateRules($id), ImagesRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('images'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('images'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('images'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'images_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('images') . '?' . http_build_query((array) $this->request->getGet()))
