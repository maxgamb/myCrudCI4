...
use App\Services\RefCostiTipologiaService;
use App\Validation\RefCostiTipologiaRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per ref_costi_tipologia.
 *
 ...
 */
final class RefCostiTipologiaController extends BaseController
{
    ...
    private RefCostiTipologiaService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new RefCostiTipologiaService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('ref_costi_tipologia/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('ref_costi_tipologia/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('ref_costi_tipologia/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('ref_costi_tipologia/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(RefCostiTipologiaRules::createRules(), RefCostiTipologiaRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['ref_costi_tipologia_id']);
        ...
        return redirect()->to(site_url('ref_costi_tipologia'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('ref_costi_tipologia/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(RefCostiTipologiaRules::updateRules($id), RefCostiTipologiaRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('ref_costi_tipologia'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('ref_costi_tipologia'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('ref_costi_tipologia'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'ref_costi_tipologia_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'ref_costi_tipologia',
                languageGroup: 'RefCostiTipologia',
    ...
    {
        return redirect()->to(site_url('ref_costi_tipologia') . '?' . http_build_query((array) $this->request->getGet()))
