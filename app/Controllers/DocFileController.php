...
use App\Validation\DocFileRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per doc_file.
 *
 ...
 */
final class DocFileController extends BaseController
{
    ...
    private DocFileService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new DocFileService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('doc_file/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('doc_file/index', $data);
    }
  ...
  0 => 'hotel_id',
    ...
    public function store()
    {
        ...
        if (!$this->validate(DocFileRules::createRules(), DocFileRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['doc_files_id']);
        ...
        return redirect()->to(site_url('doc_file'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('doc_file/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(DocFileRules::updateRules($id), DocFileRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('doc_file'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('doc_file'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('doc_file'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'doc_files_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('doc_file') . '?' . http_build_query((array) $this->request->getGet()))
