...
use App\Services\ObmpPaymentsService;
use App\Validation\ObmpPaymentsRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_payments.
 *
 ...
 */
final class ObmpPaymentsController extends BaseController
{
    ...
    private ObmpPaymentsService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpPaymentsService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_payments/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_payments/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('obmp_payments/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    public function create()
    {
        ...
        return view('obmp_payments/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpPaymentsRules::createRules(), ObmpPaymentsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['obmp_payment_id']);
        ...
        return redirect()->to(site_url('obmp_payments'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_payments/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpPaymentsRules::updateRules($id), ObmpPaymentsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_payments'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_payments'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_payments'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'obmp_payment_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'obmp_payments',
                languageGroup: 'ObmpPayments',
    ...
    {
        return redirect()->to(site_url('obmp_payments') . '?' . http_build_query((array) $this->request->getGet()))
