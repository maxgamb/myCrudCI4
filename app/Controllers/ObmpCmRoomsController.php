...
use App\Services\ObmpCmRoomsService;
use App\Validation\ObmpCmRoomsRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_cm_rooms.
 *
 ...
 */
final class ObmpCmRoomsController extends BaseController
{
    ...
    private ObmpCmRoomsService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpCmRoomsService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_cm_rooms/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_cm_rooms/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('obmp_cm_rooms/view', [
            'title' => 'Dettaglio',
            'row' => $row,
  ...
  1 => 'obmp_cm_rooms_tipologia_id',
        ...
        return view('obmp_cm_rooms/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpCmRoomsRules::createRules(), ObmpCmRoomsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['obmp_cm_rooms_id']);
        ...
        return redirect()->to(site_url('obmp_cm_rooms'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_cm_rooms/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpCmRoomsRules::updateRules($id), ObmpCmRoomsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_cm_rooms'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_cm_rooms'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_cm_rooms'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'obmp_cm_rooms_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'obmp_cm_rooms',
                languageGroup: 'ObmpCmRooms',
    ...
    {
        return redirect()->to(site_url('obmp_cm_rooms') . '?' . http_build_query((array) $this->request->getGet()))
