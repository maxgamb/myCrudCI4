...
use App\Services\ObmpBoardService;
use App\Validation\ObmpBoardRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_board.
 *
 ...
 */
final class ObmpBoardController extends BaseController
{
    ...
    private ObmpBoardService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpBoardService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_board/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_board/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpBoardRules::createRules(), ObmpBoardRules::messages())) {
        ...
        return redirect()->to(site_url('obmp_board'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_board/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpBoardRules::updateRules($id), ObmpBoardRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(true);
        unset($data['obmp_board_id']);
        ...
        return redirect()->to(site_url('obmp_board'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_board'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_board'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'obmp_board_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('obmp_board') . '?' . http_build_query((array) $this->request->getGet()))
