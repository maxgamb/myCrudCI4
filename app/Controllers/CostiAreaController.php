...
use App\Services\CostiAreaService;
use App\Validation\CostiAreaRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per costi_area.
 *
 ...
 */
final class CostiAreaController extends BaseController
{
    ...
    private CostiAreaService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new CostiAreaService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('costi_area/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('costi_area/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(CostiAreaRules::createRules(), CostiAreaRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['costi_area_id']);
        ...
        return redirect()->to(site_url('costi_area'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('costi_area/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CostiAreaRules::updateRules($id), CostiAreaRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('costi_area'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('costi_area'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('costi_area'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'costi_area_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('costi_area') . '?' . http_build_query((array) $this->request->getGet()))
