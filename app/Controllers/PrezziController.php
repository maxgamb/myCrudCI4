...
use App\Validation\PrezziRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per prezzi.
 *
 ...
 */
final class PrezziController extends BaseController
{
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new PrezziService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('prezzi/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('prezzi/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(PrezziRules::createRules(), PrezziRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['prezzo_id']);
        ...
        return redirect()->to(site_url('prezzi'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('prezzi/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(PrezziRules::updateRules($id), PrezziRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('prezzi'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('prezzi'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('prezzi'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'prezzo_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('prezzi') . '?' . http_build_query((array) $this->request->getGet()))
