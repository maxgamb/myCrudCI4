...
use App\Services\ObmpQuoteSubService;
use App\Validation\ObmpQuoteSubRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_quote_sub.
 *
 ...
 */
final class ObmpQuoteSubController extends BaseController
{
    ...
    private ObmpQuoteSubService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpQuoteSubService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_quote_sub/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_quote_sub/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('obmp_quote_sub/view', [
            'title' => 'Dettaglio',
            'row' => $row,
  ...
  0 => 'obmp_quote_id',
        ...
        return view('obmp_quote_sub/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpQuoteSubRules::createRules(), ObmpQuoteSubRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['obmp_quote_sub_id']);
        ...
        return redirect()->to(site_url('obmp_quote_sub'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_quote_sub/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpQuoteSubRules::updateRules($id), ObmpQuoteSubRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_quote_sub'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_quote_sub'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_quote_sub'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'obmp_quote_sub_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'obmp_quote_sub',
                languageGroup: 'ObmpQuoteSub',
    ...
    {
        return redirect()->to(site_url('obmp_quote_sub') . '?' . http_build_query((array) $this->request->getGet()))
