...
use App\Services\ObmpQuoteService;
use App\Validation\ObmpQuoteRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_quote.
 *
 ...
 */
final class ObmpQuoteController extends BaseController
{
    ...
    private ObmpQuoteService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpQuoteService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_quote/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_quote/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpQuoteRules::createRules(), ObmpQuoteRules::messages())) {
        ...
        return redirect()->to(site_url('obmp_quote'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_quote/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpQuoteRules::updateRules($id), ObmpQuoteRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_quote'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_quote'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_quote'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'quote_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('obmp_quote') . '?' . http_build_query((array) $this->request->getGet()))
