...
/**
 * Controller CRUD full per tipologia_camera.
 *
 ...
 */
final class TipologiaCameraController extends BaseController
{
    ...
    private TipologiaCameraService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new TipologiaCameraService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('tipologia_camera/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('tipologia_camera/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(TipologiaCameraRules::createRules(), TipologiaCameraRules::messages())) {
        ...
        return redirect()->to(site_url('tipologia_camera'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(TipologiaCameraRules::updateRules($id), TipologiaCameraRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('tipologia_camera'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('tipologia_camera'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('tipologia_camera'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'tipologia_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'tipologia_camera',
                languageGroup: 'TipologiaCamera',
    ...
    {
        return redirect()->to(site_url('tipologia_camera') . '?' . http_build_query((array) $this->request->getGet()))
