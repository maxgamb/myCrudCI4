...
use App\Services\PraticheService;
use App\Validation\PraticheRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per pratiche.
 *
 ...
 */
final class PraticheController extends BaseController
{
    ...
    private PraticheService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new PraticheService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('pratiche/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('pratiche/index', $data);
    }
  ...
  0 => 'pratica_agenzia_id',
    ...
    public function store()
    {
        ...
        if (!$this->validate(PraticheRules::createRules(), PraticheRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['pratica_id']);
        ...
        return redirect()->to(site_url('pratiche'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(PraticheRules::updateRules($id), PraticheRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('pratiche'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('pratiche'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('pratiche'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'pratica_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('pratiche') . '?' . http_build_query((array) $this->request->getGet()))
