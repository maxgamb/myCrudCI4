...
use App\Services\WrehSuppliersService;
use App\Validation\WrehSuppliersRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per wreh_suppliers.
 *
 ...
 */
final class WrehSuppliersController extends BaseController
{
    ...
    private WrehSuppliersService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new WrehSuppliersService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('wreh_suppliers/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('wreh_suppliers/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('wreh_suppliers/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('wreh_suppliers/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(WrehSuppliersRules::createRules(), WrehSuppliersRules::messages())) {
        ...
        return redirect()->to(site_url('wreh_suppliers'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('wreh_suppliers/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(WrehSuppliersRules::updateRules($id), WrehSuppliersRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('wreh_suppliers'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('wreh_suppliers'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('wreh_suppliers'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'supplier_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'wreh_suppliers',
                languageGroup: 'WrehSuppliers',
    ...
    {
        return redirect()->to(site_url('wreh_suppliers') . '?' . http_build_query((array) $this->request->getGet()))
