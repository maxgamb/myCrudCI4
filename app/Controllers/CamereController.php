...
/**
 * Controller CRUD full per camere.
 *
 ...
 */
final class CamereController extends BaseController
{
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new CamereService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('camere/_table', $data);
        }
  ...
  0 => 'tipologia_id',
    ...
    public function store()
    {
        ...
        if (!$this->validate(CamereRules::createRules(), CamereRules::messages())) {
        ...
        return redirect()->to(site_url('camere'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CamereRules::updateRules($id), CamereRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('camere'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('camere'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('camere'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'camera_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('camere') . '?' . http_build_query((array) $this->request->getGet()))
