...
use App\Services\EmailAiHistoryService;
use App\Validation\EmailAiHistoryRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per email_ai_history.
 *
 ...
 */
final class EmailAiHistoryController extends BaseController
{
    ...
    private EmailAiHistoryService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new EmailAiHistoryService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('email_ai_history/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('email_ai_history/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('email_ai_history/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    public function create()
    {
        ...
        return view('email_ai_history/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(EmailAiHistoryRules::createRules(), EmailAiHistoryRules::messages())) {
        ...
        return redirect()->to(site_url('email_ai_history'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('email_ai_history/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(EmailAiHistoryRules::updateRules($id), EmailAiHistoryRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('email_ai_history'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('email_ai_history'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('email_ai_history'))->with('message', 'Record eliminato correttamente.');
    }
  ...
  2 => 100,
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'email_ai_history',
                languageGroup: 'EmailAiHistory',
    ...
    {
        return redirect()->to(site_url('email_ai_history') . '?' . http_build_query((array) $this->request->getGet()))
