...
use App\Services\ObmpCmService;
use App\Validation\ObmpCmRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_cm.
 *
 ...
 */
final class ObmpCmController extends BaseController
{
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpCmService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_cm/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_cm/index', $data);
    }
  ...
  0 => 'agenzia_id',
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpCmRules::createRules(), ObmpCmRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['obmp_cm_id']);
        ...
        return redirect()->to(site_url('obmp_cm'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_cm/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpCmRules::updateRules($id), ObmpCmRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_cm'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_cm'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_cm'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'obmp_cm_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('obmp_cm') . '?' . http_build_query((array) $this->request->getGet()))
