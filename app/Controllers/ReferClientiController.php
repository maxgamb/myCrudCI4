...
use App\Validation\ReferClientiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per refer_clienti.
 *
 ...
 */
final class ReferClientiController extends BaseController
{
    ...
    private ReferClientiService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ReferClientiService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('refer_clienti/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('refer_clienti/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('refer_clienti/view', [
            'title' => 'Dettaglio',
            'row' => $row,
  ...
  0 => 'conto_id',
        ...
        return view('refer_clienti/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ReferClientiRules::createRules(), ReferClientiRules::messages())) {
        ...
        return redirect()->to(site_url('refer_clienti'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('refer_clienti/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ReferClientiRules::updateRules($id), ReferClientiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('refer_clienti'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('refer_clienti'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('refer_clienti'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'conto_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'refer_clienti',
                languageGroup: 'ReferClienti',
    ...
    {
        return redirect()->to(site_url('refer_clienti') . '?' . http_build_query((array) $this->request->getGet()))
