...
use App\Services\TaxPagamentoService;
use App\Validation\TaxPagamentoRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per tax_pagamento.
 *
 ...
 */
final class TaxPagamentoController extends BaseController
{
    ...
    private TaxPagamentoService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new TaxPagamentoService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('tax_pagamento/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('tax_pagamento/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('tax_pagamento/view', [
            'title' => 'Dettaglio',
            'row' => $row,
  ...
  0 => 'conto_id',
        ...
        return view('tax_pagamento/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(TaxPagamentoRules::createRules(), TaxPagamentoRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['tax_pagamento_id']);
        ...
        return redirect()->to(site_url('tax_pagamento'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('tax_pagamento/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(TaxPagamentoRules::updateRules($id), TaxPagamentoRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('tax_pagamento'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('tax_pagamento'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('tax_pagamento'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'tax_pagamento_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'tax_pagamento',
                languageGroup: 'TaxPagamento',
    ...
    {
        return redirect()->to(site_url('tax_pagamento') . '?' . http_build_query((array) $this->request->getGet()))
