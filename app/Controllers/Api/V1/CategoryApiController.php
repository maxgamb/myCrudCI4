...
use App\Validation\CategoryApiRules;
use RuntimeException;
use Throwable;

/** API REST v1 per la risorsa category. */
final class CategoryApiController extends BaseApiController
{
    ...
    public function index()
    {
        try {
            ...
            $result = $this->service->apiList($query, CategoryResource::filterableFields(), CategoryResource::sortableFields());
            return $this->success(CategoryResource::collection($result['rows']), $result['meta'], $result['links']);
        } catch (Throwable $e) {
    ...
    {
        try {
            return $this->success(CategoryResource::make($this->service->find($id)));
    ...
    {
        $data = CategoryResource::writableData($this->payload());
        if ($data === []) {
            return $this->error('EMPTY_PAYLOAD', 'Nessun campo scrivibile ricevuto.', 422);
        }
        if (!$this->validateData($data, CategoryApiRules::createRules(), CategoryApiRules::messages())) {
            return $this->error('VALIDATION_ERROR', 'Dati non validi.', 422, $this->validator->getErrors());
        }
        try {
            $id = $this->service->create($data);
            return $this->success(['category_id' => $id], [], [], 201);
    ...
    private function writeUpdate(int|string $id, bool $partial)
    {
        ...
        $rules = CategoryApiRules::updateRules($id);
        if ($partial) {
            $rules = array_intersect_key($rules, $data);
        }
        if ($rules !== [] && !$this->validateData($data, $rules, CategoryApiRules::messages())) {
            return $this->error('VALIDATION_ERROR', 'Dati non validi.', 422, $this->validator->getErrors());
        }
        try {
