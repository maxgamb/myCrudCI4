...
/** API REST v1 per la risorsa camere. */
final class CamereApiController extends BaseApiController
{
    public function __construct(private readonly CamereService $service = new CamereService())
    {
    }
    ...
    public function index()
    {
        try {
            ...
            $result = $this->service->apiList($query, CamereResource::filterableFields(), CamereResource::sortableFields());
            return $this->success(CamereResource::collection($result['rows']), $result['meta'], $result['links']);
        } catch (Throwable $e) {
    ...
    {
        try {
            return $this->success(CamereResource::make($this->service->find($id)));
    ...
    {
        $data = CamereResource::writableData($this->payload());
        if ($data === []) {
        ...
        try {
            ...
            return $this->success(['camera_id' => $id], [], [], 201);
