...
use App\Services\RefAgenziaPrenoService;
use App\Validation\RefAgenziaPrenoRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per ref_agenzia_preno.
 *
 ...
 */
final class RefAgenziaPrenoController extends BaseController
{
    ...
    private RefAgenziaPrenoService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new RefAgenziaPrenoService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('ref_agenzia_preno/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('ref_agenzia_preno/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('ref_agenzia_preno/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('ref_agenzia_preno/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(RefAgenziaPrenoRules::createRules(), RefAgenziaPrenoRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['ref_agenzia_preno']);
        ...
        return redirect()->to(site_url('ref_agenzia_preno'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('ref_agenzia_preno/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(RefAgenziaPrenoRules::updateRules($id), RefAgenziaPrenoRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('ref_agenzia_preno'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('ref_agenzia_preno'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('ref_agenzia_preno'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'ref_agenzia_preno', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'ref_agenzia_preno',
                languageGroup: 'RefAgenziaPreno',
    ...
    {
        return redirect()->to(site_url('ref_agenzia_preno') . '?' . http_build_query((array) $this->request->getGet()))
