...
use App\Services\ListinoObmpService;
use App\Validation\ListinoObmpRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per listino_obmp.
 *
 ...
 */
final class ListinoObmpController extends BaseController
{
    ...
    private ListinoObmpService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ListinoObmpService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('listino_obmp/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('listino_obmp/index', $data);
    }
  ...
  1 => 'tipologia_id',
        ...
        return view('listino_obmp/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ListinoObmpRules::createRules(), ListinoObmpRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['listino_id']);
        ...
        return redirect()->to(site_url('listino_obmp'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('listino_obmp/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ListinoObmpRules::updateRules($id), ListinoObmpRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('listino_obmp'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('listino_obmp'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('listino_obmp'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'listino_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'listino_obmp',
                languageGroup: 'ListinoObmp',
    ...
    {
        return redirect()->to(site_url('listino_obmp') . '?' . http_build_query((array) $this->request->getGet()))
