...
use App\Services\WrehOrdersService;
use App\Validation\WrehOrdersRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per wreh_orders.
 *
 ...
 */
final class WrehOrdersController extends BaseController
{
    ...
    private WrehOrdersService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new WrehOrdersService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('wreh_orders/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('wreh_orders/index', $data);
    }
    ...
    public function create()
    {
        ...
        return view('wreh_orders/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(WrehOrdersRules::createRules(), WrehOrdersRules::messages())) {
        ...
        return redirect()->to(site_url('wreh_orders'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('wreh_orders/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(WrehOrdersRules::updateRules($id), WrehOrdersRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('wreh_orders'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('wreh_orders'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('wreh_orders'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'order_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'wreh_orders',
                languageGroup: 'WrehOrders',
    ...
    {
        return redirect()->to(site_url('wreh_orders') . '?' . http_build_query((array) $this->request->getGet()))
