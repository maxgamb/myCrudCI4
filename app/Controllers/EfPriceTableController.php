...
use App\Services\EfPriceTableService;
use App\Validation\EfPriceTableRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per ef_price_table.
 *
 ...
 */
final class EfPriceTableController extends BaseController
{
    ...
    private EfPriceTableService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new EfPriceTableService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('ef_price_table/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('ef_price_table/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('ef_price_table/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    public function create()
    {
        ...
        return view('ef_price_table/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(EfPriceTableRules::createRules(), EfPriceTableRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['price_ef_is']);
        ...
        return redirect()->to(site_url('ef_price_table'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('ef_price_table/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(EfPriceTableRules::updateRules($id), EfPriceTableRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('ef_price_table'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('ef_price_table'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('ef_price_table'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'price_ef_is', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'ef_price_table',
                languageGroup: 'EfPriceTable',
    ...
    {
        return redirect()->to(site_url('ef_price_table') . '?' . http_build_query((array) $this->request->getGet()))
