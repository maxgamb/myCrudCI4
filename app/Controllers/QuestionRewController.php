...
use App\Services\QuestionRewService;
use App\Validation\QuestionRewRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per question_rew.
 *
 ...
 */
final class QuestionRewController extends BaseController
{
    ...
    private QuestionRewService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new QuestionRewService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('question_rew/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('question_rew/index', $data);
    }
  ...
  0 => 'question_id',
        ...
        return view('question_rew/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(QuestionRewRules::createRules(), QuestionRewRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['question_rew_id']);
        ...
        return redirect()->to(site_url('question_rew'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('question_rew/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(QuestionRewRules::updateRules($id), QuestionRewRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('question_rew'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('question_rew'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('question_rew'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'question_rew_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'question_rew',
                languageGroup: 'QuestionRew',
    ...
    {
        return redirect()->to(site_url('question_rew') . '?' . http_build_query((array) $this->request->getGet()))
