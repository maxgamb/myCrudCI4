...
use App\Validation\HotelsRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per hotels.
 *
 ...
 */
final class HotelsController extends BaseController
{
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new HotelsService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('hotels/_table', $data);
        }
    ...
    public function store()
    {
        ...
        if (!$this->validate(HotelsRules::createRules(), HotelsRules::messages())) {
        ...
        return redirect()->to(site_url('hotels'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(HotelsRules::updateRules($id), HotelsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('hotels'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('hotels'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('hotels'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'hotel_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('hotels') . '?' . http_build_query((array) $this->request->getGet()))
