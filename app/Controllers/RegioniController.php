...
use App\Services\RegioniService;
use App\Validation\RegioniRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per regioni.
 *
 ...
 */
final class RegioniController extends BaseController
{
    ...
    private RegioniService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new RegioniService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('regioni/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('regioni/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(RegioniRules::createRules(), RegioniRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['regione_id']);
        ...
        return redirect()->to(site_url('regioni'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('regioni/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(RegioniRules::updateRules($id), RegioniRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('regioni'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('regioni'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('regioni'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'regione_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('regioni') . '?' . http_build_query((array) $this->request->getGet()))
