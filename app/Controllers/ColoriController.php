...
use App\Services\ColoriService;
use App\Validation\ColoriRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per colori.
 *
 ...
 */
final class ColoriController extends BaseController
{
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ColoriService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('colori/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('colori/index', $data);
    }
  ...
  0 => 'col_preno_id',
    ...
    public function store()
    {
        ...
        if (!$this->validate(ColoriRules::createRules(), ColoriRules::messages())) {
        ...
        return redirect()->to(site_url('colori'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('colori/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ColoriRules::updateRules($id), ColoriRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(true);
        unset($data['colore_nome']);
        ...
        return redirect()->to(site_url('colori'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('colori'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('colori'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'colore_nome', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('colori') . '?' . http_build_query((array) $this->request->getGet()))
