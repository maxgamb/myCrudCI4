...
/**
 * Controller CRUD full per comuni.
 *
 ...
 */
final class ComuniController extends BaseController
{
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ComuniService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('comuni/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('comuni/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(ComuniRules::createRules(), ComuniRules::messages())) {
        ...
        return redirect()->to(site_url('comuni'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ComuniRules::updateRules($id), ComuniRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(true);
        unset($data['Comuni_Codice']);
        ...
        return redirect()->to(site_url('comuni'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('comuni'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('comuni'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'Comuni_Codice', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('comuni') . '?' . http_build_query((array) $this->request->getGet()))
