...
use App\Services\PagamentiSospesiService;
use App\Validation\PagamentiSospesiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per pagamenti_sospesi.
 *
 ...
 */
final class PagamentiSospesiController extends BaseController
{
    ...
    private PagamentiSospesiService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new PagamentiSospesiService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('pagamenti_sospesi/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('pagamenti_sospesi/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('pagamenti_sospesi/view', [
            'title' => 'Dettaglio',
            'row' => $row,
  ...
  0 => 'sospeso_id',
        ...
        return view('pagamenti_sospesi/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(PagamentiSospesiRules::createRules(), PagamentiSospesiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['pagamento_id']);
        ...
        return redirect()->to(site_url('pagamenti_sospesi'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('pagamenti_sospesi/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(PagamentiSospesiRules::updateRules($id), PagamentiSospesiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('pagamenti_sospesi'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('pagamenti_sospesi'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('pagamenti_sospesi'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'pagamento_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'pagamenti_sospesi',
                languageGroup: 'PagamentiSospesi',
    ...
    {
        return redirect()->to(site_url('pagamenti_sospesi') . '?' . http_build_query((array) $this->request->getGet()))
