...
use App\Services\AgenziaPrezziService;
use App\Validation\AgenziaPrezziRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per agenzia_prezzi.
 *
 ...
 */
final class AgenziaPrezziController extends BaseController
{
    ...
    private AgenziaPrezziService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new AgenziaPrezziService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('agenzia_prezzi/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('agenzia_prezzi/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('agenzia_prezzi/view', [
            'title' => 'Dettaglio',
            'row' => $row,
  ...
  0 => 'agenzia_prezzi_id',
        ...
        return view('agenzia_prezzi/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(AgenziaPrezziRules::createRules(), AgenziaPrezziRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['agenzia_prezzi_id']);
        ...
        return redirect()->to(site_url('agenzia_prezzi'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('agenzia_prezzi/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(AgenziaPrezziRules::updateRules($id), AgenziaPrezziRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('agenzia_prezzi'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('agenzia_prezzi'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('agenzia_prezzi'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'agenzia_prezzi_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'agenzia_prezzi',
                languageGroup: 'AgenziaPrezzi',
    ...
    {
        return redirect()->to(site_url('agenzia_prezzi') . '?' . http_build_query((array) $this->request->getGet()))
