...
use App\Services\TipDocService;
use App\Validation\TipDocRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per tip_doc.
 *
 ...
 */
final class TipDocController extends BaseController
{
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new TipDocService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('tip_doc/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('tip_doc/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(TipDocRules::createRules(), TipDocRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['tip_doc_id']);
        ...
        return redirect()->to(site_url('tip_doc'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('tip_doc/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(TipDocRules::updateRules($id), TipDocRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('tip_doc'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('tip_doc'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('tip_doc'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'tip_doc_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('tip_doc') . '?' . http_build_query((array) $this->request->getGet()))
