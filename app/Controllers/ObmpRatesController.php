...
use App\Services\ObmpRatesService;
use App\Validation\ObmpRatesRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_rates.
 *
 ...
 */
final class ObmpRatesController extends BaseController
{
    ...
    private ObmpRatesService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpRatesService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_rates/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_rates/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpRatesRules::createRules(), ObmpRatesRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['obmp_rate_id']);
        ...
        return redirect()->to(site_url('obmp_rates'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_rates/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpRatesRules::updateRules($id), ObmpRatesRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_rates'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_rates'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_rates'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'obmp_rate_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('obmp_rates') . '?' . http_build_query((array) $this->request->getGet()))
