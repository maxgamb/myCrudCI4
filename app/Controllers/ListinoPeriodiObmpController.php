...
use App\Services\ListinoPeriodiObmpService;
use App\Validation\ListinoPeriodiObmpRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per listino_periodi_obmp.
 *
 ...
 */
final class ListinoPeriodiObmpController extends BaseController
{
    ...
    private ListinoPeriodiObmpService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ListinoPeriodiObmpService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('listino_periodi_obmp/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('listino_periodi_obmp/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('listino_periodi_obmp/view', [
            'title' => 'Dettaglio',
            'row' => $row,
  ...
  0 => 'listino_nome_id',
        ...
        return view('listino_periodi_obmp/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ListinoPeriodiObmpRules::createRules(), ListinoPeriodiObmpRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['listino_periodi_id']);
        ...
        return redirect()->to(site_url('listino_periodi_obmp'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('listino_periodi_obmp/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ListinoPeriodiObmpRules::updateRules($id), ListinoPeriodiObmpRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('listino_periodi_obmp'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('listino_periodi_obmp'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('listino_periodi_obmp'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'listino_periodi_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'listino_periodi_obmp',
                languageGroup: 'ListinoPeriodiObmp',
    ...
    {
        return redirect()->to(site_url('listino_periodi_obmp') . '?' . http_build_query((array) $this->request->getGet()))
