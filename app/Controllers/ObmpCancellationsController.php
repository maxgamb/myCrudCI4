...
use App\Services\ObmpCancellationsService;
use App\Validation\ObmpCancellationsRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_cancellations.
 *
 ...
 */
final class ObmpCancellationsController extends BaseController
{
    ...
    private ObmpCancellationsService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpCancellationsService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_cancellations/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_cancellations/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('obmp_cancellations/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('obmp_cancellations/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpCancellationsRules::createRules(), ObmpCancellationsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['obmp_cancellation_id']);
        ...
        return redirect()->to(site_url('obmp_cancellations'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_cancellations/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpCancellationsRules::updateRules($id), ObmpCancellationsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_cancellations'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_cancellations'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_cancellations'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'obmp_cancellation_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'obmp_cancellations',
                languageGroup: 'ObmpCancellations',
    ...
    {
        return redirect()->to(site_url('obmp_cancellations') . '?' . http_build_query((array) $this->request->getGet()))
