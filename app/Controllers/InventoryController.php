...
use App\Validation\InventoryRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * CRUD full controller for resource `inventory`.
 *
...
final class InventoryController extends BaseController
{
  ...
  2 => 'store_id',
        ...
        if ($this->request->isAJAX()) {
            return view('inventory/_table', $data);
        }
  ...
  1 => 'store_id',
    ...
    public function store()
    {
        ...
        $createRules = InventoryRules::createRules();
        ...
        }        if (!$this->validate($createRules, InventoryRules::messages())) {
        ...
        $redirectUrl = $parentContext['url'] ?? $this->contextUrl('inventory', $navigationContext, $cascadeTrail);
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(InventoryRules::updateRules($id), InventoryRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to($this->contextUrl('inventory', $navigationContext, $cascadeTrail))->with('message', 'Record updated successfully.');
    }
    ...
    public function delete(int|string $id)
    {
        ...
        try {
            ...
            return redirect()->to($this->contextUrl('inventory', $navigationContext))->with('error', $e->getMessage());
        }
        return redirect()->to($this->contextUrl('inventory', $navigationContext))->with('message', 'Record deleted successfully.');
    }
    /**
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'inventory_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
), array (
  ...
  0 => 'last_update',
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = InventoryRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('inventory') . ($query === [] ? '' : '?' . http_build_query($query));
