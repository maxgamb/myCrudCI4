...
use App\Services\ContiNoteService;
use App\Validation\ContiNoteRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per conti_note.
 *
 ...
 */
final class ContiNoteController extends BaseController
{
    ...
    private ContiNoteService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ContiNoteService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('conti_note/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('conti_note/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(ContiNoteRules::createRules(), ContiNoteRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['conto_nota_id']);
        ...
        return redirect()->to(site_url('conti_note'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('conti_note/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ContiNoteRules::updateRules($id), ContiNoteRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('conti_note'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('conti_note'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('conti_note'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'conto_nota_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('conti_note') . '?' . http_build_query((array) $this->request->getGet()))
