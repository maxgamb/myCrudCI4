...
use App\Validation\EfTipologiaRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per ef_tipologia.
 *
 ...
 */
final class EfTipologiaController extends BaseController
{
    ...
    private EfTipologiaService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new EfTipologiaService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('ef_tipologia/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('ef_tipologia/index', $data);
    }
    ...
    public function create()
    {
        ...
        return view('ef_tipologia/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(EfTipologiaRules::createRules(), EfTipologiaRules::messages())) {
        ...
        return redirect()->to(site_url('ef_tipologia'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('ef_tipologia/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(EfTipologiaRules::updateRules($id), EfTipologiaRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('ef_tipologia'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('ef_tipologia'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('ef_tipologia'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'pax', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'ef_tipologia',
                languageGroup: 'EfTipologia',
    ...
    {
        return redirect()->to(site_url('ef_tipologia') . '?' . http_build_query((array) $this->request->getGet()))
