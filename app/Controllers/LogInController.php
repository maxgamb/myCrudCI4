...
/**
 * Controller CRUD full per log_in.
 *
...
final class LogInController extends BaseController
{
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('log_in/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('log_in/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(LogInRules::createRules(), LogInRules::messages())) {
        ...
        return redirect()->to(site_url('log_in'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('log_in/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(LogInRules::updateRules($id), LogInRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('log_in'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('log_in'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('log_in'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'log_in_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('log_in') . '?' . http_build_query((array) $this->request->getGet()))
