...
use App\Services\FilmTextService;
...
/**
 * Controller CRUD full per film_text.
 *
 ...
 */
final class FilmTextController extends BaseController
{
    ...
    private FilmTextService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new FilmTextService();
  ...
  1 => 'title',
        ...
        if ($this->request->isAJAX()) {
            return view('film_text/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('film_text/index', $data);
    }
    ...
    public function store()
    {
        ...
        $createRules = FilmTextRules::createRules();
        ...
        if (!$this->validate($createRules, FilmTextRules::messages())) {
        ...
        return redirect()->to($this->contextUrl('film_text', $navigationContext))->with('message', 'Record creato correttamente.');
    }
    public function edit(int|string $id)
    {
        $navigationContext = $this->navigationContextFromQuery();

        return view('film_text/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(FilmTextRules::updateRules($id), FilmTextRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to($this->contextUrl('film_text', $navigationContext))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        ...
        try {
            ...
            return redirect()->to($this->contextUrl('film_text', $navigationContext))->with('error', $e->getMessage());
        }
        return redirect()->to($this->contextUrl('film_text', $navigationContext))->with('message', 'Record eliminato correttamente.');
    }
    /**
  ...
  1 => 'title',
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = FilmTextRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('film_text') . ($query === [] ? '' : '?' . http_build_query($query));
