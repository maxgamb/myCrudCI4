...
use App\Validation\ObmpReviewRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_review.
 *
 ...
 */
final class ObmpReviewController extends BaseController
{
    ...
    private ObmpReviewService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpReviewService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_review/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_review/index', $data);
    }
    ...
    {
        return view('obmp_review/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpReviewRules::createRules(), ObmpReviewRules::messages())) {
        ...
        return redirect()->to(site_url('obmp_review'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_review/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpReviewRules::updateRules($id), ObmpReviewRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_review'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_review'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_review'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'review_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'obmp_review',
                languageGroup: 'ObmpReview',
    ...
    {
        return redirect()->to(site_url('obmp_review') . '?' . http_build_query((array) $this->request->getGet()))
