...
use App\Services\SospesiService;
use App\Validation\SospesiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per sospesi.
 *
 ...
 */
final class SospesiController extends BaseController
{
    ...
    private SospesiService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new SospesiService();
  ...
  1 => 'sopeso_societa',
    ...
    public function store()
    {
        ...
        if (!$this->validate(SospesiRules::createRules(), SospesiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['sospeso_id']);
        ...
        return redirect()->to(site_url('sospesi'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('sospesi/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(SospesiRules::updateRules($id), SospesiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('sospesi'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('sospesi'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('sospesi'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'sospeso_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('sospesi') . '?' . http_build_query((array) $this->request->getGet()))
