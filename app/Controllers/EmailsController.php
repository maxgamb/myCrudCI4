...
use App\Validation\EmailsRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per emails.
 *
 ...
 */
final class EmailsController extends BaseController
{
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new EmailsService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('emails/_table', $data);
        }
    ...
    public function store()
    {
        ...
        if (!$this->validate(EmailsRules::createRules(), EmailsRules::messages())) {
        ...
        return redirect()->to(site_url('emails'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(EmailsRules::updateRules($id), EmailsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('emails'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('emails'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('emails'))->with('message', 'Record eliminato correttamente.');
    }
  ...
  2 => 100,
    ...
    {
        return redirect()->to(site_url('emails') . '?' . http_build_query((array) $this->request->getGet()))
