...
use App\Services\TipoallogiatiService;
use App\Validation\TipoallogiatiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per tipoallogiati.
 *
 ...
 */
final class TipoallogiatiController extends BaseController
{
    ...
    private TipoallogiatiService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new TipoallogiatiService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('tipoallogiati/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('tipoallogiati/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('tipoallogiati/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    public function create()
    {
        ...
        return view('tipoallogiati/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(TipoallogiatiRules::createRules(), TipoallogiatiRules::messages())) {
        ...
        return redirect()->to(site_url('tipoallogiati'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('tipoallogiati/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(TipoallogiatiRules::updateRules($id), TipoallogiatiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(true);
        unset($data['Tip_all_Cod']);
        ...
        return redirect()->to(site_url('tipoallogiati'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('tipoallogiati'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('tipoallogiati'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'Tip_all_Cod', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'tipoallogiati',
                languageGroup: 'Tipoallogiati',
    ...
    {
        return redirect()->to(site_url('tipoallogiati') . '?' . http_build_query((array) $this->request->getGet()))
