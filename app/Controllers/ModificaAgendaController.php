...
use App\Services\ModificaAgendaService;
use App\Validation\ModificaAgendaRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per modifica_agenda.
 *
 ...
 */
final class ModificaAgendaController extends BaseController
{
    ...
    private ModificaAgendaService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ModificaAgendaService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('modifica_agenda/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('modifica_agenda/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('modifica_agenda/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('modifica_agenda/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ModificaAgendaRules::createRules(), ModificaAgendaRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['mod_agenda_id']);
        ...
        return redirect()->to(site_url('modifica_agenda'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('modifica_agenda/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ModificaAgendaRules::updateRules($id), ModificaAgendaRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('modifica_agenda'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('modifica_agenda'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('modifica_agenda'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'mod_agenda_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'modifica_agenda',
                languageGroup: 'ModificaAgenda',
    ...
    {
        return redirect()->to(site_url('modifica_agenda') . '?' . http_build_query((array) $this->request->getGet()))
