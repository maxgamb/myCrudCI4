...
use App\Services\ObmpRefEventService;
use App\Validation\ObmpRefEventRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per obmp_ref_event.
 *
 ...
 */
final class ObmpRefEventController extends BaseController
{
    ...
    private ObmpRefEventService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ObmpRefEventService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('obmp_ref_event/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('obmp_ref_event/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('obmp_ref_event/view', [
            'title' => 'Dettaglio',
            'row' => $row,
  ...
  2 => 'agenzia_id',
        ...
        return view('obmp_ref_event/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(ObmpRefEventRules::createRules(), ObmpRefEventRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['ref_event_id']);
        ...
        return redirect()->to(site_url('obmp_ref_event'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('obmp_ref_event/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ObmpRefEventRules::updateRules($id), ObmpRefEventRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('obmp_ref_event'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('obmp_ref_event'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('obmp_ref_event'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'ref_event_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'obmp_ref_event',
                languageGroup: 'ObmpRefEvent',
    ...
    {
        return redirect()->to(site_url('obmp_ref_event') . '?' . http_build_query((array) $this->request->getGet()))
