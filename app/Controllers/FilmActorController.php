...
/**
 * Controller CRUD full per film_actor.
 *
 ...
 */
final class FilmActorController extends BaseController
{
  ...
  array (
    ...
    private FilmActorService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new FilmActorService();
  ...
  1 => 'film_id',
        ...
        if ($this->request->isAJAX()) {
            return view('film_actor/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('film_actor/index', $data);
    }
  ...
  1 => 'film_id',
    ...
    public function store()
    {
        ...
        $createRules = FilmActorRules::createRules();
        ...
        if (!$this->validate($createRules, FilmActorRules::messages())) {
        ...
        return redirect()->to($this->contextUrl('film_actor', $navigationContext))->with('message', 'Record creato correttamente.');
    }
    /**
  ...
  0 => 'last_update',
    ...
    private function validateRelatedCreates(array $related): array
    {
        ...
        $definitions = FilmActorRules::relatedCreateRules();
        $errors = [];
    ...
    private function exportLimitRedirect(string $format, bool $unfiltered)
    {
        ...
        $url = site_url('film_actor') . ($query === [] ? '' : '?' . http_build_query($query));
