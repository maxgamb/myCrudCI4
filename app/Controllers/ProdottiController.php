...
use App\Validation\ProdottiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per prodotti.
 *
 ...
 */
final class ProdottiController extends BaseController
{
    ...
    private ProdottiService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new ProdottiService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('prodotti/_table', $data);
        }
    ...
    public function store()
    {
        ...
        if (!$this->validate(ProdottiRules::createRules(), ProdottiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['prodotto_id']);
        ...
        return redirect()->to(site_url('prodotti'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ProdottiRules::updateRules($id), ProdottiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('prodotti'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('prodotti'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('prodotti'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'prodotto_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('prodotti') . '?' . http_build_query((array) $this->request->getGet()))
