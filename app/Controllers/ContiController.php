...
use App\Services\ContiService;
use App\Validation\ContiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per conti.
 *
 ...
 */
final class ContiController extends BaseController
{
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('conti/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('conti/index', $data);
    }
  ...
  1 => 'camera_id',
    ...
    public function store()
    {
        ...
        if (!$this->validate(ContiRules::createRules(), ContiRules::messages())) {
        ...
        return redirect()->to(site_url('conti'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('conti/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(ContiRules::updateRules($id), ContiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('conti'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('conti'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('conti'))->with('message', 'Record eliminato correttamente.');
    }
  ...
  2 => 100,
    ...
    {
        return redirect()->to(site_url('conti') . '?' . http_build_query((array) $this->request->getGet()))
