...
use App\Services\BancaHotelService;
use App\Validation\BancaHotelRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per banca_hotel.
 *
 ...
 */
final class BancaHotelController extends BaseController
{
    ...
    private BancaHotelService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new BancaHotelService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('banca_hotel/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('banca_hotel/index', $data);
    }
    ...
    {
        return view('banca_hotel/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(BancaHotelRules::createRules(), BancaHotelRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['banca_hotel_id']);
        ...
        return redirect()->to(site_url('banca_hotel'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('banca_hotel/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(BancaHotelRules::updateRules($id), BancaHotelRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('banca_hotel'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('banca_hotel'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('banca_hotel'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'banca_hotel_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'banca_hotel',
                languageGroup: 'BancaHotel',
    ...
    {
        return redirect()->to(site_url('banca_hotel') . '?' . http_build_query((array) $this->request->getGet()))
