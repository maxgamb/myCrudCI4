...
use App\Validation\LettereRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per lettere.
 *
 ...
 */
final class LettereController extends BaseController
{
    ...
    private LettereService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new LettereService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('lettere/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('lettere/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(LettereRules::createRules(), LettereRules::messages())) {
        ...
        return redirect()->to(site_url('lettere'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('lettere/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(LettereRules::updateRules($id), LettereRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('lettere'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('lettere'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('lettere'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'lettere_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('lettere') . '?' . http_build_query((array) $this->request->getGet()))
