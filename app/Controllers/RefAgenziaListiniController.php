...
use App\Services\RefAgenziaListiniService;
use App\Validation\RefAgenziaListiniRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per ref_agenzia_listini.
 *
 ...
 */
final class RefAgenziaListiniController extends BaseController
{
    ...
    private RefAgenziaListiniService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new RefAgenziaListiniService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('ref_agenzia_listini/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('ref_agenzia_listini/index', $data);
    }
    ...
    public function view(int|string $id)
    {
        ...
        return view('ref_agenzia_listini/view', [
            'title' => 'Dettaglio',
            'row' => $row,
    ...
    {
        return view('ref_agenzia_listini/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(RefAgenziaListiniRules::createRules(), RefAgenziaListiniRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['ref_agenzia_listini_id']);
        ...
        return redirect()->to(site_url('ref_agenzia_listini'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('ref_agenzia_listini/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(RefAgenziaListiniRules::updateRules($id), RefAgenziaListiniRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('ref_agenzia_listini'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('ref_agenzia_listini'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('ref_agenzia_listini'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'ref_agenzia_listini_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'ref_agenzia_listini',
                languageGroup: 'RefAgenziaListini',
    ...
    {
        return redirect()->to(site_url('ref_agenzia_listini') . '?' . http_build_query((array) $this->request->getGet()))
