...
/**
 * Controller CRUD full per utenti.
 *
...
final class UtentiController extends BaseController
{
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new UtentiService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('utenti/_table', $data);
        }
  ...
  0 => 'staff_id',
    ...
    public function store()
    {
        ...
        if (!$this->validate(UtentiRules::createRules(), UtentiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['Utente_id']);
        ...
        return redirect()->to(site_url('utenti'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(UtentiRules::updateRules($id), UtentiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('utenti'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('utenti'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('utenti'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'Utente_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('utenti') . '?' . http_build_query((array) $this->request->getGet()))
