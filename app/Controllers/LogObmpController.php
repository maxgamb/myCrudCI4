...
use App\Services\LogObmpService;
use App\Validation\LogObmpRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per log_obmp.
 *
 ...
 */
final class LogObmpController extends BaseController
{
    ...
    private LogObmpService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new LogObmpService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('log_obmp/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('log_obmp/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(LogObmpRules::createRules(), LogObmpRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['log_obmp_id']);
        ...
        return redirect()->to(site_url('log_obmp'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('log_obmp/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(LogObmpRules::updateRules($id), LogObmpRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('log_obmp'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('log_obmp'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('log_obmp'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'log_obmp_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('log_obmp') . '?' . http_build_query((array) $this->request->getGet()))
