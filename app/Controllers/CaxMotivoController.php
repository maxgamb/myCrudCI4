...
use App\Services\CaxMotivoService;
use App\Validation\CaxMotivoRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per cax_motivo.
 *
 ...
 */
final class CaxMotivoController extends BaseController
{
    ...
    private CaxMotivoService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new CaxMotivoService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('cax_motivo/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('cax_motivo/index', $data);
    }
    ...
    public function store()
    {
        ...
        if (!$this->validate(CaxMotivoRules::createRules(), CaxMotivoRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['cax_motivo_id']);
        ...
        return redirect()->to(site_url('cax_motivo'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('cax_motivo/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(CaxMotivoRules::updateRules($id), CaxMotivoRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('cax_motivo'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('cax_motivo'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('cax_motivo'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'cax_motivo_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('cax_motivo') . '?' . http_build_query((array) $this->request->getGet()))
