...
use App\Services\RegistroPsService;
use App\Validation\RegistroPsRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per registro_ps.
 *
 ...
 */
final class RegistroPsController extends BaseController
{
    ...
    private RegistroPsService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new RegistroPsService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('registro_ps/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('registro_ps/index', $data);
    }
    ...
    {
        return view('registro_ps/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(RegistroPsRules::createRules(), RegistroPsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['registro_ps_id']);
        ...
        return redirect()->to(site_url('registro_ps'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('registro_ps/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(RegistroPsRules::updateRules($id), RegistroPsRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('registro_ps'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('registro_ps'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('registro_ps'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'registro_ps_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'registro_ps',
                languageGroup: 'RegistroPs',
    ...
    {
        return redirect()->to(site_url('registro_ps') . '?' . http_build_query((array) $this->request->getGet()))
