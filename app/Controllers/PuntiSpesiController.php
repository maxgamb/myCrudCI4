...
use App\Services\PuntiSpesiService;
use App\Validation\PuntiSpesiRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per punti_spesi.
 *
 ...
 */
final class PuntiSpesiController extends BaseController
{
    ...
    private PuntiSpesiService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new PuntiSpesiService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('punti_spesi/_table', $data);
        }

        $data['options'] = $this->gateway->relationOptions();

        return view('punti_spesi/index', $data);
    }
  ...
  1 => 'conto_id',
        ...
        return view('punti_spesi/create', [
            'title' => 'Nuovo record',
            'row' => null,
    ...
    public function store()
    {
        ...
        if (!$this->validate(PuntiSpesiRules::createRules(), PuntiSpesiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }

        $data = $this->formData(false);
        unset($data['punti_spesi_id']);
        ...
        return redirect()->to(site_url('punti_spesi'))->with('message', 'Record creato correttamente.');
    }

    public function edit(int|string $id)
    {
        return view('punti_spesi/edit', [
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(PuntiSpesiRules::updateRules($id), PuntiSpesiRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('punti_spesi'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('punti_spesi'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('punti_spesi'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'punti_spesi_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
        ...
        try {
            return $this->exporter->download(
                ...
                filename: 'punti_spesi',
                languageGroup: 'PuntiSpesi',
    ...
    {
        return redirect()->to(site_url('punti_spesi') . '?' . http_build_query((array) $this->request->getGet()))
