...
use App\Validation\QuestionRules;
use CodeIgniter\Exceptions\PageNotFoundException;
use RuntimeException;
use Throwable;

/**
 * Controller CRUD full per question.
 *
...
final class QuestionController extends BaseController
{
    ...
    private QuestionService $gateway;
    ...
    {
        helper(['form', 'url']);
        $this->gateway = new QuestionService();
  ...
  2 => 100,
        ...
        if ($this->request->isAJAX()) {
            return view('question/_table', $data);
        }
    ...
    public function store()
    {
        ...
        if (!$this->validate(QuestionRules::createRules(), QuestionRules::messages())) {
        ...
        return redirect()->to(site_url('question'))->with('message', 'Record creato correttamente.');
    }
    ...
    public function update(int|string $id)
    {
        ...
        if (!$this->validate(QuestionRules::updateRules($id), QuestionRules::messages())) {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
        ...
        return redirect()->to(site_url('question'))->with('message', 'Record aggiornato correttamente.');
    }
    ...
    public function delete(int|string $id)
    {
        try {
            ...
            return redirect()->to(site_url('question'))->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('question'))->with('message', 'Record eliminato correttamente.');
    }
    ...
    private function export(string $format)
    {
        ...
        $listRequest = CrudListRequest::fromRequest($this->request, 'question_id', array (
  0 => 25,
  1 => 50,
  2 => 100,
));
    ...
    {
        return redirect()->to(site_url('question') . '?' . http_build_query((array) $this->request->getGet()))
